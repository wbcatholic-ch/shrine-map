...
(function(){
  ...
  /* V8-1-14-353: 순례길 지도 확대/축소 체감 개선을 위해 마커 이미지를 캐시하고
  ...
  function relayoutTrailMap(delay, reason){
    ...
    if(isFoldViewport){
      try{ document.documentElement.classList.add('oai-trail-map-settling'); }catch(_e){}
    }
    setTimeout(function(){
      if(!(trailState.map && window.kakao && window.kakao.maps)){
        if(isFoldViewport){ try{ document.documentElement.classList.remove('oai-trail-map-settling'); }catch(_e){} }
        return;
      }
      try{
        ...
        if(isFoldViewport){
          setTimeout(function(){
            ...
            try{ document.documentElement.classList.remove('oai-trail-map-settling'); }catch(_e){}
          }, 180);
        }
      }catch(e){ console.warn("[가톨릭길동무]", e); if(isFoldViewport){ try{ document.documentElement.classList.remove('oai-trail-map-settling'); }catch(_e){} } }
