...
(function(){
  ...
  /* V8-1-14-466: 순례길 지도 확대/축소 체감 개선을 위해 마커 이미지를 캐시하고
  ...
  function showHantiRouteOverlays(routeData){
    ...
    clearHantiRouteOverlays();
    try{
      /* V8-1-14-466: 원본 GPX segment를 강제 병합하지 않고 그대로 그린다.
  ...
  function relayoutTrailMap(delay, reason){
    ...
    /* V8-1-14-466: 순례길 지도 relayout은 공통 Fold 관리자 흐름에서만 보정한다. */
    setTimeout(function(){
  ...
  window.trailOpenSheet = function(i){
    ...
    if(hantiSelected){
      setTrailHantiNote('');
      // V8-1-14-466: 버튼과 중복되는 '홈페이지 · 전체경로보기' 안내문구는 숨긴다.
