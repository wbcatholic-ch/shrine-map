...
(function(){
  ...
  window.addEventListener('popstate', function(){
    ...
    if(isGuideModalOpen()){
      _restoring = false;
      closeGuideModals();
      /* 팝업 닫힌 후 커버 트랩을 [_p:0, _p:1] 두 개로 완전 재확립한다.
         closeMassQuickMenu() 내 _ensureCoverBackTrap()이 이미 트랩을 심은 경우에도
         pushState만 추가하면 트랩이 3개 이상 쌓여 종료 안내 없이 앱이 탈출되는 버그가 발생한다.
         replaceState로 현재 엔트리를 _p:0으로 확정한 뒤 pushState로 _p:1을 1개만 심는다. */
      try{
        history.replaceState({_p:0}, '', _href);
        history.pushState({_p:1}, '', _href);
      }catch(e){ console.warn("[가톨릭길동무]", e); }
