...
(function(){
  ...
  /* ─────────────────────────────────────────────
     V2-S-r3 기도문 전용 뒤로가기 컨트롤러 — history 단계 분리 제거
  ...
  function armPrayerBackTrap(reason){
    /* 호환용 함수. V2-S-r3부터 기도문 detail/list용 별도 pushState는 만들지 않는다.
  ...
  function settleCoverTrapAfterPrayer(reason){
    // V2-S-r3: 기도문 팝업 → 커버 후에는 이미 공통 컨트롤러가 history.go(1)로 trap을 복원한 상태다.
  ...
  function prayerListToPopupOrCover(reason){
    try{
      ...
      /* V2-S-r3: 기도문 목록 → 빠른메뉴 팝업 복귀는 직접 팝업을 띄우지 않는다.
...
(function(){
  ...
  // V2-S-r3: 커버 글자 크기 조절은 prayer.js에 의존하지 않는 공통 함수가 담당한다.
  // prayer.js는 기도문 화면이 열렸을 때 같은 localStorage 값을 읽어 자체 UI를 맞춘다.
  var QA_URL="qa-firebase.html?v=V2-S-r3";
