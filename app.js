...
function markExternalReturnStabilize(kind){
  // V21: 외부 사이트 이동은 브라우저 기본 동작에 맡긴다.
...
function oaiSmoothNavigate(url, kind){
  // V21: 호환용 함수. 보호막/지연/전역 가로채기 없이 즉시 이동한다.
  if(!url) return;
...
function applyExternalReturnStabilize(){
  // V21: 복귀 시 화면을 재계산하지 않고, 예전 이동중 잔여 상태만 제거한다.
  try{ oaiClearExternalNavigationState(); }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
function refreshAppFilesOnly(){
  ...
  try{
    ...
    // V21: 새로고침 전에는 레이아웃/스크롤/모달 DOM을 건드리지 않고,
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || 'V21';
    var current = '';
...
// V21: 커버 전용 주요 기능 안내. 별도 파일 없이 작은 자동 안내 + 자세한 카드형 팝업을 제공한다.
(function(){
  'use strict';
  var HIDE_DAYS = 7;
  ...
  function maybeShowIntro(){
    ...
    if(forceRefresh){
      // V21: 안정형 새로고침 뒤에는 어떤 커버 팝업도 자동으로 다시 띄우지 않는다.
...
// V21: iPhone 카카오톡 인앱 브라우저에서만 Safari 설치 안내 배너를 표시한다.
(function(){
  'use strict';
...
function restoreCoreReturnState(){
  ...
  // V21: 외부사이트 복귀 시 지도 중심을 두 단계로 움직이지 않는다.
...
// ─── API 키 및 REST 프록시 설정 ─────────────────────────────────────
// V21: 카카오 REST 호출은 Cloudflare Worker 프록시를 사용합니다.
...
/* V21: 카카오 REST 호출은 Cloudflare Worker 프록시로 처리합니다. */
...
function _getInfoCardCenterTargetY(mapH){
  // V21: 성지·성당·피정 지도 중심은 항상 인포카드가 올라왔을 때의 기준으로 통일한다.
...
function closeInfoCard(opts){
  ...
  // V21: 시트 전환으로 닫을 때는 지도 중심을 다시 움직이지 않는다.
  // 사용자가 X/지도 터치로 인포카드만 닫을 때는 기존 V21 기준 중심을 유지한다.
...
function _mkrImg(color,big){
  const w=big?40:28,h=big?52:36;
  // V21: iPhone/Android marker cross unified to the cleaner text-glyph marker.
...
function _focusParishPointAround(lat, lng, opts){
  ...
  try{
    ...
    // V21: 현재 위치/내 주변/선택 성당 모두 인포카드 기준 중심으로 통일한다.
...
function _drawLine(s1,s2,path){
  ...
  // V21: 길찾기 경로도 성지·성당·피정의집 일반 인포카드와 같은 중심 기준을 사용한다.
