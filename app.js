...
function markExternalReturnStabilize(kind){
  try{
    ...
    // V8-1-14-326: 외부 이동 전 보호창은 #oai-external-entry-guard로 유지하고, 숨김 복귀 때도 같은 보호창을 이어서 사용한다.
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
/* V8-1-14-326:
   V315의 안정된 커버 Back 흐름은 그대로 두고, Fold reload 때 현재 카테고리 화면만 1회 복귀한다.
   - pagehide마다 전체 화면 자동복원 금지
   - 성지/성당/피정은 현재 tab/sheet만 최소 저장
   - Fold 복귀 중 _autoLocate / nearby 거리계산 금지
   - 복귀 완료 후 app 화면용 history root/trap만 다시 심음 */
(function(){
  'use strict';
  if(window.__OAI_FOLD_SURFACE_V326__) return;
  window.__OAI_FOLD_SURFACE_V326__ = true;

  var KEY = 'oai_fold_surface_v326';
  ...
  function now(){ return Date.now ? Date.now() : new Date().getTime(); }
  function width(){ try{ return Math.round(window.innerWidth || document.documentElement.clientWidth || 0); }catch(_e){ return 0; } }
  function ssGet(k){ try{ return sessionStorage.getItem(k) || ''; }catch(_e){ return ''; } }
  function ssSet(k,v){ try{ sessionStorage.setItem(k, v); }catch(_e){} }
  function ssDel(k){ try{ sessionStorage.removeItem(k); }catch(_e){} }
  function visibleCover(){ try{ return typeof window._isCoverScreenVisible === 'function' ? window._isCoverScreenVisible() : !document.documentElement.classList.contains('app-active'); }catch(_e){ return false; } }
  function appActive(){
    ...
    try{ return document.documentElement.classList.contains('app-active') && !visibleCover(); }catch(_e){ return false; }
  }
  function modeOk(m){ return m === 'shrine' || m === 'parish' || m === 'retreat'; }
  function readState(){ try{ var raw=ssGet(KEY); return raw ? JSON.parse(raw) : null; }catch(_e){ return null; } }
  function writeState(st){ try{ ssSet(KEY, JSON.stringify(st)); }catch(_e){} }
  function markNoIntro(){ try{ ssSet(KEY_NO_INTRO, String(now()+120000)); }catch(_e){} }
  function getOpenTab(){
  ...
  function record(reason){
    try{
      ...
      var m = (typeof _mode !== 'undefined') ? _mode : '';
  ...
  function validForRestore(st){
    try{
      if(!st || st.kind !== 'category' || !modeOk(st.mode)) return false;
      if(!st.ts || now() - st.ts > MAX_AGE) return false;
      var cw = width();
      if(st.w && cw && Math.abs(cw - Number(st.w || 0)) >= WIDTH_DELTA) return true;
      var ni = Number(ssGet(KEY_NO_INTRO) || 0);
      return !!(ni && now() < ni && st.w && cw);
    }catch(_e){ return false; }
  }
  function armAppHistory(reason){
    try{
      if(!appActive()) return false;
      if(visibleCover()) return false;
      if(typeof window._oaiArmAppBackTrap === 'function'){
        window._oaiArmAppBackTrap(reason || 'fold-surface', {force:true});
        return true;
      }
      var href = location.href.split('#')[0];
      history.replaceState({_p:0, oai_app_root:reason || 'fold-surface-root'}, '', href);
      history.pushState({_p:1, oai_app_trap:reason || 'fold-surface-trap'}, '', href);
      return true;
    }catch(_e){ return false; }
  }
  window.oaiArmFoldAppHistory = armAppHistory;
  ...
  window.oaiApplyFoldSurfaceAfterMap = function(reason){
    try{
      var st = window.__OAI_FOLD_SURFACE_STATE__ || readState();
      if(!st || !modeOk(st.mode)) return false;
      window.__OAI_FOLD_SURFACE_RESTORE_UNTIL__ = now() + 7000;
      applyCamera(st);
      var tab = st.tab || 'map';
      if(tab === 'nearby' || tab === 'list' || tab === 'region'){
        if(typeof openTab === 'function') openTab(tab, {skipNearbyLoad:true});
        restoreBody(st);
      }else if(tab === 'route'){
        if(typeof openTab === 'function') openTab('route');
      }
      setTimeout(function(){ armAppHistory(reason || 'fold-map-applied'); }, 80);
      setTimeout(function(){ armAppHistory((reason || 'fold-map-applied') + '-late'); }, 420);
      st.w = width(); st.ts = now(); st.restored = true; writeState(st);
      return true;
    }catch(_e){ return false; }
  };

  window.oaiRecordFoldSurface = record;

  function tryRestore(reason){
    try{
      if(_restoreStarted) return false;
      var st = readState();
      if(!validForRestore(st)) return false;
      _restoreStarted = true;
      markNoIntro();
      window.__OAI_FOLD_SURFACE_STATE__ = st;
      window.__OAI_FOLD_SURFACE_RESTORE_UNTIL__ = now() + 9000;
      ...
      if(typeof startApp === 'function'){
        startApp(st.mode, {foldSurfaceRestore:true, foldSurface:st});
        setTimeout(function(){ armAppHistory('fold-restore-start'); }, 250);
        return true;
      }
    }catch(_e){}
    return false;
  ...
  function scheduleRestore(reason){
    setTimeout(function(){ tryRestore(reason || 'schedule'); }, 120);
    setTimeout(function(){ tryRestore((reason || 'schedule') + '-late'); }, 520);
  }
  window.oaiTryRestoreFoldSurface = tryRestore;
  ...
  document.addEventListener('visibilitychange', function(){
    try{
      ...
      else setTimeout(function(){ armAppHistory('visible'); }, 180);
    }catch(_e){}
  }, {passive:true});
  ...
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ scheduleRestore('dom'); }, {once:true});
  else scheduleRestore('dom-ready');
  window.addEventListener('load', function(){ scheduleRestore('load'); }, {once:true});
})();
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-326: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V8-1-14-326: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.APP_VERSION || 'V8-1-14-326');
    var current = '';
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-326'));
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-326';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-326';
}
...
/* V8-1-14-326: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V8-1-14-326: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V8-1-14-326: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V8-1-14-326: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V8-1-14-326: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V8-1-14-326: 동작 없는 빈 포커스 리스너와 빈 지도 진입 훅 제거 */
...
/* V8-1-14-326: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V8-1-14-326';
...
const _PRAYER_ASSET_VERSION='V8-1-14-326';
...
const _RETREAT_ASSET_VERSION='V8-1-14-326';
...
// V8-1-14-326: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
const _SHRINE_ASSET_VERSION='V8-1-14-326';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-326: 성지/성당/피정 진입은 cover를 내리기 전 내위치 안내 화면을 먼저 열어 첫 화면으로 고정한다.
...
function oaiPreopenNearbySheetForCategory(){
  try{
    ...
    if(body){
      // V8-1-14-326: 데이터 지연 후 startApp이 다시 호출되어도 같은 내주변 로딩 화면을 다시 그리지 않는다.
      try{
...
function startApp(mode, opts){
  opts = opts || {};
  var oaiFoldSurfaceRestore = !!(opts && opts.foldSurfaceRestore);
  var oaiFoldSurfaceState = opts && opts.foldSurface ? opts.foldSurface : null;
  if(oaiFoldSurfaceRestore){
    try{ window.__OAI_FOLD_SURFACE_RESTORE_UNTIL__ = (Date.now ? Date.now() : new Date().getTime()) + 9000; }catch(_e){}
    try{ window.__OAI_FOLD_SURFACE_STATE__ = oaiFoldSurfaceState || window.__OAI_FOLD_SURFACE_STATE__ || null; }catch(_e){}
  }
  if(mode==='shrine' && (!_shrineRawLoaded || !SHRINES.length)){
    _mode='shrine';
    try{
      if(!oaiFoldSurfaceRestore){ try{ oaiPreopenNearbySheetForCategory(); }catch(_e){} }
      const cover=$('cover');
  ...
  _filterDio='all';
  _listSrch='';
  window._noAutoNearby = !!oaiFoldSurfaceRestore;  // 직접 진입 시 nearby 열기 허용, Fold 복귀는 자동 내주변 금지
 ...
 _renderDioFilterBars();  _screen='map';
  if((mode==='shrine' || mode==='parish' || mode==='retreat') && !oaiFoldSurfaceRestore){
  ...
  setTimeout(function(){ try{ if(typeof window.oaiRecordFoldSurface === 'function') window.oaiRecordFoldSurface('startApp:'+mode); }catch(_e){} }, 160);
}
...
function goToCover(){
  ...
  // V8-1-14-326: 커버 복귀 때 화면 높이/폰 화면 크기를 다시 계산하지 않는다.
}
...
function _onMapReady(){
 const KM=kakao.maps;window._LL=KM.LatLng;window._MM=KM.Marker;window._MI=KM.MarkerImage;window._SZ=KM.Size;window._PT=KM.Point;window._LB=KM.LatLngBounds;window._PL=KM.Polyline;window._EL=KM.event.addListener;
  ...
  renderList();
  var __oaiFoldMapRestoreActive = !!(window.__OAI_FOLD_SURFACE_RESTORE_UNTIL__ && (Date.now ? Date.now() : new Date().getTime()) < window.__OAI_FOLD_SURFACE_RESTORE_UNTIL__);
  if(!__oaiFoldMapRestoreActive) _autoLocate();
  if(_mode==='parish') { _buildParishDioSystem(); _syncParishDioLabels(); }
  else if(_mode==='retreat') _buildRetreatMarkers();
  if(!window._noAutoNearby && !__oaiFoldMapRestoreActive){
    openTab('nearby');
  }
  window._noAutoNearby = false;
  if(__oaiFoldMapRestoreActive){
    setTimeout(function(){ try{ if(typeof window.oaiApplyFoldSurfaceAfterMap === 'function') window.oaiApplyFoldSurfaceAfterMap('map-ready'); }catch(_e){} }, 120);
    setTimeout(function(){ try{ if(typeof window.oaiArmFoldAppHistory === 'function') window.oaiArmFoldAppHistory('map-ready'); }catch(_e){} }, 260);
  }
  try{ _updateShrineVisitMapFilterUI(); _updateShrineVisitCardsButtonUI(); }catch(e){ console.warn('[가톨릭길동무]', e); }
  if(typeof oaiHideCategoryEntryVeil==='function') setTimeout(oaiHideCategoryEntryVeil, 260);
}
...
function openTab(name, opts){
  ...
  var oaiSkipNearbyLoad = opts.skipNearbyLoad === true;
  ...
  if(name==='nearby'){
    try{ window.__OAI_NEARBY_PREOPEN_MODE__ = null; }catch(_e){}
    if(!oaiSkipNearbyLoad) _loadNearby();
    else {
      try{ window.__OAI_SHRINE_NEARBY_DISTANCE_DONE__=true; window.__OAI_SHRINE_NEARBY_LOADING__=false; }catch(_e){}
    }
  }
  ...
  setTimeout(function(){ try{ if(typeof window.oaiRecordFoldSurface === 'function') window.oaiRecordFoldSurface('openTab:'+name); }catch(_e){} }, 80);
}
...
function _autoLocate(){
  try{ var _oaiNow = Date.now ? Date.now() : new Date().getTime(); if(window.__OAI_SKIP_AUTO_LOCATE_UNTIL__ && _oaiNow < window.__OAI_SKIP_AUTO_LOCATE_UNTIL__) return; }catch(_e){}
...
function _loadNearby(){
  ...
  if(!body) return;
  try{
    var _oaiNow = Date.now ? Date.now() : new Date().getTime();
    if(window.__OAI_SUPPRESS_NEARBY_LOAD_UNTIL__ && _oaiNow < window.__OAI_SUPPRESS_NEARBY_LOAD_UNTIL__){
      window.__OAI_SHRINE_NEARBY_DISTANCE_DONE__=true;
      window.__OAI_SHRINE_NEARBY_LOADING__=false;
      window.__OAI_SHRINE_NEARBY_AWAITING_FRESH__=false;
      try{ _updateShrineVisitCardsButtonUI(); _updateShrineNearbyLocationButtonUI(); }catch(_e){}
      return;
    }
  }catch(_e){}
  _cancelNearbyLoad();
  ...
  try{ _updateShrineVisitCardsButtonUI(); }catch(_e){}
  try{ _updateShrineNearbyLocationButtonUI(); }catch(_e){}
  // V8-1-14-326: 카테고리 진입 직후 이미 열린 로딩 화면은 유지하고, 빈 화면/이전 목록일 때만 로딩 화면을 그린다.
  try{
    if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
      body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
    }
...
function _loadNearbyWithDist(lat,lng,items,getIdx,getColor,getLabel){
  ...
  if(body && _isNearbyLoadCurrent(requestMode,requestToken,body)){
    // V8-1-14-326: 자동차 거리 계산 단계에서 같은 로딩 화면을 다시 렌더링하지 않는다.
    try{
      if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
        body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
      }
