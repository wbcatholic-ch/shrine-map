...
function markExternalReturnStabilize(kind){
  try{
    ...
    // V8-1-14-612: 외부 이동 전 보호창은 #oai-external-entry-guard로 유지하고, 숨김 복귀 때도 같은 보호창을 이어서 사용한다.
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
function oaiApplyPendingRefreshVeil(){
  try{
    // V8-1-14-612: 새로고침 보호창은 짧은/긴 새로고침 모두 reload 전 현재 문서에서만 1회 사용한다.
...
(function(){
  ...
  function markBackgrounded(reason){
    try{
      ...
      /* V8-1-14-612: 앱이 백그라운드 중 WebView/PWA가 재로딩되어 index.html early boot만 먼저 실행되는 경우에도
...
function _setFaithFrameLoading(on, kind, url){
  // V8-1-14-612: 매일미사 iframe 진입/복귀 보호창은 이 함수와 _replaceFaithFrameWith에서만 관리한다.
  try{
...
function _openFaithExternalSite(info, kind){
  ...
  // V8-1-14-612: 성가/성경 외부사이트도 비상 fallback까지 진입·복귀 보호창 기준을 유지한다.
...
function openFaithPortal(kind, opts){
  ...
  if(kind==='hymn' || kind==='bible'){
    /* V8-1-14-612: 성가·성경은 신앙 외부사이트 단일 진입 함수로 열고, 진입/복귀 보호창 기준을 유지한다. */
    try{
      if(typeof _setFaithReturnTarget === 'function') _setFaithReturnTarget('massQuick');
      if(typeof _setMassQuickReturn === 'function') _setMassQuickReturn(true);
    }catch(e){ console.warn('[가톨릭길동무]', e); }
...
function _getShrineVisitActionLayerTarget(){
  try{
    /* V8-1-14-612: 나의 성지순례 현황 버튼은 리스트 안으로 이동하지 않는다.
...
function _updateShrineNearbyLocationButtonUI(){
  try{
    ...
    if(!btn) return;
    /* V8-1-14-612: 성지 내위치 버튼은 성당/피정의집과 같은 지도 조작 버튼이다.
...
function _updateShrineVisitFloatingListButtonUI(){
  ...
  /* V8-1-14-612: 버튼 표시 위치는 지도 기준 하단 하나로 고정한다.
  ...
  /* V8-1-14-612: 버튼 위치 기준은 지도 하단 하나로 유지하되,
...
function _renderShrineVisitDetail(idx){
  ...
  const telHref=item.tel?'tel:'+String(item.tel).replace(/[^0-9+]/g,''):'';
    // V8-1-14-612: 상세 인포카드 색상은 style.css의 공통 클래스에서만 관리한다.
...
function _renderShrineVisitCardsModal(){
  ...
  body.innerHTML='<div class="shrine-visit-card-grid">'+entries.map(function(entry){
    ...
    // V8-1-14-612: 카드 디자인은 style.css 단일 기준으로 관리하고, 신규 탭 방문 성지는 순례도장도 표시한다.
...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V8-1-14-612: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function _runRefreshAppFilesOnly(){
  ...
  /* V8-1-14-612: 짧은 새로고침과 긴 새로고침은 같은 보호창/로딩십자가 흐름을 쓴다.
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.OAI_APP_BUILD_VERSION || window.APP_VERSION || 'V8-1-14-612');
    var current = '';
    /* V8-1-14-612:
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-612'));
  ...
  function bindGuide(){
    /* V8-1-14-612: legacy direct guide-button binding removed; cover menu handler owns this entry. */
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-612';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-612';
}
...
/* V8-1-14-612: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V8-1-14-612: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V8-1-14-612: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V8-1-14-612: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
    if(typeof oaiSmoothNavigate === 'function'){
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V8-1-14-612: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V8-1-14-612: route reset helper is kept without inactive focus/map-entry hooks. */
...
/* V8-1-14-612: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V8-1-14-612';
...
const _PRAYER_ASSET_VERSION='V8-1-14-612';
...
const _RETREAT_ASSET_VERSION='V8-1-14-612';
...
// V8-1-14-612: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
function _setMarkerMapIfChanged(marker, map){
  ...
  try{
    ...
    /* V8-1-14-612: 일부 Kakao Marker는 getMap이 없어 __oaiMapTarget만 남는데,
...
const _SHRINE_ASSET_VERSION='V8-1-14-612';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function oaiEnterView(el){
  ...
  try{
    ...
    // V8-1-14-612: 카테고리 첫 화면은 nearby-distance-loading이 담당한다.
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-612: 성지/성당/피정 진입은 내주변 거리계산 화면이 기준이다.
...
function oaiPreopenNearbySheetForCategory(){
  try{
    ...
    if(body){
      // V8-1-14-612: 데이터 지연 후 startApp이 다시 호출되어도 같은 내주변 로딩 화면을 다시 그리지 않는다.
...
function goToCover(){
  ...
  // V8-1-14-612: 커버 복귀 때 화면 크기 class/cover-vh를 다시 계산하지 않는다. 저장된 화면 기준을 유지한다.
}
...
function openTab(name, opts){
  ...
  // V8-1-14-612: 성지/성당/피정의집 시트 전환은 무거운 가로 swiper 애니메이션 없이 즉시 전환하고, 내부 좌우 스와이프 전환은 비활성화한다.
...
function _scheduleNearbyMyLocationPlainRestore(reason){
  try{
    ...
    // V8-1-14-612: 성지·성당·피정의집 내주변 목록을 닫은 뒤에는
...
function _showInfoCard(item, idx){
  // V8-1-14-612: 길찾기 탭/경로검색 시트가 열린 동안에는
...
function _buildShrineMarkers(){
  ...
  idx=end;
  if(idx<SHRINES.length) setTimeout(function(){ requestAnimationFrame(next); }, 8);
  }
...
function _focusParishPointAround(lat, lng, opts){
  ...
  try{
    ...
    // V8-1-14-612: 성당의 현재 위치 기준 화면은 성지/피정과 동일하게
...
function _showParishDioMkrs(code){
  ...
  /* V8-1-14-612: 성당 지도는 idle 때마다 교구 전체 마커 setMap을 반복해
...
function _updateParishViewport(code){
  ...
  /* V8-1-14-612: 성당 교구 마커는 한 번에 모두 setMap하지 않고 작은 묶음으로 나눠
     지도 확대/이동 직후 WebView가 멈칫하는 느낌을 줄인다. */
  let i=0;
  const batch=24;
  (function applyBatch(){
    if(_mode!=='parish' || !_map || !_dioMkrs[code]) return;
    const end=Math.min(i+batch, mkrs.length);
    for(;i<end;i++){
      try{ _setMarkerMapIfChanged(mkrs[i],_map); }catch(e){ console.warn('[가톨릭길동무]',e); }
    }
    if(i<mkrs.length) setTimeout(function(){ requestAnimationFrame(applyBatch); }, 8);
  })();
}
...
function _buildRetreatMarkers(){
  ...
  if(!_retreatMarkers.length){
    ...
    const batch=18;
    function buildBatch(){
      if(_mode!=='retreat' || !_map){ window.__OAI_RETREAT_MARKER_BUILDING__ = false; return; }
      ...
      if(i<RETREATS.length) setTimeout(function(){ requestAnimationFrame(buildBatch); }, 8);
  ...
  try{
    _GEO.getCurrentPosition(ok,function(e){
      ...
      if(elapsed < 300) setTimeout(function(){ fallback(e); }, 150);
        ...
        if(fail) fail(e);
        /* V8-1-14-612: 빠른 위치가 실패했을 때도 UI를 오래 잡아두지 않고,
...
function _loadNearbyWithDist(lat,lng,items,getIdx,getColor,getLabel){
  ...
  if(body && _isNearbyLoadCurrent(requestMode,requestToken,body)){
    // V8-1-14-612: 자동차 거리 계산이 끝나기 전에는 예상 목록을 먼저 보여주지 않는다.
...
function _enterRouteMode(){
  ...
  // V8-1-14-612: 경로검색 시트가 열리면 기존 장소 정보카드는 숨겨
...
function _syncRouteWaypointBoxes(){
  ...
  // V8-1-14-612: 경로 결과판이 보이는 동안에는 +경유지를 완전히 숨긴다.
...
function _updateSearchBtn(){
  ...
  /* V8-1-14-612: 경로검색 버튼은 출발/도착창 아래에서 항상 보이고,
...
function clearRoute(role, opts){
  ...
  if(_isRouteWaypointRole(role) && (_getRoutePointByRole(role)||_getRouteWaypointEnabledByRole(role))){
    ...
    if(keepWaypointBox){
      // V8-1-14-612: 경유지 마커 취소 확인창에서 취소할 때는
...
(function(){
  ...
  function _doMainSwipe(dx){
    ...
    /* V8-1-14-612: 성지·성당·피정의집 좌우 스와이프는 하단 시트 내부에서 시작한 한 손가락 터치만 처리한다.
