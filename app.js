...
function markExternalReturnStabilize(kind){
  try{
    ...
    // V8-1-14-599: 외부 이동 전 보호창은 #oai-external-entry-guard로 유지하고, 숨김 복귀 때도 같은 보호창을 이어서 사용한다.
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
(function(){
  ...
  function markBackgrounded(reason){
    try{
      ...
      /* V8-1-14-599: 앱이 백그라운드 중 WebView/PWA가 재로딩되어 index.html early boot만 먼저 실행되는 경우에도
  ...
  function shouldKeepHantiRouteOnBackgroundReturn(){
    try{
      return !!(window.oaiShouldKeepHantiRouteOnBackgroundReturn && window.oaiShouldKeepHantiRouteOnBackgroundReturn());
    }catch(_e){ return false; }
  }

  function restoreHantiRouteAfterBackground(reason){
    try{
      if(window.oaiRestoreHantiBackgroundRouteResume){
        setTimeout(function(){
          try{ window.oaiRestoreHantiBackgroundRouteResume(reason || 'background-return'); }catch(_e){}
        }, 80);
      }
    }catch(_e){}
  }
  ...
  function keepOriginalForShortReturn(reason){
    try{
      /* V8-1-14-599:
         ...
         다만 Android/WebView가 visible/focus/pageshow와 visualViewport/resize를 거의 동시에 쏘면서
         성지·성당·피정의집 지도/시트/하단 버튼이 흔들릴 수 있으므로, 짧은 무표시 freeze로
         전환/애니메이션/키보드 보정 리스너를 잠깐 멈춘 뒤 지도 relayout 1회만 조용히 보정한다. */
      armShortSuppress();
      armResumeFreeze(1180, reason || 'short-background-return');
      try{ window.__OAI_BACKGROUND_VIEWPORT_QUIET_UNTIL__ = now() + 3200; }catch(_e){}
      clearBgStamp();
...
(function(){
  ...
  var KEY_VIEWPORT_NO_INTRO = 'oai_viewport_no_intro_until_v315';
  var KEY_VIEWPORT_WIDTH = 'oai_viewport_width_v315';
  var VIEWPORT_HOLD_MS = 120000;
  var _lastW = 0;
  var _settleTimer = 0;
  var _foldStableTimer = 0;
  var _foldTransitionBaseW = 0;
  var _pendingMainMapView = null;
  var _foldIvoryVeilTimer = 0;
  var _backgroundResumeQuietUntil = 0;
  var _backgroundResumeMapTimer = 0;
  ...
  function isAppActive(){
    try{
      if(typeof window._hasOpenAppSurface === 'function' && window._hasOpenAppSurface()) return true;
      if(typeof window._isAppScreenActive === 'function' && window._isAppScreenActive()) return true;
  ...
  function markRealExit(reason){
    ...
    try{ delBoth(KEY_VIEWPORT_NO_INTRO); delBoth(KEY_VIEWPORT_WIDTH); }catch(_e){}
  }
  function clearResumeOnly(reason){
    clearLegacyResumeKeys();
  }
  try{
    ...
    window.oaiSaveResumeScreenState = function(reason){
      clearLegacyResumeKeys();
      try{ if(window.oaiSaveHantiBackgroundRouteState) window.oaiSaveHantiBackgroundRouteState(reason || 'background-hidden'); }catch(_e){}
      return null;
    };
  }catch(_e){}
  clearLegacyResumeKeys();

  function clearFoldIvoryVeil(){
    try{ document.documentElement.classList.remove('oai-fold-ivory-veil'); }catch(_e){}
    try{ clearTimeout(_foldIvoryVeilTimer); _foldIvoryVeilTimer = 0; }catch(_e){}
  }
  try{ window.oaiShowFoldIvoryVeil = function(){ clearFoldIvoryVeil(); }; }catch(_e){}

  function isBackgroundResumeQuiet(reason){
    try{
      var n = now();
      var explicitUntil = Number(window.__OAI_BACKGROUND_VIEWPORT_QUIET_UNTIL__ || 0) || 0;
      var until = Math.max(_backgroundResumeQuietUntil || 0, explicitUntil || 0);
      if(n >= until) return false;
      return /resize|visualViewport|viewport/i.test(String(reason || ''));
    }catch(_e){ return false; }
  }

  function markViewportNoIntro(reason){
    try{
      var n = now();
      setBoth(KEY_VIEWPORT_NO_INTRO, String(n + VIEWPORT_HOLD_MS));
      var w = width();
      if(w) setBoth(KEY_VIEWPORT_WIDTH, String(w));
      try{ document.documentElement.classList.add('oai-refresh-no-intro'); }catch(_e){}
  ...
  function viewportSnapshot(){
    try{
      ...
      return {
        w: Math.round(window.innerWidth || root.clientWidth || (vv&&vv.width) || 0),
        h: Math.round(window.innerHeight || root.clientHeight || (vv&&vv.height) || 0),
        vvW: Math.round((vv&&vv.width) || 0),
        vvH: Math.round((vv&&vv.height) || 0),
        t: now()
      };
    }catch(_e){ return {w:0,h:0,vvW:0,vvH:0,t:now()}; }
  }
  ...
  function recalibrateFoldViewport(reason){
    /* V8-1-14-599: 화면 등급은 boot/refresh/저장값 없음 때만 정한다.
       resize, Fold 흔들림, background return에서는 oaiApplyAndroidScreenClass를 호출하지 않는다. */
    return;
  }
  function relayoutCurrentMaps(reason){
    reason = reason || 'viewport';
    var mapView = _pendingMainMapView || captureMainMapView();
    function runMainMapRestore(tag){
      try{
        if(window._appExiting) return;
        if(!isAppActive()) return;
        if(typeof _map !== 'undefined' && _map && typeof _map.relayout === 'function'){
          _map.relayout();
          restoreMainMapView(mapView, tag || reason);
          if(typeof window.oaiNormalizeMainMapAfterViewport === 'function'){
            window.oaiNormalizeMainMapAfterViewport(tag || reason, mapView);
          }
        }
      }catch(_e){}
    }
    try{
      if(typeof requestAnimationFrame === 'function'){
        requestAnimationFrame(function(){ runMainMapRestore(reason + '-raf'); });
      }else{
        runMainMapRestore(reason);
      }
      setTimeout(function(){ runMainMapRestore(reason + '-final'); }, 180);
    }catch(_e){}
    try{
      if(typeof window.relayoutTrailMap === 'function'){
        window.relayoutTrailMap(60, reason + '-final');
      }
    }catch(_e){}
  }

  function relayoutMainMapLightAfterBackground(reason){
    try{
      if(window._appExiting) return;
      if(!isAppActive()) return;
      if(typeof _map === 'undefined' || !_map || typeof _map.relayout !== 'function') return;
      var view = captureMainMapView();
      _map.relayout();
      restoreMainMapView(view, reason || 'background-resume-light');
    }catch(_e){}
  }
  function beginFoldViewportSettle(reason){
    reason = reason || 'viewport';
    if(!_pendingMainMapView) _pendingMainMapView = captureMainMapView();
    /* V8-1-14-599: viewport 변화는 더 이상 no-intro/화면 프로필 키를 새로 쓰지 않는다. */
    try{ window.__OAI_FOLD_VIEWPORT_SETTLING__ = true; }catch(_e){}
  }
  function finishFoldViewportSettle(reason){
    reason = reason || 'viewport';
    clearTimeout(_settleTimer);
    /* V8-1-14-599:
       저장된 화면 class는 유지하고, 필요한 경우 지도 relayout만 Fold 전환 안정 후 한 곳에서 실행한다. */
    recalibrateFoldViewport(reason + '-immediate');
    _settleTimer = setTimeout(function(){ relayoutCurrentMaps(reason + '-stable'); }, 120);
    setTimeout(function(){
      try{ window.__OAI_FOLD_VIEWPORT_SETTLING__ = false; }catch(_e){}
      _pendingMainMapView = null;
      _foldTransitionBaseW = 0;
    }, 360);
  }
  function scheduleStableViewport(reason){
    clearFoldIvoryVeil();
    beginFoldViewportSettle(reason);
    clearTimeout(_foldStableTimer);
    finishFoldViewportSettle(reason || 'fold-viewport-change');
    _foldStableTimer = setTimeout(function(){
      _foldStableTimer = 0;
      relayoutCurrentMaps((reason || 'fold-viewport-change') + '-late');
    }, 260);
  }
  function onViewportChange(reason){
    if(isBackgroundResumeQuiet(reason)){
      _foldTransitionBaseW = 0;
      clearTimeout(_settleTimer);
      if(!_foldStableTimer) _pendingMainMapView = null;
      return;
    }
    var snap = viewportSnapshot();
    var w = snap.w || 0;
    if(!w) return;
    if(!_lastW){ _lastW = w; return; }
    var diff = Math.abs(w - _lastW);
    var active = !!_foldStableTimer;
    if(diff >= 80 || active){
      if(!_foldTransitionBaseW) _foldTransitionBaseW = _lastW;
      var base = _foldTransitionBaseW || _lastW;
      if(Math.abs(w - base) >= 80 || active){
        scheduleStableViewport(reason || 'viewport-change');
      }
    }else{
      _foldTransitionBaseW = 0;
    }
  ...
  document.addEventListener('visibilitychange', function(){
    try{
      if(document.visibilityState === 'hidden'){
        clearTimeout(_settleTimer);
        clearTimeout(_foldStableTimer);
        clearTimeout(_backgroundResumeMapTimer);
        _foldStableTimer = 0;
        _backgroundResumeMapTimer = 0;
        _pendingMainMapView = null;
        _foldTransitionBaseW = 0;
        return;
      }
      if(document.visibilityState === 'visible'){
        /* 30분 안 복귀는 화면 사이즈/지도 relayout을 다시 계산하지 않는다.
           실제 Fold 전환은 resize 이벤트에서 저장된 프로필 전환으로만 처리한다. */
        _backgroundResumeQuietUntil = now() + 2600;
        _foldTransitionBaseW = 0;
        clearTimeout(_settleTimer);
        clearTimeout(_foldStableTimer);
        clearTimeout(_backgroundResumeMapTimer);
  ...
  try{
    if(window.visualViewport){
      window.visualViewport.addEventListener('resize', function(){
        /* height only 변화는 키보드/네비게이션바인 경우가 많아 Fold 전환으로 쓰지 않는다. */
        var w = width();
        if(_lastW && Math.abs(w - _lastW) < 80 && !_foldStableTimer) return;
        onViewportChange('visualViewport-resize');
      }, {passive:true});
    }
  }catch(_e){}
...
function _setFaithFrameLoading(on, kind, url){
  // V8-1-14-599: 매일미사 iframe 진입/복귀 보호창은 이 함수와 _replaceFaithFrameWith에서만 관리한다.
  try{
...
function _openFaithExternalSite(info, kind){
  ...
  // V8-1-14-599: 성가/성경 외부사이트도 비상 fallback까지 진입·복귀 보호창 기준을 유지한다.
...
function openFaithPortal(kind, opts){
  ...
  if(kind==='hymn' || kind==='bible'){
    /* V8-1-14-599: 성가·성경은 신앙 외부사이트 단일 진입 함수로 열고, 진입/복귀 보호창 기준을 유지한다. */
    try{
      if(typeof _setFaithReturnTarget === 'function') _setFaithReturnTarget('massQuick');
      if(typeof _setMassQuickReturn === 'function') _setMassQuickReturn(true);
    }catch(e){ console.warn('[가톨릭길동무]', e); }
...
function _getShrineVisitActionLayerTarget(){
  try{
    /* V8-1-14-599: 나의 성지순례 현황 버튼은 리스트 안으로 이동하지 않는다.
...
function _updateShrineNearbyLocationButtonUI(){
  try{
    ...
    if(!btn) return;
    /* V8-1-14-599: 성지 내위치 버튼은 성당/피정의집과 같은 지도 조작 버튼이다.
...
function _updateShrineVisitFloatingListButtonUI(){
  ...
  /* V8-1-14-599: 버튼 표시 위치는 지도 기준 하단 하나로 고정한다.
  ...
  /* V8-1-14-599: 버튼 위치 기준은 지도 하단 하나로 유지하되,
...
function _renderShrineVisitDetail(idx){
  ...
  const telHref=item.tel?'tel:'+String(item.tel).replace(/[^0-9+]/g,''):'';
    // V8-1-14-599: 상세 인포카드 색상은 style.css의 공통 클래스에서만 관리한다.
...
function _renderShrineVisitCardsModal(){
  ...
  body.innerHTML='<div class="shrine-visit-card-grid">'+entries.map(function(entry){
    ...
    // V8-1-14-599: 카드 디자인은 style.css 단일 기준으로 관리하고, 신규 탭 방문 성지는 순례도장도 표시한다.
...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V8-1-14-599: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.OAI_APP_BUILD_VERSION || window.APP_VERSION || 'V8-1-14-599');
    var current = '';
    /* V8-1-14-599:
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-599'));
  ...
  function bindGuide(){
    /* V8-1-14-599: legacy direct guide-button binding removed; cover menu handler owns this entry. */
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-599';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-599';
}
...
/* V8-1-14-599: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V8-1-14-599: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V8-1-14-599: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V8-1-14-599: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
    if(typeof oaiSmoothNavigate === 'function'){
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V8-1-14-599: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V8-1-14-599: route reset helper is kept without inactive focus/map-entry hooks. */
...
/* V8-1-14-599: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V8-1-14-599';
...
function _ensureMapLoading(mapEl, text){
  ...
  try{
    if(mapEl.querySelector && mapEl.querySelector('.map-loading')) return false;
  }catch(_e){}
...
const _PRAYER_ASSET_VERSION='V8-1-14-599';
...
const _RETREAT_ASSET_VERSION='V8-1-14-599';
...
// V8-1-14-599: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
function _setMarkerMapIfChanged(marker, map){
  ...
  try{
    ...
    /* V8-1-14-599: 일부 Kakao Marker는 getMap이 없어 __oaiMapTarget만 남는데,
...
const _SHRINE_ASSET_VERSION='V8-1-14-599';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function oaiEnterView(el){
  ...
  try{
    ...
    // V8-1-14-599: 카테고리 첫 화면은 nearby-distance-loading이 담당한다.
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-599: 성지/성당/피정 진입은 내주변 거리계산 화면이 기준이다.
...
function oaiPreopenNearbySheetForCategory(){
  try{
    ...
    if(body){
      // V8-1-14-599: 데이터 지연 후 startApp이 다시 호출되어도 같은 내주변 로딩 화면을 다시 그리지 않는다.
...
function goToCover(){
  ...
  // V8-1-14-599: 커버 복귀 때 화면 크기 class를 다시 계산하지 않는다. 저장된 화면 기준을 유지한다.
  try{ if(typeof window.oaiSettleCoverSize === 'function') window.oaiSettleCoverSize('go-to-cover-settle-only'); }catch(e){ console.warn('[가톨릭길동무]', e); }
}
...
function openTab(name, opts){
  ...
  // V8-1-14-599: 성지/성당/피정의집 시트 전환은 무거운 가로 swiper 애니메이션 없이 즉시 전환하고, 내부 좌우 스와이프 전환은 비활성화한다.
...
function _scheduleNearbyMyLocationPlainRestore(reason){
  try{
    ...
    // V8-1-14-599: 성지·성당·피정의집 내주변 목록을 닫은 뒤에는
...
function _showInfoCard(item, idx){
  // V8-1-14-599: 길찾기 탭/경로검색 시트가 열린 동안에는
...
function _focusParishPointAround(lat, lng, opts){
  ...
  try{
    ...
    // V8-1-14-599: 성당의 현재 위치 기준 화면은 성지/피정과 동일하게
...
function _showParishDioMkrs(code){
  ...
  /* V8-1-14-599: 성당 지도는 idle 때마다 교구 전체 마커 setMap을 반복해
...
function _updateParishViewport(code){
  ...
  mkrs.forEach(mk=>{
    try{ _setMarkerMapIfChanged(mk,_map); }catch(e){ console.warn('[가톨릭길동무]',e); }
  });
}
...
function _buildRetreatMarkers(){
  ...
  _retreatMarkers.forEach(o=>_setMarkerMapIfChanged(o.marker,_map));
  _raiseMyLocationMarker();
}
  ...
  try{
    _GEO.getCurrentPosition(ok,function(e){
      ...
      if(elapsed < 300) setTimeout(function(){ fallback(e); }, 150);
        ...
        if(fail) fail(e);
        /* V8-1-14-599: 빠른 위치가 실패했을 때도 UI를 오래 잡아두지 않고,
...
function _loadNearbyWithDist(lat,lng,items,getIdx,getColor,getLabel){
  ...
  if(body && _isNearbyLoadCurrent(requestMode,requestToken,body)){
    // V8-1-14-599: 자동차 거리 계산이 끝나기 전에는 예상 목록을 먼저 보여주지 않는다.
...
function _enterRouteMode(){
  ...
  // V8-1-14-599: 경로검색 시트가 열리면 기존 장소 정보카드는 숨겨
...
function _syncRouteWaypointBoxes(){
  ...
  // V8-1-14-599: 경로 결과판이 보이는 동안에는 +경유지를 완전히 숨긴다.
...
function _updateSearchBtn(){
  ...
  /* V8-1-14-599: 경로검색 버튼은 출발/도착창 아래에서 항상 보이고,
...
function clearRoute(role, opts){
  ...
  if(_isRouteWaypointRole(role) && (_getRoutePointByRole(role)||_getRouteWaypointEnabledByRole(role))){
    ...
    if(keepWaypointBox){
      // V8-1-14-599: 경유지 마커 취소 확인창에서 취소할 때는
...
(function(){
  ...
  function _doMainSwipe(dx){
    ...
    /* V8-1-14-599: 성지·성당·피정의집 좌우 스와이프는 하단 시트 내부에서 시작한 한 손가락 터치만 처리한다.
