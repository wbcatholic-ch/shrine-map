...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.OAI_APP_BUILD_VERSION || window.APP_VERSION || 'V8-1-14-654');
    var current = '';
    /* V8-1-14-621:
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-654'));
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-654';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-654';
}
...
const _PARISH_ASSET_VERSION='V8-1-14-654';
...
const _PRAYER_ASSET_VERSION='V8-1-14-654';
...
const _RETREAT_ASSET_VERSION='V8-1-14-654';
...
const _SHRINE_ASSET_VERSION='V8-1-14-654';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
 ...
 const KM=kakao.maps;window._LL=KM.LatLng;window._MM=KM.Marker;window._MI=KM.MarkerImage;window._SZ=KM.Size;window._PT=KM.Point;window._LB=KM.LatLngBounds;window._PL=KM.Polyline;window._EL=KM.event.addListener;
  ...
  if(!window.__OAI_MAIN_MAP_CLICK_BOUND__){
    ...
    kakao.maps.event.addListener(_map,'click',()=>{
      // V8-1-14-654: 지도 빈 곳 클릭은 패널 표시만 접고 선택 마커와 경로선은 유지한다.
...
function _relayoutMainMapForPanel(){
  try{
    ...
    requestAnimationFrame(function(){
      try{_map.relayout();if(center&&typeof _map.setCenter==='function')_map.setCenter(center);}catch(e){console.warn('[가톨릭길동무]',e);}
    });
  }catch(e){console.warn('[가톨릭길동무]',e);}
}
function _observeMapPanelSize(el){
  try{
    ...
    if(el&&window.ResizeObserver){
      _oaiMapPanelResizeObserver=new ResizeObserver(function(){_positionMapPanelToggle();});
...
function _syncMapPanelUI(reason){
  try{
    ...
    if(!present){
      ...
      if(btn){
        btn.hidden=false;btn.dataset.panelKind=candidate.kind;
...
function _setMapPanelCollapsed(on,reason){
  ...
  _oaiMapPanelCollapsed=!!on;_syncMapPanelUI(reason||'manual');return true;
}
...
function _bindMapPanelController(){
  ...
  try{
    ...
    ['sheet-nearby','sheet-list','sheet-region','sheet-route','info-card'].forEach(function(id){const el=$(id);if(el)mo.observe(el,{attributes:true,attributeFilter:['class','style','hidden']});});
...
function selectItem(idx, opts={}){
  ...
  else _focusMarkerAboveInfoCard(item);
}
...
function _showParishNearbyMarkersOnMap(items, lat, lng, phase){
  ...
  try{
    ...
    // V8-1-14-654: 첫 진입에서는 가장 가까운 성당을 자동 선택하지 않는다.
...
function renderList(){
  ...
  items.forEach((s,i)=>{
    ...
    /* V8-1-14-654: 성당은 교구 탭이 선택되어 있어도 검색어가 있으면 전국 전체에서 찾는다.
...
function setDioFilter(v,btn){
  ...
  btn?.classList.add('active');
  _scrollDioFilterButtonIntoView(btn);
  /* V8-1-14-654: 성당 전국 검색 중 교구 탭을 눌러도 검색어와 결과를 유지한다.
...
function _syncRouteWaypointBoxes(){
  ...
  const slotStates=OAI_ROUTE_WAYPOINT_CONFIGS.map(function(cfg,index){
    ...
    return {
      ...
      index:index+1,
      has:has,
      visible:foldWide ? true : (resultShowing ? has : !!(_getRouteWaypointEnabledByRole(cfg.role) || has))
    };
  });
  ...
  slotStates.forEach(function(slot){
    ...
    const showTools=!resultShowing && slot.visible && (!foldWide || slot.has);
    ...
    if(clear){
      const showClear=showBox && (!foldWide || slot.has);
      clear.style.display=showClear?'inline-flex':'none';
    }
  });
  const add=$('rs-add-waypoint-btn');
  const nextWaypointRole=(!resultShowing && !foldWide ? _nextAvailableWaypointRole() : null);
  if(add){
...
function resetRoute(opts){
  ...
  _hide($('rs-result'));
  _setRouteResultTipVisible(false);
  // V8-1-14-654: 다시선택에서는 경로선과 결과판을 먼저 모두 닫은 뒤 경유지 UI를 한 번만 동기화한다.
