...
function markExternalReturnStabilize(kind){
  try{
    ...
    // V8-1-14-314: 외부 이동 전 보호창은 #oai-external-entry-guard로 유지하고, 숨김 복귀 때도 같은 보호창을 이어서 사용한다.
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
/* V8-1-14-314:
   Catholic file 원리 기준 정리.
   Fold 화면 전환은 앱 재시작/복원 이벤트가 아니므로 보이는 화면 저장·복원 키를 사용하지 않는다.
   intro는 첫 진입과 장기 background 복귀 담당으로 되돌리고, Fold 때는 지도 relayout만 수행한다. */
(function(){
  'use strict';
  if(window.__OAI_CATHOLIC_STYLE_NO_VISIBLE_RESTORE_V313__) return;
  window.__OAI_CATHOLIC_STYLE_NO_VISIBLE_RESTORE_V313__ = true;

  var EXIT_KEY = 'oai_real_app_exit_until_v300';
  var RESTORE_KEYS = [
    'oai_visible_screen_state_v300','oai_fold_transition_no_intro_until_v300','oai_fold_transition_width_v300',
    'oai_visible_screen_state_v299','oai_fold_transition_no_intro_until_v299','oai_fold_transition_width_v299',
    'oai_visible_screen_state_v296','oai_fold_transition_no_intro_until_v296','oai_fold_transition_width_v296',
    'oai_visible_screen_state_v293','oai_fold_transition_no_intro_until_v293','oai_fold_transition_width_v293',
  ...
  function now(){ return Date.now ? Date.now() : new Date().getTime(); }
  function delBoth(k){ try{ sessionStorage.removeItem(k); }catch(_e){} try{ localStorage.removeItem(k); }catch(_e){} }
  function setBoth(k,v){ try{ sessionStorage.setItem(k,v); }catch(_e){} try{ localStorage.setItem(k,v); }catch(_e){} }
  function clearRestoreState(reason){
    try{ RESTORE_KEYS.forEach(delBoth); }catch(_e){}
    try{ document.documentElement.classList.remove('oai-pending-visible-restore','oai-refresh-no-intro'); document.documentElement.removeAttribute('data-oai-pending-visible-restore'); }catch(_e){}
  }
  function markRealExit(reason){
    try{ setBoth(EXIT_KEY, String(now()+5*60*1000)); }catch(_e){}
    clearRestoreState(reason || 'real-exit');
  }
  function relayoutMainMap(reason){
    try{
  ...
  function markViewportNoIntro(reason){
    try{
      var until = now() + 12000;
      sessionStorage.setItem('oai_viewport_transition_no_intro_until_v314', String(until));
      localStorage.setItem('oai_viewport_transition_no_intro_until_v314', String(until));
      sessionStorage.setItem('oai_viewport_transition_no_intro_reason_v314', reason || 'viewport');
    }catch(_e){}
  }
  function scheduleFoldSettle(reason){
    try{
      markViewportNoIntro(reason || 'viewport-settle');
      [40,160,360,720].forEach(function(delay){
        setTimeout(function(){ relayoutMainMap(reason || 'viewport-settle'); }, delay);
      });
    }catch(_e){}
  }
  try{
    ...
    window.oaiSaveResumeScreenState = function(){ clearRestoreState('save-disabled'); return null; };
  }catch(_e){}
  clearRestoreState('boot-cleanup');
  window.addEventListener('resize', function(){ scheduleFoldSettle('resize'); }, {passive:true});
  window.addEventListener('orientationchange', function(){ scheduleFoldSettle('orientationchange'); }, {passive:true});
  window.addEventListener('pageshow', function(){ scheduleFoldSettle('pageshow'); }, {passive:true});
  try{ if(window.visualViewport) window.visualViewport.addEventListener('resize', function(){ scheduleFoldSettle('visualViewport-resize'); }, {passive:true}); }catch(_e){}
})();
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-314: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V8-1-14-314: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function _ensureCoverBackTrap(reason){
  try{
    if(typeof window._oaiArmCoverBackTrap === 'function') window._oaiArmCoverBackTrap(reason || 'app-cover-ensure', {force:true});
  }catch(e){ console.warn('[가톨릭길동무]', e); }
}
function _resetCoverBackTrap(reason){
  try{
    ...
    if(typeof _clearHardCoverExitFlags === 'function') _clearHardCoverExitFlags(reason || 'cover-reset');
    if(typeof window._oaiArmCoverBackTrap === 'function') window._oaiArmCoverBackTrap(reason || 'app-cover-reset', {force:true});
  }catch(e){ console.warn('[가톨릭길동무]', e); }
}
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.APP_VERSION || 'V8-1-14-314');
    var current = '';
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-314'));
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-314';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-314';
}
...
/* V8-1-14-314: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V8-1-14-314: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V8-1-14-314: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V8-1-14-314: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V8-1-14-314: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V8-1-14-314: 동작 없는 빈 포커스 리스너와 빈 지도 진입 훅 제거 */
...
/* V8-1-14-314: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V8-1-14-314';
...
const _PRAYER_ASSET_VERSION='V8-1-14-314';
...
const _RETREAT_ASSET_VERSION='V8-1-14-314';
...
// V8-1-14-314: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
const _SHRINE_ASSET_VERSION='V8-1-14-314';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-314: 성지/성당/피정 진입은 cover를 내리기 전 내위치 안내 화면을 먼저 열어 첫 화면으로 고정한다.
...
function oaiPreopenNearbySheetForCategory(){
  try{
    ...
    if(body){
      // V8-1-14-314: 데이터 지연 후 startApp이 다시 호출되어도 같은 내주변 로딩 화면을 다시 그리지 않는다.
      try{
...
function goToCover(){
  ...
  // V8-1-14-314: 커버 복귀 때 화면 높이/폰 화면 크기를 다시 계산하지 않는다.
}
...
function _loadNearby(){
  ...
  try{ _updateShrineVisitCardsButtonUI(); }catch(_e){}
  try{ _updateShrineNearbyLocationButtonUI(); }catch(_e){}
  // V8-1-14-314: 카테고리 진입 직후 이미 열린 로딩 화면은 유지하고, 빈 화면/이전 목록일 때만 로딩 화면을 그린다.
  try{
    if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
      body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
    }
...
function _loadNearbyWithDist(lat,lng,items,getIdx,getColor,getLabel){
  ...
  if(body && _isNearbyLoadCurrent(requestMode,requestToken,body)){
    // V8-1-14-314: 자동차 거리 계산 단계에서 같은 로딩 화면을 다시 렌더링하지 않는다.
    try{
      if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
        body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
      }
