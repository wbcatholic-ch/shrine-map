...
function markExternalReturnStabilize(kind){
  // V20-12: 외부 사이트 이동은 브라우저 기본 동작에 맡긴다.
...
function oaiSmoothNavigate(url, kind){
  // V20-12: 호환용 함수. 보호막/지연/전역 가로채기 없이 즉시 이동한다.
  if(!url) return;
...
function applyExternalReturnStabilize(){
  // V20-12: 복귀 시 화면을 재계산하지 않고, 예전 이동중 잔여 상태만 제거한다.
  try{ oaiClearExternalNavigationState(); }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
function refreshAppFilesOnly(){
  ...
  try{
    ...
    try{ var mq=document.getElementById('mass-quick-modal'); if(mq){ mq.classList.remove('show'); mq.setAttribute('aria-hidden','true'); } }catch(_e){}
    try{ if(history && 'scrollRestoration' in history) history.scrollRestoration='manual'; window.scrollTo(0,0); }catch(_e){}
  }catch(e){
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || 'V20-12';
    var current = '';
...
// V20-12: 커버 전용 주요 기능 안내. 별도 파일 없이 작은 자동 안내 + 자세한 카드형 팝업을 제공한다.
(function(){
  'use strict';
  var HIDE_DAYS = 7;
  ...
  function maybeShowIntro(){
    ...
    if(forceRefresh){
      // V20-12: 안정형 새로고침 뒤에는 주요 기능 안내 팝업을 새로고침 전용 안정 팝업처럼 다시 표시한다.
      // 미사·기도·성가 팝업 복귀 상태와 분리해 커버가 아래로 처지는 흐름을 막는다.
      try{ if(typeof closeMassQuickMenu === 'function') closeMassQuickMenu(); }catch(e){ console.warn('[가톨릭길동무]', e); }
      clearSoftRefreshRequest();
    }
    if(shouldShowIntro(forceRefresh)) showModal('guide-intro-modal');
  }
  function bindGuide(){
...
// V20-12: 확인용 임시 설정. 이번 버전에서만 Android에서도 iPhone Safari 설치 안내 배너를 표시한다.
// Android 카카오톡 Chrome intent 흐름은 건드리지 않는다.
(function(){
  'use strict';
  ...
  function shouldShow(){
    // V20-12 확인용: 원래 조건(iPhone + Kakao + 홈화면 앱 아님)에 Android 브라우저를 임시로 포함한다.
    var isAndroid = ua().indexOf('android') > -1;
    return ((isIOS() && isKakao()) || isAndroid) && !isStandalone();
  }
  function showModal(){
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V20-12';
  }else if(!restore){
...
function restoreCoreReturnState(){
  ...
  // V20-12: 외부사이트 복귀 시 지도 중심을 두 단계로 움직이지 않는다.
...
function _getInfoCardCenterTargetY(mapH){
  // V20-12: 성지·성당·피정 지도 중심은 항상 인포카드가 올라왔을 때의 기준으로 통일한다.
...
function closeInfoCard(opts){
  ...
  // V20-12: 시트 전환으로 닫을 때는 지도 중심을 다시 움직이지 않는다.
  // 사용자가 X/지도 터치로 인포카드만 닫을 때는 기존 V20-12 기준 중심을 유지한다.
...
function _focusParishPointAround(lat, lng, opts){
  ...
  try{
    ...
    // V20-12: 현재 위치/내 주변/선택 성당 모두 인포카드 기준 중심으로 통일한다.
