...
function markExternalReturnStabilize(kind){
  try{
    ...
    // V8-1-14-319: 외부 이동 전 보호창은 #oai-external-entry-guard로 유지하고, 숨김 복귀 때도 같은 보호창을 이어서 사용한다.
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
(function(){
  ...
  function showSimpleBackgroundIntro(toCover){
    ...
    try{
      if(veil){
        ...
        requestAnimationFrame(function(){ try{ veil.classList.add('oai-show'); }catch(_e){} });
      }
    }catch(_e){}
    setTimeout(function(){
  ...
  function scheduleBackgroundReturn(reason){
    try{
      ...
      _bgReturnTimer = setTimeout(function(){ checkBackgroundReturn(reason || 'return'); }, 220);
    }catch(e){ console.warn('[가톨릭길동무]', e); }
  }
...
/* V8-1-14-319:
   ...
   - 지도는 현재 화면에서 relayout만 수행하고 center/level은 건드리지 않음
   - 커버 종료 Back 구조는 V297/back-controller.js 기준으로 보존 */
(function(){
  'use strict';
  if(window.__OAI_VISIBLE_SCREEN_RESTORE_V319_CLEANUP__) return;
  window.__OAI_VISIBLE_SCREEN_RESTORE_V319_CLEANUP__ = true;
  ...
  var KEY_VIEWPORT_NO_INTRO = 'oai_viewport_no_intro_until_v319';
  var KEY_VIEWPORT_WIDTH = 'oai_viewport_width_v319';
  ...
  function relayoutCurrentMaps(reason){
    ...
    try{
      if(typeof window.relayoutTrailMap === 'function'){
        window.relayoutTrailMap(40);
        window.relayoutTrailMap(220);
      }
    }catch(_e){}
  }
  ...
  function onViewportChange(reason){
    var w=width();
    if(w && _lastW && Math.abs(w-_lastW)>=80) settleViewport(reason || 'viewport-change');
    _lastW = w || _lastW;
  }
  function init(){
    _lastW = width();
...
/* V8-1-14-319:
   V315의 커버/Back 안정 흐름은 그대로 두고, Fold/viewport reload 직후에만
   마지막 앱 화면을 1회 재진입한다.
   - cover trap/back-controller는 수정하지 않음
   - pagehide마다 전체 복원하지 않고, 현재 표면(surface)만 최소 저장
   - 폭 변화가 확인될 때만 복귀하여 일반 백그라운드 복귀와 분리 */
(function(){
  'use strict';
  if(window.__OAI_FOLD_SURFACE_MIN_RESTORE_V319__) return;
  window.__OAI_FOLD_SURFACE_MIN_RESTORE_V319__ = true;

  var KEY = 'oai_fold_surface_last_v319';
  var CANDIDATE = 'oai_fold_surface_candidate_until_v319';
  var RESTORED = 'oai_fold_surface_restored_at_v319';
  var TTL = 10 * 60 * 1000;
  var CANDIDATE_MS = 90 * 1000;
  var RESTORE_GUARD_MS = 5200;
  ...
  function readJson(){
    try{
      var raw=getSS(KEY) || getLS(KEY);
      if(!raw) return null;
      var st=JSON.parse(raw);
      if(!st || !st.type || st.type==='cover') return null;
      if(!st.ts || now()-Number(st.ts)>TTL) return null;
      return st;
    }catch(_e){ return null; }
  }
  function write(st){
    try{
      if(!st || !st.type || st.type==='cover') return;
      st.ts = now();
      st.w = width();
      st.h = height();
      setBoth(KEY, JSON.stringify(st));
    }catch(_e){}
  }
  function markCandidate(reason){
    try{ setBoth(CANDIDATE, String(now()+CANDIDATE_MS)); }catch(_e){}
  }
  function candidateAlive(){
    try{ return now() < Math.max(Number(getSS(CANDIDATE)||0), Number(getLS(CANDIDATE)||0)); }catch(_e){ return false; }
  }
  function viewportChanged(st){
    try{
      var w = width();
      if(!st || !st.w || !w) return false;
      return Math.abs(w - Number(st.w)) >= 40;
    }catch(_e){ return false; }
  }
  function coverVisible(){
    try{ if(typeof window._isCoverScreenVisible === 'function') return window._isCoverScreenVisible(); }catch(_e){}
    try{
      var c=document.getElementById('cover');
      if(!c) return !document.documentElement.classList.contains('app-active');
      var cs=window.getComputedStyle?getComputedStyle(c):null;
      return !(c.classList.contains('hidden') || (cs&&(cs.display==='none'||cs.visibility==='hidden')));
    }catch(_e){ return false; }
  }
  function appActive(){
    try{ if(typeof window._hasOpenAppSurface === 'function' && window._hasOpenAppSurface()) return true; }catch(_e){}
    try{ if(typeof window._isAppScreenActive === 'function' && window._isAppScreenActive()) return true; }catch(_e){}
    try{ return document.documentElement.classList.contains('app-active') && !coverVisible(); }catch(_e){ return false; }
  }
  function getMapCamera(st){
    try{
      if(typeof _map !== 'undefined' && _map && _map.getCenter && _map.getLevel){
        var c=_map.getCenter();
        if(c && c.getLat && c.getLng){ st.lat=Number(c.getLat()); st.lng=Number(c.getLng()); st.level=Number(_map.getLevel()); }
      }
    }catch(_e){}
    return st;
  }
  function currentSurface(reason){
    try{
      if(coverVisible() && !appActive()) return null;
      var doc=document;
      var st=null;
      if(doc.getElementById('diocese-view') && doc.getElementById('diocese-view').classList.contains('open')) st={type:'diocese'};
      else if(doc.getElementById('prayer-view') && doc.getElementById('prayer-view').classList.contains('open')) st={type:'prayer'};
      else if(doc.getElementById('qna-view') && doc.getElementById('qna-view').classList.contains('open')) st={type:'qna'};
      else if(doc.getElementById('web-view') && doc.getElementById('web-view').classList.contains('open')) st={type:'web'};
      else if(doc.getElementById('trail-view') && doc.getElementById('trail-view').classList.contains('open')) st={type:'trail'};
      else if(document.documentElement.classList.contains('app-active')){
        var mode='';
        try{ mode = (typeof _mode !== 'undefined' && _mode) ? String(_mode) : ''; }catch(_e){}
        if(mode==='shrine' || mode==='parish' || mode==='retreat'){
          st={type:'map', mode:mode};
          try{ if(typeof _activeTab !== 'undefined' && _activeTab) st.tab=String(_activeTab); }catch(_e){}
          getMapCamera(st);
        }
      }
      if(st){ st.reason=reason||'capture'; }
      return st;
    }catch(_e){ return null; }
  }
  function remember(surface, reason){
    try{
      var st=null;
      if(surface && typeof surface === 'object') st={};
      else if(typeof surface === 'string') st={type:surface};
      if(st){
        for(var k in surface){ if(Object.prototype.hasOwnProperty.call(surface,k)) st[k]=surface[k]; }
      }else st=currentSurface(reason);
      if(!st) return;
      if(st.type==='map') getMapCamera(st);
      st.reason=reason||st.reason||'remember';
      write(st);
    }catch(_e){}
  }
  function clear(reason){
    delBoth(KEY); delBoth(CANDIDATE);
  }
  function isRestoring(){
    try{ return !!(_restoring || (window.__OAI_FOLD_SURFACE_RESTORING_UNTIL_V319__ && now() < Number(window.__OAI_FOLD_SURFACE_RESTORING_UNTIL_V319__))); }catch(_e){ return false; }
  }
  function armHistoryAfterProgrammaticOpen(reason){
    try{
      var r = reason || 'fold-surface-restore';
      [0,120,360,900].forEach(function(ms){
        setTimeout(function(){
          try{ if(typeof window._oaiArmCoverBackTrap === 'function') window._oaiArmCoverBackTrap(r, {force:true}); }catch(_e){}
        }, ms);
      });
    }catch(_e){}
  }
  function applyMapState(st){
    try{
      if(!st || st.type!=='map') return;
      if(st.lat && st.lng && typeof _map !== 'undefined' && _map && window.kakao && kakao.maps){
        if(st.level && _map.setLevel) _map.setLevel(Number(st.level));
        if(_map.setCenter) _map.setCenter(new kakao.maps.LatLng(Number(st.lat), Number(st.lng)));
      }
    }catch(_e){}
  }
  function restoreTab(st){
    try{
      if(!st || st.type!=='map' || !st.tab || typeof openTab !== 'function') return;
      var tab=String(st.tab);
      if(tab==='route') tab='list';
      if(tab==='nearby' || tab==='list' || tab==='region') openTab(tab, {restore:true});
    }catch(_e){}
  }
  function openSurface(st, reason){
    if(!st || _restoring) return false;
    _restoring = true;
    window.__OAI_FOLD_SURFACE_RESTORING_UNTIL_V319__ = now() + RESTORE_GUARD_MS;
    window.__OAI_FOLD_KEEP_SKIP_AUTOLOCATE_UNTIL = now() + RESTORE_GUARD_MS;
    setBoth(RESTORED, String(now()));
    try{ document.documentElement.classList.add('oai-refresh-no-intro'); }catch(_e){}
    var opened=false;
    try{
      if(st.type==='map' && typeof startApp === 'function'){
        startApp(st.mode || 'shrine'); opened=true;
        setTimeout(function(){ applyMapState(st); restoreTab(st); }, 360);
        setTimeout(function(){ applyMapState(st); restoreTab(st); }, 900);
        setTimeout(function(){ applyMapState(st); restoreTab(st); }, 1600);
        setTimeout(function(){ applyMapState(st); restoreTab(st); }, 2600);
      }else if(st.type==='trail' && typeof window.openTrailView === 'function'){
        window.openTrailView({restore:true}); opened=true;
      }else if(st.type==='web' && typeof window.openWebView === 'function'){
        window.openWebView({restore:true}); opened=true;
      }else if(st.type==='diocese' && typeof openDioceseView === 'function'){
        openDioceseView({restore:true}); opened=true;
      }else if(st.type==='prayer' && typeof openPrayerBook === 'function'){
        openPrayerBook({restore:true, instant:true}); opened=true;
      }else if(st.type==='qna' && typeof window.openQnaView === 'function'){
        window.openQnaView({restore:true}); opened=true;
      }
      if(opened){
        armHistoryAfterProgrammaticOpen('fold-surface-restore');
        var updated=st; updated.w=width(); updated.h=height(); updated.ts=now(); updated.restoredAt=now(); write(updated);
      }
    }catch(e){ console.warn('[가톨릭길동무] fold surface open failed', e); }
    setTimeout(function(){
      _restoring=false;
      window.__OAI_FOLD_SURFACE_RESTORING_UNTIL_V319__=0;
      try{ document.documentElement.classList.remove('oai-refresh-no-intro'); }catch(_e){}
    }, RESTORE_GUARD_MS);
    return opened;
  }
  function tryRestore(reason){
    try{
      var st=readJson();
      if(!st) return false;
      if(!viewportChanged(st)) return false;
      return openSurface(st, reason||'try');
    }catch(e){ console.warn('[가톨릭길동무]', e); return false; }
  }
  function snapshotForFold(reason){
    try{
      var st=currentSurface(reason||'fold-snapshot');
      if(!st) return;
      write(st);
      markCandidate(reason||'fold-snapshot');
    }catch(_e){}
  }
  function schedule(reason){
    [0,90,240,520,920,1500,2300,3400].forEach(function(ms){ setTimeout(function(){ tryRestore((reason||'schedule')+'-'+ms); }, ms); });
  }

  window.oaiRememberCurrentSurface = remember;
  window.oaiClearCurrentSurface = clear;
  window.oaiTryFoldSurfaceRestore = tryRestore;
  window.oaiIsFoldSurfaceRestoring = isRestoring;

  window.addEventListener('resize', function(){ if(appActive()) snapshotForFold('resize'); }, {passive:true});
  try{ if(window.visualViewport) window.visualViewport.addEventListener('resize', function(){ if(appActive()) snapshotForFold('visualViewport-resize'); }, {passive:true}); }catch(_e){}
  window.addEventListener('orientationchange', function(){ setTimeout(function(){ if(appActive()) snapshotForFold('orientationchange'); }, 60); }, {passive:true});
  window.addEventListener('pagehide', function(){ if(appActive()) snapshotForFold('pagehide'); }, {passive:true});
  document.addEventListener('visibilitychange', function(){ if(document.visibilityState==='hidden' && appActive()) snapshotForFold('hidden'); }, {passive:true});
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ schedule('dom'); }, {once:true});
  else schedule('ready');
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-319: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V8-1-14-319: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.APP_VERSION || 'V8-1-14-319');
    var current = '';
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-319'));
...
function openPrayerBook(opts){
  try{ if(typeof window.oaiRememberCurrentSurface === 'function' && !(opts && opts.restore)) window.oaiRememberCurrentSurface({type:'prayer'}, 'openPrayerBook'); }catch(_e){}
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  try{ if(typeof window.oaiRememberCurrentSurface === 'function' && !restore) window.oaiRememberCurrentSurface({type:'diocese'}, 'openDioceseView'); }catch(_e){}
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-319';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-319';
}
...
/* V8-1-14-319: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V8-1-14-319: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V8-1-14-319: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V8-1-14-319: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V8-1-14-319: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V8-1-14-319: 동작 없는 빈 포커스 리스너와 빈 지도 진입 훅 제거 */
...
/* V8-1-14-319: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V8-1-14-319';
...
const _PRAYER_ASSET_VERSION='V8-1-14-319';
...
const _RETREAT_ASSET_VERSION='V8-1-14-319';
...
// V8-1-14-319: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
const _SHRINE_ASSET_VERSION='V8-1-14-319';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-319: 성지/성당/피정 진입은 cover를 내리기 전 내위치 안내 화면을 먼저 열어 첫 화면으로 고정한다.
...
function oaiPreopenNearbySheetForCategory(){
  try{
    ...
    if(body){
      // V8-1-14-319: 데이터 지연 후 startApp이 다시 호출되어도 같은 내주변 로딩 화면을 다시 그리지 않는다.
      try{
...
function startApp(mode){
  var __oaiFoldSurfaceRestore = false;
  try{ __oaiFoldSurfaceRestore = !!(typeof window.oaiIsFoldSurfaceRestoring === 'function' && window.oaiIsFoldSurfaceRestoring()); }catch(_e){}
  try{ if(typeof window.oaiRememberCurrentSurface === 'function' && !__oaiFoldSurfaceRestore) window.oaiRememberCurrentSurface({type:'map', mode:mode}, 'startApp'); }catch(_e){}
  ...
  _filterDio='all';
  _listSrch='';
  window._noAutoNearby = !!__oaiFoldSurfaceRestore;  // 직접 진입은 nearby 허용, Fold 복귀는 저장 tab 복원 전까지 차단
 ...
 _renderDioFilterBars();  _screen='map';
  if(mode==='shrine' || mode==='parish' || mode==='retreat'){
    ...
    } else if(!__oaiFoldSurfaceRestore) {
...
function goToCover(){
  try{ if(typeof window.oaiClearCurrentSurface === 'function') window.oaiClearCurrentSurface('goToCover'); }catch(_e){}
  try{ _cancelNearbyLoad(); }catch(e){ console.warn('[가톨릭길동무]', e); }
  try{
  ...
  // V8-1-14-319: 커버 복귀 때 화면 높이/폰 화면 크기를 다시 계산하지 않는다.
}
...
function _onMapReady(){
 const KM=kakao.maps;window._LL=KM.LatLng;window._MM=KM.Marker;window._MI=KM.MarkerImage;window._SZ=KM.Size;window._PT=KM.Point;window._LB=KM.LatLngBounds;window._PL=KM.Polyline;window._EL=KM.event.addListener;
  ...
  renderList();
  var __oaiFoldSurfaceRestoreNow = false;
  try{ __oaiFoldSurfaceRestoreNow = !!(typeof window.oaiIsFoldSurfaceRestoring === 'function' && window.oaiIsFoldSurfaceRestoring()); }catch(_e){}
  if(!__oaiFoldSurfaceRestoreNow) _autoLocate();
  if(_mode==='parish') { _buildParishDioSystem(); _syncParishDioLabels(); }
  else if(_mode==='retreat') _buildRetreatMarkers();
  if(!window._noAutoNearby && !__oaiFoldSurfaceRestoreNow){
    openTab('nearby');
  }
  window._noAutoNearby = false;
  try{ _updateShrineVisitMapFilterUI(); _updateShrineVisitCardsButtonUI(); }catch(e){ console.warn('[가톨릭길동무]', e); }
...
function closeSheetPanelOnly(name){
  ...
  try{ if(typeof window.oaiRememberCurrentSurface === 'function') window.oaiRememberCurrentSurface({type:'map', mode:_mode}, 'closeTab'); }catch(_e){}
  _updateTabBtns(null);
}
...
function openTab(name, opts){
  ...
  _activeTab=name;
  try{ if(typeof window.oaiRememberCurrentSurface === 'function') window.oaiRememberCurrentSurface({type:'map', mode:_mode, tab:name}, 'openTab'); }catch(_e){}
  try{ _updateShrineNearbyLocationButtonUI(); }catch(_e){}
...
function _loadNearby(){
  ...
  try{ _updateShrineVisitCardsButtonUI(); }catch(_e){}
  try{ _updateShrineNearbyLocationButtonUI(); }catch(_e){}
  // V8-1-14-319: 카테고리 진입 직후 이미 열린 로딩 화면은 유지하고, 빈 화면/이전 목록일 때만 로딩 화면을 그린다.
  try{
    if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
      body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
    }
  ...
  const fallbackCached=_readRecentStoredLocation(2*60*1000);
  _refreshFreshLocationThen(function(lat,lng){ runOnce(lat,lng,false); },function(err){
    if(settled) return;
    if(fallbackCached){ runOnce(fallbackCached.lat,fallbackCached.lng,true); return; }
...
function _loadNearbyWithDist(lat,lng,items,getIdx,getColor,getLabel){
  ...
  if(body && _isNearbyLoadCurrent(requestMode,requestToken,body)){
    // V8-1-14-319: 자동차 거리 계산 단계에서 같은 로딩 화면을 다시 렌더링하지 않는다.
    try{
      if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
        body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
      }
