...
function markExternalReturnStabilize(kind){
  try{
    ...
    // V8-1-14-342: 외부 이동 전 보호창은 #oai-external-entry-guard로 유지하고, 숨김 복귀 때도 같은 보호창을 이어서 사용한다.
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
(function(){
  ...
  /* V8-1-14-342:
     백그라운드 복귀 기준을 더 엄격하게 분리한다.
     짧은 복귀는 즉시 원화면 유지, 10~30분은 안정막 후 원화면 유지,
     30분 이상만 커버로 정리한다. */
  var MEDIUM_BG_RETURN_MS = 10 * 60 * 1000;
  var COVER_BG_RETURN_MS = 30 * 60 * 1000;
  var BG_KEY = 'oai_home_backgrounded_at';
  var BG_TOKEN_KEY = 'oai_home_background_token_v340';
  var BG_SHORT_RETURN_UNTIL_KEY = 'oai_short_background_return_until_v340';
  ...
  /* V8-1-14-342:
     sessionStorage에 남은 옛 백그라운드 시간값이 짧은 복귀를
     장기 복귀로 오판하지 않도록, 현재 JS 실행 중 실제로 hidden/pagehide에서
     찍힌 값만 복귀 판단에 사용한다. */
  ...
  function removeBackgroundVeilNow(reason){
    try{ clearTimeout(_bgIntroTimer); clearTimeout(_bgVeilRemoveTimer); }catch(_e){}
    try{
      var veil = document.getElementById('oai-background-return-photo-veil');
  ...
  function cancelPendingBackgroundReturn(reason){
    try{ clearTimeout(_bgReturnTimer); }catch(_e){}
    clearBgStamp();
    removeBackgroundVeilNow(reason || 'cancel');
  }
  ...
  function ensureSimpleBackgroundVeil(){
    try{
      ...
      photo.className = 'oai-bg-photo';
      veil.appendChild(photo);
      (document.body || document.documentElement).appendChild(veil);
      return veil;
    }catch(e){ console.warn('[가톨릭길동무]', e); return null; }
  }

  function removeSimpleBackgroundVeil(veil){
    try{ if(veil && veil.parentNode) veil.parentNode.removeChild(veil); }catch(_e){}
    _bgIntroRunning = false;
    clearLegacyBackgroundReturnState();
  }

  function showSimpleBackgroundIntro(toCover, expectedStarted, expectedToken){
    if(_bgIntroRunning) return;
    _bgIntroRunning = true;
    var veil = ensureSimpleBackgroundVeil();
    var coverApplied = false;
    try{
      if(veil){
        /* 복귀 첫 프레임에 원화면이 먼저 보이지 않도록, RAF 지연 없이 바로 덮는다. */
        veil.classList.add('oai-show');
      }
    }catch(_e){}
    if(toCover){
      /* 30분 이상 복귀는 원화면을 먼저 보여준 뒤 커버로 보내지 않고,
         보호막 아래에서 즉시 커버 상태를 확정한다. */
      coverApplied = applyLongBackgroundCoverNow(expectedStarted, expectedToken, 'long-background-immediate');
    }
    _bgIntroTimer = setTimeout(function(){
      try{
        if(toCover && !coverApplied){
          coverApplied = applyLongBackgroundCoverNow(expectedStarted, expectedToken, 'long-background-final');
        }
      }catch(_e){}
      clearBgStamp();
      try{ if(veil) veil.classList.remove('oai-show'); }catch(_e){}
      _bgVeilRemoveTimer = setTimeout(function(){ removeSimpleBackgroundVeil(veil); }, 260);
    }, toCover ? 850 : 650);
  }

  function checkBackgroundReturn(reason, expectedStarted, expectedToken){
    try{
      if(document.visibilityState === 'hidden') return;
      var fresh = readFreshRuntimeBackground();
      if(!fresh){
        cancelPendingBackgroundReturn('stale-background-stamp');
        return;
      }
      var started = fresh.started;
      var token = fresh.token;
      if(expectedStarted && started !== expectedStarted) return;
      if(expectedToken && token !== expectedToken) return;
      if(isExternalReturnContext()){
        cancelPendingBackgroundReturn('external-return');
        return;
      }
      var elapsed = now() - started;
      if(elapsed < MEDIUM_BG_RETURN_MS){
        armShortBackgroundReturnSuppress(reason || 'short-return');
        cancelPendingBackgroundReturn('short-return');
        return;
      }
      ...
      showSimpleBackgroundIntro(elapsed >= COVER_BG_RETURN_MS, started, token);
  ...
  function scheduleBackgroundReturn(reason){
    try{
      ...
      _bgReturnTimer = setTimeout(function(){ checkBackgroundReturn(reason || 'return', started, token); }, 260);
    }catch(e){ console.warn('[가톨릭길동무]', e); }
  }
...
(function(){
  ...
  function relayoutCurrentMaps(reason){
    ...
    try{
      if(typeof requestAnimationFrame === 'function'){
        requestAnimationFrame(function(){ requestAnimationFrame(function(){ runMainMapRestore(reason + '-raf'); }); });
      }else{
        runMainMapRestore(reason);
      }
      /* V8-1-14-342: Fold 전환 체감이 무거워지지 않도록 중간 relayout 횟수를 줄이고,
         화면이 안정된 뒤 필요한 마지막 보정만 남긴다. */
      setTimeout(function(){ runMainMapRestore(reason + '-settle'); }, 160);
      setTimeout(function(){ runMainMapRestore(reason + '-final'); }, 520);
    }catch(_e){}
    try{
      if(typeof window.relayoutTrailMap === 'function'){
        window.relayoutTrailMap(80, reason);
        window.relayoutTrailMap(280, reason + '-settle');
        window.relayoutTrailMap(620, reason + '-final');
      }
    }catch(_e){}
  }
  function beginFoldViewportSettle(reason){
    ...
    try{ document.documentElement.classList.add('oai-fold-resizing'); }catch(_e){}
  ...
  function finishFoldViewportSettle(reason){
    ...
    setTimeout(function(){ recalibrateFoldViewport(reason + '-late'); }, 120);
    clearTimeout(_settleTimer);
    _settleTimer = setTimeout(function(){ relayoutCurrentMaps(reason + '-settle'); }, 120);
    setTimeout(function(){ relayoutCurrentMaps(reason + '-final'); }, 360);
    setTimeout(function(){
      try{ document.documentElement.classList.remove('oai-fold-resizing'); }catch(_e){}
  ...
  function scheduleStableViewport(reason){
    ...
    _foldStableTimer = setTimeout(function(){ _foldStableTimer = 0; finishFoldViewportSettle(reason || 'viewport-change'); }, 180);
  }
  function onViewportChange(reason){
    ...
    if(w && _lastW){
      ...
      if(Math.abs(w-base)>=80) scheduleStableViewport(reason || 'viewport-change');
      else if(!_foldStableTimer) _foldTransitionBaseW = 0;
    }
    _lastW = w || _lastW;
  }
  function init(){
  ...
  window.addEventListener('orientationchange', function(){ setTimeout(function(){ onViewportChange('orientationchange'); },120); }, {passive:true});
  ...
  window.addEventListener('pagehide', function(){
    try{
      clearLegacyResumeKeys();
      if(isAppActive()) markViewportNoIntro('pagehide-app-active');
    }catch(_e){}
  }, {passive:true});
  document.addEventListener('visibilitychange', function(){
    try{
      if(document.visibilityState === 'hidden'){
        clearLegacyResumeKeys();
        if(isAppActive()) markViewportNoIntro('hidden-app-active');
      }else{
        clearLegacyResumeKeys();
        setTimeout(function(){ relayoutCurrentMaps('visible'); }, 120);
      }
    }catch(_e){}
  }, {passive:true});
  window.addEventListener('pageshow', function(){
    try{
      clearLegacyResumeKeys();
      setTimeout(function(){ relayoutCurrentMaps('pageshow'); }, 140);
    }catch(_e){}
  }, {passive:true});
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-342: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V8-1-14-342: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.APP_VERSION || 'V8-1-14-342');
    var current = '';
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-342'));
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-342';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-342';
}
...
/* V8-1-14-342: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V8-1-14-342: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V8-1-14-342: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V8-1-14-342: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V8-1-14-342: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V8-1-14-342: 동작 없는 빈 포커스 리스너와 빈 지도 진입 훅 제거 */
...
/* V8-1-14-342: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V8-1-14-342';
...
const _PRAYER_ASSET_VERSION='V8-1-14-342';
...
const _RETREAT_ASSET_VERSION='V8-1-14-342';
...
// V8-1-14-342: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
/* V8-1-14-342: 지도 확대/축소 중 버벅임을 줄이기 위해
...
const _SHRINE_ASSET_VERSION='V8-1-14-342';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-342: 성지/성당/피정 진입은 cover를 내리기 전 내위치 안내 화면을 먼저 열어 첫 화면으로 고정한다.
...
function oaiPreopenNearbySheetForCategory(){
  try{
    ...
    if(body){
      // V8-1-14-342: 데이터 지연 후 startApp이 다시 호출되어도 같은 내주변 로딩 화면을 다시 그리지 않는다.
      try{
...
function goToCover(){
  ...
  // V8-1-14-342: Fold 화면 전환 뒤 커버로 돌아올 때는 현재 화면 크기를 다시 확정한다.
...
function _scheduleNearbyMyLocationPlainRestore(reason){
  try{
    ...
    // V8-1-14-342: 성지·성당·피정의집 내주변 목록을 닫은 뒤에는
...
function _focusParishPointAround(lat, lng, opts){
  ...
  try{
    ...
    // V8-1-14-342: 성당의 현재 위치 기준 화면은 성지/피정과 동일하게
...
function _loadNearbyWithDist(lat,lng,items,getIdx,getColor,getLabel){
  ...
  if(body && _isNearbyLoadCurrent(requestMode,requestToken,body)){
    // V8-1-14-342: 자동차 거리 계산 단계에서 같은 로딩 화면을 다시 렌더링하지 않는다.
    try{
      if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
        body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
      }
