...
function markExternalReturnStabilize(kind){
  try{
    ...
    // V8-1-14-304: 외부 이동 전 보호창은 #oai-external-entry-guard로 유지하고, 숨김 복귀 때도 같은 보호창을 이어서 사용한다.
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
(function(){
  ...
  function markAppSurfaceBackToCover(reason, ms){
    try{
      var until = now() + (Number(ms || 0) || 9000);
      ...
      setTransient(KEY_BACK_TO_COVER_UNTIL, String(until));
    }catch(_e){}
  }
  function appSurfaceBackToCoverActive(){
    try{
      var until = Number(window.__OAI_APP_SURFACE_BACK_TO_COVER_UNTIL__ || getTransient(KEY_BACK_TO_COVER_UNTIL) || 0);
      if(until && now() < until) return true;
      if(until){
        try{ window.__OAI_APP_SURFACE_BACK_TO_COVER_UNTIL__ = 0; }catch(_e){}
        delTransient(KEY_BACK_TO_COVER_UNTIL);
      }
    }catch(_e){}
    return false;
  }
  function stateHasLiveAppBackTrap(){
    try{
      var hs = history.state || null;
      return !!(hs && (hs.oai_app_trap || hs.oai_restore_trap));
    }catch(_e){ return false; }
  }
  function nativeBackEscapeSaveBlocked(reason, st){
    try{
      if(!st || st.kind === 'cover') return false;
      if(!/^(hidden|pagehide)$/i.test(String(reason || ''))) return false;
      var restoredUntil = Number(window.__OAI_LAST_RESTORED_APP_SURFACE_UNTIL__ || 0);
      var viewportUntil = Number(window.__OAI_VIEWPORT_BACK_REARM_UNTIL__ || 0);
      try{ viewportUntil = Math.max(viewportUntil, Number(sessionStorage.getItem('oai_viewport_back_rearm_until_v304') || 0)); }catch(_e){}
      /* V8-1-14-304:
         Fold/화면전환 직후 Android WebView native back이 JS popstate를 건너뛰고
         앱을 밖으로 보내는 경우가 있다. 이때 hidden/pagehide에서 마지막 카테고리
         화면을 다시 저장하면 다음 실행 때 '탈출한 화면'으로 복원된다.
         복원/화면전환 보호 시간 안의 hidden/pagehide 저장은 history.state 유무와
         관계없이 저장 대상에서 제외한다. */
      if((restoredUntil && now() < restoredUntil) || (viewportUntil && now() < viewportUntil)) return true;
      return !stateHasLiveAppBackTrap();
    }catch(_e){ return false; }
  }
  try{
    ...
    window.oaiMarkAppSurfaceBackToCover = markAppSurfaceBackToCover;
  }catch(_e){}
  clearLegacyResumeKeys();
  ...
  function saveVisibleScreen(reason){
    try{
      ...
      if(appSurfaceBackToCoverActive()){
        clearRestoreState('app-back-to-cover-save-block-' + String(reason || 'save'));
        return null;
      }
      var st=visibleScreenState(reason);
      if(nativeBackEscapeSaveBlocked(reason, st)){
        clearRestoreState('native-back-escape-save-block-' + String(reason || 'save'));
        return null;
      }
  ...
  function armVisibleRestoreBackGuard(kind, st){
    try{
      var until = now() + 30000;
      window.__OAI_RESTORE_BACK_GUARD_UNTIL__ = until;
      window.__OAI_RESTORE_BACK_GUARD_READY__ = false;
      window.__OAI_RESTORE_BACK_GUARD_KIND__ = String(kind || 'screen');
      window.__OAI_RESTORE_BACK_GUARD_STATE_KIND__ = st && st.kind ? String(st.kind) : '';
      if(typeof window.oaiArmRestorePendingBackTrap === 'function'){
        window.oaiArmRestorePendingBackTrap('visible-restore-start-' + String(kind || 'screen'), {force:true});
      }
    }catch(_e){}
  }
  function beginVisibleRestore(kind, st){
    var token = 'v304-' + (++_visibleRestoreSeq) + '-' + now();
    try{
      ...
      armVisibleRestoreBackGuard(kind, st);
  ...
  function completeVisibleRestore(reason){
    try{
      ...
      try{
        window.__OAI_RESTORE_BACK_GUARD_READY__ = true;
        window.__OAI_RESTORE_BACK_GUARD_UNTIL__ = now() + 6000;
        window.__OAI_LAST_RESTORED_APP_SURFACE_UNTIL__ = now() + 12000;
      }catch(_e){}
  ...
  function maybeRestoreVisible(done){
    try{
      ...
      var allow=foldViewportResume || boundaryResume || bgReload;
      ...
      armVisibleRestoreBackGuard('startup-' + String(st.kind || 'screen'), st);
      var tries=0;
      function finish(ok){
        if(ok){
          try{ window._appExiting=false; document.documentElement.classList.remove('app-exiting'); delTransient(KEY_EXIT_UNTIL); }catch(_e){}
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-304: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V8-1-14-304: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.APP_VERSION || 'V8-1-14-304');
    var current = '';
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-304'));
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-304';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-304';
}
...
/* V8-1-14-304: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V8-1-14-304: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V8-1-14-304: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V8-1-14-304: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V8-1-14-304: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V8-1-14-304: 동작 없는 빈 포커스 리스너와 빈 지도 진입 훅 제거 */
...
/* V8-1-14-304: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V8-1-14-304';
...
const _PRAYER_ASSET_VERSION='V8-1-14-304';
...
const _RETREAT_ASSET_VERSION='V8-1-14-304';
...
// V8-1-14-304: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
const _SHRINE_ASSET_VERSION='V8-1-14-304';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-304: 성지/성당/피정 진입은 cover를 내리기 전 내위치 안내 화면을 먼저 열어 첫 화면으로 고정한다.
...
function oaiPreopenNearbySheetForCategory(){
  try{
    ...
    if(body){
      // V8-1-14-304: 데이터 지연 후 startApp이 다시 호출되어도 같은 내주변 로딩 화면을 다시 그리지 않는다.
      try{
...
function goToCover(){
  ...
  // V8-1-14-304: 커버 복귀 때 화면 높이/폰 화면 크기를 다시 계산하지 않는다.
}
...
function _loadNearby(){
  ...
  try{ _updateShrineVisitCardsButtonUI(); }catch(_e){}
  try{ _updateShrineNearbyLocationButtonUI(); }catch(_e){}
  // V8-1-14-304: 카테고리 진입 직후 이미 열린 로딩 화면은 유지하고, 빈 화면/이전 목록일 때만 로딩 화면을 그린다.
  try{
    if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
      body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
    }
...
function _loadNearbyWithDist(lat,lng,items,getIdx,getColor,getLabel){
  ...
  if(body && _isNearbyLoadCurrent(requestMode,requestToken,body)){
    // V8-1-14-304: 자동차 거리 계산 단계에서 같은 로딩 화면을 다시 렌더링하지 않는다.
    try{
      if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
        body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
      }
