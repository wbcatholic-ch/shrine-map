...
(function(){
  ...
  /* V8-1-14-118-DETAIL-INFO-RESUME-SHIELD: 미사용 후 복귀는 예전 WebView 방식으로 단순화한다.
  ...
  window.addEventListener('pageshow', function(){ checkBackgroundReturn('pageshow'); }, {passive:true});
  window.addEventListener('focus', function(){ checkBackgroundReturn('focus'); }, {passive:true});
...
function oaiOpenExternalSite(url, options){
  ...
  // V8-1-14-118-DETAIL-INFO-RESUME-SHIELD: 십자가 보호창이 실제로 보인 뒤 Chrome 전환이 시작되도록 표시 시간을 조금 더 확보한다.
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-118-DETAIL-INFO-RESUME-SHIELD'));
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V8-1-14-118-DETAIL-INFO-RESUME-SHIELD';
...
const _PRAYER_ASSET_VERSION='V8-1-14-118-DETAIL-INFO-RESUME-SHIELD';
...
// V8-1-14-118-DETAIL-INFO-RESUME-SHIELD: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
function _loadNearby(){
  ...
  const usedCache=_useStoredLocationIfAny(go,OAI_LOCATION_CACHE_MAX_MS);
  if(!_GEO){
    if(usedCache) return;
    if(_mode==='shrine') window.__OAI_SHRINE_NEARBY_LOADING__=false;
  ...
  _refreshFreshLocationThen(function(lat,lng){ go(lat,lng,false); },function(err){
    if(usedCache) return;
    if(_mode==='shrine') window.__OAI_SHRINE_NEARBY_LOADING__=false;
    try{ _updateShrineVisitCardsButtonUI(); }catch(_e){}
    if(err.code===1){
...
/* V8-1-14-118-DETAIL-INFO-RESUME-SHIELD: 홈버튼/다른 앱 전환 후 복귀 첫 화면 흔들림을 첫 프레임부터 아이보리 십자가 보호막으로 가린다. */
(function(){
  var KEY='oai_home_resume_shield_pending';
  var timer=null;
  function now(){ return Date.now ? Date.now() : new Date().getTime(); }
  function isExternalPending(){
    try{
      return sessionStorage.getItem('oai_external_nav_pending')==='1' || sessionStorage.getItem('oai_external_nav_pagehide')==='1';
    }catch(_e){ return false; }
  }
  function arm(reason){
    try{
      if(isExternalPending()) return;
      var root=document.documentElement;
      if(!root || !root.classList) return;
      sessionStorage.setItem(KEY, String(now()));
      root.classList.remove('oai-home-resume-shield-releasing');
      root.classList.add('oai-home-resume-shield');
      root.setAttribute('data-oai-home-resume-shield', reason || 'background');
    }catch(_e){}
  }
  function release(reason){
    try{
      ...
      var root=document.documentElement;
      if(!root || !root.classList) return;
      var t=parseInt(sessionStorage.getItem(KEY)||'0',10)||0;
      if(!t && !root.classList.contains('oai-home-resume-shield')) return;
      if(timer) clearTimeout(timer);
      root.classList.add('oai-home-resume-shield');
      root.classList.remove('oai-home-resume-shield-releasing');
      var elapsed=t ? Math.max(0, now()-t) : 0;
      var hold=elapsed > 60000 ? 1500 : 1150;
      timer=setTimeout(function(){
        try{
          root.classList.add('oai-home-resume-shield-releasing');
          setTimeout(function(){
            try{
              root.classList.remove('oai-home-resume-shield','oai-home-resume-shield-releasing');
              root.removeAttribute('data-oai-home-resume-shield');
  ...
  document.addEventListener('visibilitychange', function(){
    if(document.visibilityState==='hidden') arm('visibility-hidden');
    else release('visibility-visible');
  }, true);
  window.addEventListener('pagehide', function(){ arm('pagehide'); }, true);
  window.addEventListener('pageshow', function(){ release('pageshow'); }, true);
  window.addEventListener('focus', function(){ release('focus'); }, true);
})();
