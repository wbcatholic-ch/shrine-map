...
function markExternalReturnStabilize(kind){
  // V37-15: 외부 사이트 이동은 브라우저 기본 동작에 맡긴다.
...
function oaiSmoothNavigate(url, kind){
  // V37-15: 호환용 함수. 보호막/지연/전역 가로채기 없이 즉시 이동한다.
  if(!url) return;
...
function applyExternalReturnStabilize(){
  // V37-15: 복귀 시 화면을 재계산하지 않고, 예전 이동중 잔여 상태만 제거한다.
  try{ oaiClearExternalNavigationState(); }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
function _armMassQuickHistoryTrap(force){
  try{
    // 빠른메뉴 팝업은 커버 위에 뜨는 화면이라 브라우저가 자동으로 history를 만들지 않는다.
    // 예전 플래그가 남아 있어도 실제 history trap이 없으면 팝업에서 뒤로가기 시 앱이 바로 종료될 수 있으므로,
    // 팝업을 새로 열 때는 항상 현재 커버 상태를 trap으로 보정한 뒤 팝업용 trap을 1개 심는다.
    var href = location.href.split('#')[0];
    try{ history.replaceState({_p:1, oai_cover_trap:1}, '', href); }catch(_e){}
    history.pushState({_p:2, oai_mass_quick:1}, '', href);
...
function refreshAppFilesOnly(){
  ...
  try{
    ...
    // V37-15: 새로고침 전에는 레이아웃/스크롤/모달 DOM을 건드리지 않고,
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || 'V37-15';
    var current = '';
...
// V37-15: 커버 전용 주요 기능 안내. 별도 파일 없이 작은 자동 안내 + 자세한 카드형 팝업을 제공한다.
(function(){
  'use strict';
  var HIDE_DAYS = 7;
  ...
  function maybeShowIntro(){
    ...
    if(forceRefresh){
      // V37-15: 안정형 새로고침 뒤에는 어떤 커버 팝업도 자동으로 다시 띄우지 않는다.
...
// V37-15: iPhone 카카오톡 인앱 브라우저에서만 Safari 설치 안내 배너를 표시한다.
(function(){
  'use strict';
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V37-15';
  }else if(!restore){
...
function restoreCoreReturnState(){
  ...
  // V37-15: 외부사이트 복귀 시 지도 중심을 두 단계로 움직이지 않는다.
...
function _getInfoCardCenterTargetY(mapH){
  // V37-15: 성지·성당·피정 지도 중심은 항상 인포카드가 올라왔을 때의 기준으로 통일한다.
...
function closeInfoCard(opts){
  ...
  // V37-15: 시트 전환으로 닫을 때는 지도 중심을 다시 움직이지 않는다.
  // 사용자가 X/지도 터치로 인포카드만 닫을 때는 기존 V37-15 기준 중심을 유지한다.
...
function _mkrImg(color,big){
  const w=big?40:28,h=big?52:36;
  // V37-15: iPhone/Android marker cross unified to the cleaner text-glyph marker.
...
function _focusParishPointAround(lat, lng, opts){
  ...
  try{
    ...
    // V37-15: 현재 위치/내 주변/선택 성당 모두 인포카드 기준 중심으로 통일한다.
...
function _drawLine(s1,s2,path){
  ...
  // V37-15: 길찾기 경로도 성지·성당·피정의집 일반 인포카드와 같은 중심 기준을 사용한다.
