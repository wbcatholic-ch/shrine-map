...
function oaiOpenExternalSite(url, options){
  ...
  // V8-1-14-91-SHRINE-UPDATE-BANNER: 십자가 보호창이 실제로 보인 뒤 Chrome 전환이 시작되도록 표시 시간을 조금 더 확보한다.
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-91-SHRINE-UPDATE-BANNER'));
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V8-1-14-91-SHRINE-UPDATE-BANNER';
...
const _PRAYER_ASSET_VERSION='V8-1-14-91-SHRINE-UPDATE-BANNER';
...
// V8-1-14-91-SHRINE-UPDATE-BANNER: 이동 후 이전 위치가 남지 않도록 위치 캐시는 사용하지 않고, 빠른 신선 위치를 우선 요청한다.
