...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V2-167';
  }else if(!restore){
...
const _PARISH_ASSET_VERSION='V2-167';
...
const _PRAYER_ASSET_VERSION='V2-167';
...
const _SHRINE_ASSET_VERSION='V2-167';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
(function(){
  // V2-167: Android/WebView에서 키보드가 올라올 때 viewport 높이 축소를
...
function resetRoute(opts){
  ...
  if(fromButton){
    ...
    // V2-167: 경로검색 결과의 '다시선택'은 현재 지도 위치·줌을 움직이지 않는다.
...
function _selectRouteItem(idx){
  ...
  // V2-167: 출발지 또는 도착지 중 하나만 남아 있을 때는
...
(function(){
  ...
  function _isFoldWideMapEdgeBack(start, dx, dy, target){
    // V2-167: Fold 큰 화면 지도에서 시스템 back(popstate)와 JS edge swipe가
