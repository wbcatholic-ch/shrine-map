...
function _closeParishVisitEditor(opts){opts=opts||{};if(!opts.fromPopstate&&window.__OAI_PV_EDITOR_HISTORY__){window.__OAI_PV_EDITOR_HISTORY__=false;try{history.back();return;}catch(_e){}}const m=document.getElementById('parish-visit-editor');if(m)m.classList.remove('show');window.__OAI_PV_EDITOR_HISTORY__=false;_updateParishVisitButton();}
