...
function oaiReleaseStabilityVeil(){
  try{
    ...
    if(reason === 'external-leave'){
      ...
      // V2-S: 외부사이트가 실제로 열려 앱이 hidden/pagehide 상태가 된 경우에는
...
function oaiHasExternalReturnPending(){
  try{
    ...
    // V2-S: 실제로 앱이 hidden/pagehide 된 적이 있을 때만 "외부사이트에서 복귀"로 본다.
...
function applyExternalReturnStabilize(){
  ...
  // V2-S: 실제 pagehide/hidden이 확인되지 않은 "열기 시도 중" 상태는 복귀로 오판하지 않는다.
  try{
...
function _resetCoverBackTrap(reason){
  /* V2-S: 커버 trap 재설정도 patches.js의 공통 armCoverBackTrap을 우선 사용한다.
...
function openDioceseExternal(url, state){
  ...
  // V2-S: iframe에서 온 교구 홈페이지 클릭은 지연 setTimeout 없이 즉시 top 페이지를 이동한다.
...
function _getRetreatColor(item){return '#556B4E';}
function _getModeMarkerColor(item){return _mode==='shrine'?(TC[item.type]||'#555'):(_mode==='retreat'?_getRetreatColor(item):'#3F4752');}
...
function _loadNearbyParishes(lat,lng){
  ...
  _loadNearbyWithDist(lat,lng,PARISHES,p=>PARISHES.indexOf(p),()=>'#475569',()=>'⛪ 성당');
}
