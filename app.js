...
function markExternalReturnStabilize(kind){
  // V19-6: 외부 사이트 이동은 브라우저 기본 동작에 맡긴다.
...
function oaiSmoothNavigate(url, kind){
  // V19-6: 호환용 함수. 보호막/지연/전역 가로채기 없이 즉시 이동한다.
  if(!url) return;
...
function applyExternalReturnStabilize(){
  // V19-6: 복귀 시 화면을 재계산하지 않고, 예전 이동중 잔여 상태만 제거한다.
  try{ oaiClearExternalNavigationState(); }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || 'V19-6';
    var current = '';
...
// V19-6: 커버 전용 주요 기능 안내. 별도 파일 없이 작은 자동 안내 + 자세한 카드형 팝업을 제공한다.
(function(){
  'use strict';
  var HIDE_DAYS = 7;
...
// V19-6: iPhone 카카오톡 인앱 브라우저에서는 Safari 설치 안내 배너만 표시한다.
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V19-6';
  }else if(!restore){
...
function restoreCoreReturnState(){
  ...
  // V19-6: 외부사이트 복귀 시 지도 중심을 두 단계로 움직이지 않는다.
...
function _getInfoCardCenterTargetY(mapH){
  // V19-6: 성지·성당·피정 지도 중심은 항상 인포카드가 올라왔을 때의 기준으로 통일한다.
...
function closeInfoCard(opts){
  ...
  // V19-6: 시트 전환으로 닫을 때는 지도 중심을 다시 움직이지 않는다.
  // 사용자가 X/지도 터치로 인포카드만 닫을 때는 기존 V19-6 기준 중심을 유지한다.
...
function _focusParishPointAround(lat, lng, opts){
  ...
  try{
    ...
    // V19-6: 현재 위치/내 주변/선택 성당 모두 인포카드 기준 중심으로 통일한다.
...
document.addEventListener('DOMContentLoaded', function bindEvents() {
  ...
  on('mass-quick-missa', 'click', function() { closeMassQuickMenu(); if (typeof openMissa === 'function') openMissa(); });
  on('mass-quick-prayer', 'click', function() { closeMassQuickMenu(); hideCoverAndRun(function() { if (typeof openPrayerBook === 'function') openPrayerBook(); else alert('기도문 기능이 연결되지 않았습니다.'); }); });
  on('mass-quick-hymn', 'click', function() { closeMassQuickMenu(); if (typeof openCatholicHymn === 'function') openCatholicHymn(); });
