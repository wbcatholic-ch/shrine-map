...
function _openFaithQuickTarget(target){
  try{
    _markFaithSmallButtonUsed(true);
    const mv=$('missa-view');
    if(mv) mv.classList.remove('open');
...
function closeMissa(opts){
  ...
  if(typeof _isFaithSmallButtonUsed==='function' && _isFaithSmallButtonUsed()){
    _returnFaithSmallButtonFlowToCover('faith-small-close');
    return;
  }
...
function _closePrayerAndReturn(){
  ...
  if(typeof _isFaithSmallButtonUsed==='function' && _isFaithSmallButtonUsed()){
    _returnFaithSmallButtonFlowToCover('prayer-small-close');
    return;
  }
  // 작은 버튼을 누르지 않은 기도문 리스트는 기존 정상 흐름대로 '미사 중 빠른 사용' 배너로 복귀한다.
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=WebView-Clean-112';
  }else if(!restore){
...
const _PARISH_ASSET_VERSION='WebView-Clean-112';
...
const _PRAYER_ASSET_VERSION='WebView-Clean-112';
...
const _SHRINE_ASSET_VERSION='WebView-Clean-112';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
