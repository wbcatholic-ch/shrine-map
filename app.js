...
function _runRefreshAppFilesOnly(){
  ...
  try{
    if(btn){
      btn.disabled = true;
      btn.textContent = ((btn.getAttribute('data-target-version') || 'V1-31') + ' 새로고침 중');
    }
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
        var target = btn.getAttribute('data-target-version') || 'V1-31';
    var current = '';
    ...
    if(marker){
      marker.textContent = target || 'V1-31';
...
// Google Play Android 정리본: 모바일 브라우저 안내 로직 제거
  ...
  function bindGuide(){
    ...
    // V1-31 cover menu popstate close
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V1-31';
  }else if(!restore){
...
const _PARISH_ASSET_VERSION='V1-31';
...
const _PRAYER_ASSET_VERSION='V1-31';
...
const _RETREAT_ASSET_VERSION='V1-31';
...
const _SHRINE_ASSET_VERSION='V1-31';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
// V1-31 cover menu hardware back guard
(function(){
  window.addEventListener('popstate', function(){
    try{
