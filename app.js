...
function oaiShowExternalEntryGuard(kind){
  // V8-1-14-161-CATEGORY-FIRST-STABILIZE:
...
(function(){
  ...
  /* V8-1-14-161-CATEGORY-FIRST-STABILIZE: 미사용 후 복귀는 예전 WebView 방식으로 단순화한다.
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-161-CATEGORY-FIRST-STABILIZE: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-161-CATEGORY-FIRST-STABILIZE'));
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V8-1-14-161-CATEGORY-FIRST-STABILIZE';
...
const _PRAYER_ASSET_VERSION='V8-1-14-161-CATEGORY-FIRST-STABILIZE';
...
// V8-1-14-161-CATEGORY-FIRST-STABILIZE: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-161-CATEGORY-FIRST-STABILIZE: 성지/성당/피정 진입은 하단 보호창 대신 내위치 안내문구를 먼저 보여준다.
...
function goToCover(){
  try{ _cancelNearbyLoad(); }catch(e){ console.warn('[가톨릭길동무]', e); }
  try{ document.documentElement.classList.add('oai-cover-return-settling'); }catch(_e){}
  try{
  ...
  try{
    ...
    setTimeout(function(){ try{ document.documentElement.classList.remove('oai-cover-return-settling'); }catch(_e){} }, 420);
  }catch(e){
    try{ document.documentElement.classList.remove('oai-cover-return-settling'); }catch(_e){}
    console.warn('[가톨릭길동무]', e);
  }
}
