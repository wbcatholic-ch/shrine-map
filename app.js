...
(function(){
  ...
  /* V8-1-14-126-DARK-BUTTONS-CROSS-STANDARD: 미사용 후 복귀는 예전 WebView 방식으로 단순화한다.
...
function oaiOpenExternalSite(url, options){
  ...
  // V8-1-14-126-DARK-BUTTONS-CROSS-STANDARD: 십자가 보호창이 실제로 보인 뒤 Chrome 전환이 시작되도록 표시 시간을 조금 더 확보한다.
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-126-DARK-BUTTONS-CROSS-STANDARD: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
  ...
  const detailHomeStyle='background:linear-gradient(180deg,#b9dfc4 0%,#7fb993 100%)!important;color:#083d24!important;border:1.5px solid #3f8f5b!important;font-weight:950!important;box-shadow:0 3px 9px rgba(28,94,55,.17), inset 0 1px 0 rgba(255,255,255,.42)!important;';
  ...
  const detailGoodnewsStyle='background:#0056d6!important;color:#ffffff!important;border:1.8px solid #003f9e!important;box-shadow:0 2px 9px rgba(0,86,214,.28)!important;text-shadow:0 1px 1px rgba(0,0,0,.20)!important;font-weight:950!important;';
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-126-DARK-BUTTONS-CROSS-STANDARD'));
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V8-1-14-126-DARK-BUTTONS-CROSS-STANDARD';
...
const _PRAYER_ASSET_VERSION='V8-1-14-126-DARK-BUTTONS-CROSS-STANDARD';
...
// V8-1-14-126-DARK-BUTTONS-CROSS-STANDARD: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
