...
function _runRefreshAppFilesOnly(){
  ...
  try{
    if(btn){
      btn.disabled = true;
      btn.textContent = ((btn.getAttribute('data-target-version') || 'V1-16') + ' 새로고침 중');
    }
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(marker){
      marker.textContent = target || 'V1-16';
...
const _PARISH_ASSET_VERSION='V1-16';
...
const _PRAYER_ASSET_VERSION='V1-16';
...
const _RETREAT_ASSET_VERSION='V1-16';
...
const _SHRINE_ASSET_VERSION='V1-16';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
