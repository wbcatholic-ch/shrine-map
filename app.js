...
function markExternalReturnStabilize(kind){
  try{
    ...
    // V8-1-14-351: 외부 이동 전 보호창은 #oai-external-entry-guard로 유지하고, 숨김 복귀 때도 같은 보호창을 이어서 사용한다.
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
(function(){
  ...
  /* V8-1-14-351:
     백그라운드 복귀는 visibility hidden에서 시작된 현재 세션 토큰만 신뢰한다.
     복귀 첫 프레임에 원화면이 보이지 않도록 숨김 순간에 단순 아이보리 막을 미리 올리고,
     30분 미만은 커버 이동 경로를 완전히 차단한다. */
  var MEDIUM_BG_RETURN_MS = 10 * 60 * 1000;
  var COVER_BG_RETURN_MS = 30 * 60 * 1000;
  var BG_KEY = 'oai_home_backgrounded_at';
  var BG_TOKEN_KEY = 'oai_home_background_token_v345';
  var BG_SHORT_RETURN_UNTIL_KEY = 'oai_short_background_return_until_v345';
  var OLD_HIDDEN_AT_KEY = 'oai_pwa_backgrounded_at_v356';
  var OLD_RESTART_LOCK_KEY = 'oai_pwa_idle_restart_lock_v356';
  var _bgReturnTimer = 0;
  var _bgVeilRemoveTimer = 0;
  var _bgRuntimeStarted = 0;
  var _bgRuntimeToken = '';
  var _bgRuntimeArmed = false;
  var _bgReturnDecisionId = 0;

  function now(){ return Date.now ? Date.now() : new Date().getTime(); }

  function clearLegacyBackgroundReturnState(opts){
    opts = opts || {};
    try{
      ...
      sessionStorage.removeItem('oai_long_background_early_intro_consumed');
      var root = document.documentElement;
  ...
  function readFreshRuntimeBackground(){
    try{
      var stored = parseInt(sessionStorage.getItem(BG_KEY) || '0', 10) || 0;
      var storedToken = sessionStorage.getItem(BG_TOKEN_KEY) || '';
      if(!_bgRuntimeArmed || !_bgRuntimeStarted || !_bgRuntimeToken) return null;
      if(!stored || !storedToken) return null;
      if(stored !== _bgRuntimeStarted || storedToken !== _bgRuntimeToken) return null;
      return {started: stored, token: storedToken};
    }catch(_e){ return null; }
  }

  function armShortBackgroundReturnSuppress(reason){
    try{
      var until = now() + 8000;
      window.__OAI_SHORT_BACKGROUND_RETURN_UNTIL__ = until;
      sessionStorage.setItem(BG_SHORT_RETURN_UNTIL_KEY, String(until));
    }catch(_e){}
  }

  function isShortBackgroundReturnSuppressed(){
    try{
      var until = Number(window.__OAI_SHORT_BACKGROUND_RETURN_UNTIL__ || sessionStorage.getItem(BG_SHORT_RETURN_UNTIL_KEY) || 0) || 0;
      if(until && now() < until) return true;
      if(until){
        window.__OAI_SHORT_BACKGROUND_RETURN_UNTIL__ = 0;
        sessionStorage.removeItem(BG_SHORT_RETURN_UNTIL_KEY);
      }
    }catch(_e){}
    return false;
  }

  function isAndroidNativeShortResume(){
    try{
      var until = Number(window.__OAI_ANDROID_NATIVE_SHORT_RESUME_UNTIL__ || 0) || 0;
      if(until && now() < until) return true;
      if(until) window.__OAI_ANDROID_NATIVE_SHORT_RESUME_UNTIL__ = 0;
    }catch(_e){}
    return false;
  }

  function cancelForAndroidNativeShortResume(reason){
    if(!isAndroidNativeShortResume()) return false;
    try{ armShortBackgroundReturnSuppress('android-native-short-' + String(reason || 'resume')); }catch(_e){}
    cancelPendingBackgroundReturn('android-native-short-resume');
    return true;
  }
  ...
  function ensureSimpleBackgroundVeil(showNow){
    try{
      var id = 'oai-background-return-photo-veil';
      var old = document.getElementById(id);
      if(!document.getElementById('oai-background-return-photo-veil-style')){
        var st = document.createElement('style');
        st.id = 'oai-background-return-photo-veil-style';
        st.textContent = '\n'
          + '#oai-background-return-photo-veil{position:fixed;inset:0;z-index:2147482498;background:#fbf8f1;opacity:0;transition:opacity .10s ease;pointer-events:none;display:block;visibility:visible;contain:strict;}\n'
          + '#oai-background-return-photo-veil.oai-show{opacity:1;}\n'
          + '#oai-background-return-photo-veil .oai-bg-photo{display:none!important;}\n';
      ...
      if(showNow) veil.classList.add('oai-show');
      return veil;
    }catch(e){ console.warn('[가톨릭길동무]', e); return null; }
  }

  function removeSimpleBackgroundVeil(delay){
    clearTimeout(_bgVeilRemoveTimer);
    var veil = null;
    try{ veil = document.getElementById('oai-background-return-photo-veil'); }catch(_e){}
    if(!veil){ clearLegacyBackgroundReturnState(); return; }
    try{ veil.classList.remove('oai-show'); }catch(_e){}
    _bgVeilRemoveTimer = setTimeout(function(){
      try{ if(veil && veil.parentNode) veil.parentNode.removeChild(veil); }catch(_e){}
      clearLegacyBackgroundReturnState();
    }, Number.isFinite(Number(delay)) ? Number(delay) : 120);
  }

  function applyLongBackgroundCoverNow(started, token, reason){
    try{
      var fresh = readFreshRuntimeBackground();
      if(!fresh || fresh.started !== started || fresh.token !== token) return false;
      var elapsed = now() - started;
      if(elapsed < COVER_BG_RETURN_MS) return false;
      if(isShortBackgroundReturnSuppressed()) return false;
      if(!isReturnableScreenActive()) return false;
      if(typeof goToCover !== 'function') return false;
      goToCover();
      return true;
    }catch(_e){ return false; }
  }

  function cancelPendingBackgroundReturn(reason, keepVeil){
    try{ clearTimeout(_bgReturnTimer); }catch(_e){}
    clearBgStamp();
    if(!keepVeil) removeSimpleBackgroundVeil(80);
  }
  ...
  function markBackgrounded(reason){
    try{
      ...
      if(isReturnableScreenActive() && !isExternalReturnContext()){
        ...
        _bgRuntimeStarted = t;
        _bgRuntimeToken = token;
        _bgRuntimeArmed = true;
        sessionStorage.setItem(BG_KEY, String(t));
        sessionStorage.setItem(BG_TOKEN_KEY, token);
        /* V8-1-14-351: 짧은 백그라운드 복귀 때 아이보리 막이 두 번 깜빡이는 것을 막기 위해
           숨김 순간에는 막을 올리지 않는다. 30분 이상 장기 복귀일 때만 복귀 판단 직후 막을 사용한다. */
      }else{
        clearBgStamp();
        removeSimpleBackgroundVeil(60);
      }
    }catch(e){ console.warn('[가톨릭길동무]', e); }
  }

  function decideBackgroundReturn(reason, decisionId, expectedStarted, expectedToken){
    try{
      if(cancelForAndroidNativeShortResume(reason || 'decide')) return;
      if(document.visibilityState === 'hidden') return;
      if(decisionId !== _bgReturnDecisionId) return;
      var fresh = readFreshRuntimeBackground();
      if(!fresh){ cancelPendingBackgroundReturn('stale-background-stamp'); return; }
      if(expectedStarted && fresh.started !== expectedStarted) return;
      if(expectedToken && fresh.token !== expectedToken) return;
      if(isExternalReturnContext()){ cancelPendingBackgroundReturn('external-return'); return; }
      var elapsed = now() - fresh.started;
      if(elapsed < COVER_BG_RETURN_MS){
        armShortBackgroundReturnSuppress(reason || 'short-return');
        clearBgStamp();
        removeSimpleBackgroundVeil(elapsed < MEDIUM_BG_RETURN_MS ? 70 : 140);
        return;
      }
      if(!isReturnableScreenActive()){
        cancelPendingBackgroundReturn('not-returnable');
        return;
      }
      ensureSimpleBackgroundVeil(true);
      applyLongBackgroundCoverNow(fresh.started, fresh.token, 'long-background-' + String(reason || 'return'));
      clearBgStamp();
      removeSimpleBackgroundVeil(160);
    }catch(e){ console.warn('[가톨릭길동무]', e); cancelPendingBackgroundReturn('error'); }
  }

  function scheduleBackgroundReturn(reason){
    try{
      if(cancelForAndroidNativeShortResume(reason || 'schedule')) return;
      if(document.visibilityState === 'hidden') return;
      var fresh = readFreshRuntimeBackground();
      if(!fresh){
        removeSimpleBackgroundVeil(60);
      ...
      clearTimeout(_bgReturnTimer);
      var decisionId = ++_bgReturnDecisionId;
      var elapsed = now() - fresh.started;
      if(elapsed < COVER_BG_RETURN_MS){
        _bgReturnTimer = setTimeout(function(){ decideBackgroundReturn(reason || 'return', decisionId, fresh.started, fresh.token); }, elapsed < MEDIUM_BG_RETURN_MS ? 35 : 90);
      }else{
        decideBackgroundReturn(reason || 'return', decisionId, fresh.started, fresh.token);
      }
    }catch(e){ console.warn('[가톨릭길동무]', e); }
  }
  ...
  window.addEventListener('oai-android-native-resume', function(ev){
    try{
      ...
      if(detail && detail.forceCover){
        ensureSimpleBackgroundVeil(true);
        clearBgStamp();
        if(typeof goToCover === 'function') goToCover();
        removeSimpleBackgroundVeil(120);
        return;
      }
      window.__OAI_ANDROID_NATIVE_SHORT_RESUME_UNTIL__ = now() + 12000;
      cancelForAndroidNativeShortResume('native-event');
    }catch(e){ console.warn('[가톨릭길동무]', e); }
  }, {passive:true});
  ...
  window.addEventListener('pagehide', function(){
    if(document.visibilityState === 'hidden') markBackgrounded('pagehide-hidden');
  }, {passive:true});
...
(function(){
  ...
  function beginFoldViewportSettle(reason){
    ...
    try{
      document.documentElement.classList.add('oai-fold-resizing');
      window.__OAI_FOLD_VIEWPORT_SETTLING__ = true;
      if(typeof window.oaiApplyAndroidScreenClass === 'function'){
        window.oaiApplyAndroidScreenClass((reason || 'viewport') + '-immediate-force-remeasure');
      }
    }catch(_e){}
  }
  function finishFoldViewportSettle(reason){
    ...
    recalibrateFoldViewport(reason + '-stable');
    _settleTimer = setTimeout(function(){ relayoutCurrentMaps(reason + '-stable'); }, 90);
    setTimeout(function(){
      try{
        document.documentElement.classList.remove('oai-fold-resizing','oai-trail-map-settling');
        window.__OAI_FOLD_VIEWPORT_SETTLING__ = false;
      }catch(_e){}
  ...
  function scheduleStableViewport(reason){
    ...
    /* V8-1-14-351: 화면 등급은 즉시 적용하고 지도/커버 보정만 짧게 안정 후 마무리한다. */
  ...
  function onViewportChange(reason){
    ...
    var active = !!(_foldStableTimer || document.documentElement.classList.contains('oai-fold-resizing'));
    if(diff >= 80 || active){
...
function openFaithPortal(kind, opts){
  ...
  if(kind==='hymn' || kind==='bible'){
    /* V8-1-14-229: 성경도 성가와 같은 외부브라우저/외부페이지 흐름으로 연다. */
    try{
      if(typeof _setFaithReturnTarget === 'function') _setFaithReturnTarget('massQuick');
      if(typeof _setMassQuickReturn === 'function') _setMassQuickReturn(true);
    }catch(e){ console.warn('[가톨릭길동무]', e); }
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-351: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V8-1-14-351: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.APP_VERSION || 'V8-1-14-351');
    var current = '';
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-351'));
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-351';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-351';
}
...
/* V8-1-14-351: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V8-1-14-351: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V8-1-14-351: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V8-1-14-351: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V8-1-14-351: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V8-1-14-351: 동작 없는 빈 포커스 리스너와 빈 지도 진입 훅 제거 */
...
/* V8-1-14-351: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V8-1-14-351';
...
const _PRAYER_ASSET_VERSION='V8-1-14-351';
...
const _RETREAT_ASSET_VERSION='V8-1-14-351';
...
// V8-1-14-351: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
const _GO_ACCURATE_FRESH={enableHighAccuracy:true,timeout:12000,maximumAge:0};
...
/* V8-1-14-351: 지도 확대/축소 중 버벅임을 줄이기 위해
...
function _oaiMarkerImageCached(key, maker){
  try{
    /* V8-1-14-351:
       Kakao MarkerImage 객체를 여러 마커가 공유하면 Android WebView에서 축소/확대 중
       마커가 네모처럼 보이거나 갱신이 꼬일 수 있다. 데이터 생성은 가볍게 유지하되
       MarkerImage 인스턴스 자체는 마커마다 새로 만들어 안전하게 사용한다. */
    return maker();
...
function _setMarkerMapIfChanged(marker, map){
  ...
  try{
    ...
    /* V8-1-14-351: 일부 Kakao Marker는 getMap이 없어 __oaiMapTarget만 남는데,
...
const _SHRINE_ASSET_VERSION='V8-1-14-351';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-351: 성지/성당/피정 진입은 cover를 내리기 전 내위치 안내 화면을 먼저 열어 첫 화면으로 고정한다.
...
function oaiPreopenNearbySheetForCategory(){
  try{
    ...
    if(body){
      // V8-1-14-351: 데이터 지연 후 startApp이 다시 호출되어도 같은 내주변 로딩 화면을 다시 그리지 않는다.
      try{
...
function goToCover(){
  ...
  // V8-1-14-351: Fold 화면 전환 뒤 커버로 돌아올 때는 현재 화면 크기를 다시 확정한다.
...
function _scheduleNearbyMyLocationPlainRestore(reason){
  try{
    ...
    // V8-1-14-351: 성지·성당·피정의집 내주변 목록을 닫은 뒤에는
...
function _focusParishPointAround(lat, lng, opts){
  ...
  try{
    ...
    // V8-1-14-351: 성당의 현재 위치 기준 화면은 성지/피정과 동일하게
...
function _showParishDioMkrs(code){
  ...
  /* V8-1-14-351: 성당 지도는 idle 때마다 교구 전체 마커 setMap을 반복해
...
function _loadNearbyWithDist(lat,lng,items,getIdx,getColor,getLabel){
  ...
  if(body && _isNearbyLoadCurrent(requestMode,requestToken,body)){
    // V8-1-14-351: 자동차 거리 계산 단계에서 같은 로딩 화면을 다시 렌더링하지 않는다.
    try{
      if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
        body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
      }
...
(function(){
  ...
  function _doMainSwipe(dx){
    ...
    /* V8-1-14-222: 성지·성당·피정의집 좌우 스와이프는 하단 시트 내부에서 시작한 한 손가락 터치만 처리한다.
