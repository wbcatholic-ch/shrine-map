...
function markExternalReturnStabilize(kind){
  try{
    ...
    // V8-1-14-440: 외부 이동 전 보호창은 #oai-external-entry-guard로 유지하고, 숨김 복귀 때도 같은 보호창을 이어서 사용한다.
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
function openFaithPortal(kind, opts){
  ...
  if(kind==='hymn' || kind==='bible'){
    /* V8-1-14-440: 성경도 성가와 같은 외부브라우저/외부페이지 흐름으로 연다. */
    try{
      if(typeof _setFaithReturnTarget === 'function') _setFaithReturnTarget('massQuick');
      if(typeof _setMassQuickReturn === 'function') _setMassQuickReturn(true);
    }catch(e){ console.warn('[가톨릭길동무]', e); }
...
function _updateShrineVisitFloatingListButtonUI(){
  ...
  /* V8-1-14-440: 성지 내주변은 캐시 위치/새 GPS 재계산 사이에도 완료 전 상태로 보고, 나의순례현황 버튼이 먼저 보였다가 사라지는 깜빡임을 막는다. */
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-440: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V8-1-14-440: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.OAI_APP_BUILD_VERSION || window.APP_VERSION || 'V8-1-14-440');
    var current = '';
    /* V8-1-14-440:
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-440'));
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-440';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-440';
}
...
/* V8-1-14-440: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V8-1-14-440: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V8-1-14-440: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V8-1-14-440: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V8-1-14-440: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V8-1-14-440: route reset helper is kept without inactive focus/map-entry hooks. */
...
/* V8-1-14-440: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V8-1-14-440';
...
const _PRAYER_ASSET_VERSION='V8-1-14-440';
...
const _RETREAT_ASSET_VERSION='V8-1-14-440';
...
// V8-1-14-440: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
function _setMarkerMapIfChanged(marker, map){
  ...
  try{
    ...
    /* V8-1-14-440: 일부 Kakao Marker는 getMap이 없어 __oaiMapTarget만 남는데,
...
const _SHRINE_ASSET_VERSION='V8-1-14-440';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-440: 성지/성당/피정 진입은 cover를 내리기 전 내위치 안내 화면을 먼저 열어 첫 화면으로 고정한다.
...
function oaiPreopenNearbySheetForCategory(){
  try{
    ...
    if(body){
      // V8-1-14-440: 데이터 지연 후 startApp이 다시 호출되어도 같은 내주변 로딩 화면을 다시 그리지 않는다.
      try{
...
function goToCover(){
  ...
  // V8-1-14-440: Fold 화면 전환 뒤 커버로 돌아올 때는 현재 화면 크기를 다시 확정한다.
...
function openTab(name, opts){
  ...
  // V8-1-14-440: 성지/성당/피정의집 시트 전환은 무거운 가로 swiper 애니메이션 없이 즉시 전환하고, 내부 좌우 스와이프 전환은 비활성화한다.
...
function _scheduleNearbyMyLocationPlainRestore(reason){
  try{
    ...
    // V8-1-14-440: 성지·성당·피정의집 내주변 목록을 닫은 뒤에는
...
function _focusParishPointAround(lat, lng, opts){
  ...
  try{
    ...
    // V8-1-14-440: 성당의 현재 위치 기준 화면은 성지/피정과 동일하게
...
function _showParishDioMkrs(code){
  ...
  /* V8-1-14-440: 성당 지도는 idle 때마다 교구 전체 마커 setMap을 반복해
  ...
  try{
    _GEO.getCurrentPosition(ok,function(e){
      ...
      if(elapsed < 300) setTimeout(function(){ fallback(e); }, 150);
        ...
        if(fail) fail(e);
        /* V8-1-14-440: 빠른 위치가 실패했을 때도 UI를 오래 잡아두지 않고,
...
function _loadNearbyWithDist(lat,lng,items,getIdx,getColor,getLabel){
  ...
  if(body && _isNearbyLoadCurrent(requestMode,requestToken,body)){
    // V8-1-14-440: 자동차 거리 계산 단계에서 같은 로딩 화면을 다시 렌더링하지 않는다.
    try{
      if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
        body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
      }
...
(function(){
  ...
  function _doMainSwipe(dx){
    ...
    /* V8-1-14-440: 성지·성당·피정의집 좌우 스와이프는 하단 시트 내부에서 시작한 한 손가락 터치만 처리한다.
