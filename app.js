...
(function(){
  ...
  /* V8-1-14-119-DETAIL-BUTTONS-FINAL-CSS: 미사용 후 복귀는 예전 WebView 방식으로 단순화한다.
...
function oaiOpenExternalSite(url, options){
  ...
  // V8-1-14-119-DETAIL-BUTTONS-FINAL-CSS: 십자가 보호창이 실제로 보인 뒤 Chrome 전환이 시작되도록 표시 시간을 조금 더 확보한다.
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-119-DETAIL-BUTTONS-FINAL-CSS'));
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V8-1-14-119-DETAIL-BUTTONS-FINAL-CSS';
...
const _PRAYER_ASSET_VERSION='V8-1-14-119-DETAIL-BUTTONS-FINAL-CSS';
...
// V8-1-14-119-DETAIL-BUTTONS-FINAL-CSS: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
/* V8-1-14-119-DETAIL-BUTTONS-FINAL-CSS: 홈버튼/다른 앱 전환 후 복귀 첫 화면 흔들림을 첫 프레임부터 아이보리 십자가 보호막으로 가린다. */
(function(){
