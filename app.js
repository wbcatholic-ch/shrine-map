...
function markExternalReturnStabilize(kind){
  // V18K: 외부 사이트 이동은 브라우저 기본 동작에 맡긴다.
...
function oaiSmoothNavigate(url, kind){
  // V18K: 호환용 함수. 보호막/지연/전역 가로채기 없이 즉시 이동한다.
  if(!url) return;
...
function applyExternalReturnStabilize(){
  // V18K: 복귀 시 화면을 재계산하지 않고, 예전 이동중 잔여 상태만 제거한다.
  try{ oaiClearExternalNavigationState(); }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || 'V18K';
    var current = '';
...
// V18K: 커버 전용 주요 기능 안내. 별도 파일 없이 작은 자동 안내 + 자세한 카드형 팝업을 제공한다.
(function(){
  'use strict';
  var HIDE_DAYS = 7;
...
// V18K: 임시 확인용으로 Safari 안내 배너를 브라우저에서 보이게 한다.
// 확인 후 다시 iPhone 카카오톡 전용 조건으로 되돌릴 예정이며, Android Chrome intent 흐름은 건드리지 않는다.
(function(){
  'use strict';
  ...
  function shouldShow(){ return !isStandalone(); }
  function showModal(){
...
function restoreCoreReturnState(){
  ...
  // V18K: 외부사이트 복귀 시 지도 중심을 두 단계로 움직이지 않는다.
...
function _getInfoCardCenterTargetY(mapH){
  // V18K: 성지·성당·피정 지도 중심은 항상 인포카드가 올라왔을 때의 기준으로 통일한다.
...
function closeInfoCard(opts){
  ...
  // V18K: 시트 전환으로 닫을 때는 지도 중심을 다시 움직이지 않는다.
  // 사용자가 X/지도 터치로 인포카드만 닫을 때는 기존 V18K 기준 중심을 유지한다.
...
function _focusParishPointAround(lat, lng, opts){
  ...
  try{
    ...
    // V18K: 현재 위치/내 주변/선택 성당 모두 인포카드 기준 중심으로 통일한다.
