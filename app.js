...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V2-178';
  }else if(!restore){
...
const _PARISH_ASSET_VERSION='V2-178';
...
const _PRAYER_ASSET_VERSION='V2-178';
...
const _SHRINE_ASSET_VERSION='V2-178';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
(function(){
  // V2-178: Android/WebView에서 키보드가 올라올 때 viewport 높이 축소를
...
function resetRoute(opts){
  ...
  // 다시선택 버튼: 현재 지도 위치와 줌은 그대로 유지하고,
  // 경로 표시만 해제한 뒤 길찾기 재선택 카드(출발/도착 입력 화면)를 유지한다.
  if(fromButton){
    ...
    // V2-178: 경로검색 결과의 '다시선택'은 현재 지도 위치·줌을 움직이지 않는다.
...
function _selectRouteItem(idx){
  ...
  // V2-178: 출발지 또는 도착지 중 하나만 남아 있을 때는
  ...
  if(shouldSetStart){
    const keepExistingEnd = !!(_rE && _rE.lat && _rE.lng);
    _routeRegionStart=null;
    ...
    if(keepExistingEnd) _setRouteLabel('end',_rE.name);
    else { _rE=null; _setRouteLabel('end',''); }
    _refreshRouteTmpMarkers();
    _restoreMarkersWhenRouteNotDisplayed();
    if(_rE && _rE.lat){
      _hideRouteGuide();
...
(function(){
  ...
  function _isFoldWideMapEdgeBack(start, dx, dy, target){
    // V2-178: Fold 큰 화면 지도에서 시스템 back(popstate)와 JS edge swipe가
...
document.addEventListener('DOMContentLoaded', function bindEvents() {
  ...
  on('mass-quick-prayer', 'click', function() {
    // 모든 교구가 앱 내부 기도문으로 진입한다.
    // 대구대교구 인준 기도 탭은 prayer.js에서 나의 교구가 대구대교구일 때만 노출한다.
    try{
