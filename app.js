...
function markExternalReturnStabilize(kind){
  try{
    ...
    // V8-1-14-327: 외부 이동 전 보호창은 #oai-external-entry-guard로 유지하고, 숨김 복귀 때도 같은 보호창을 이어서 사용한다.
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
/* V8-1-14-327:
   V315 안정 흐름을 기준으로 Fold 전환 시 현재 카테고리 표면만 localStorage에 최소 저장해 1회 복귀한다.
   - 커버 Back trap은 건드리지 않음
   - 전체 화면 자동복원/무한복원 금지
   - Activity 재생성으로 sessionStorage가 사라져도 localStorage로 한 번만 복귀
   - 내주변은 저장된 목록 HTML만 복원하고 거리계산은 재실행하지 않음 */
(function(){
  'use strict';
  if(window.__OAI_FOLD_SURFACE_V327__) return;
  window.__OAI_FOLD_SURFACE_V327__ = true;

  var KEY = 'oai_fold_surface_v327';
  var KEY_NO_INTRO = 'oai_viewport_no_intro_until_v315';
  var MAX_AGE = 5 * 60 * 1000;
  var WIDTH_DELTA = 80;
  var _restoreStarted = false;
  function now(){ return Date.now ? Date.now() : new Date().getTime(); }
  function width(){ try{ return Math.round(window.innerWidth || document.documentElement.clientWidth || 0); }catch(_e){ return 0; } }
  function get(k){ var v=''; try{ v=sessionStorage.getItem(k)||''; }catch(_e){} if(!v){ try{ v=localStorage.getItem(k)||''; }catch(_e){} } return v; }
  function set(k,v){ try{ sessionStorage.setItem(k,v); }catch(_e){} try{ localStorage.setItem(k,v); }catch(_e){} }
  function del(k){ try{ sessionStorage.removeItem(k); }catch(_e){} try{ localStorage.removeItem(k); }catch(_e){} }
  function modeOk(m){ return m === 'shrine' || m === 'parish' || m === 'retreat'; }
  function coverVisible(){ try{ return typeof window._isCoverScreenVisible === 'function' ? window._isCoverScreenVisible() : !document.documentElement.classList.contains('app-active'); }catch(_e){ return false; } }
  function appActive(){
  ...
  function markNoIntro(){ try{ set(KEY_NO_INTRO, String(now() + 120000)); }catch(_e){} }
  function readState(){ try{ var raw=get(KEY); return raw ? JSON.parse(raw) : null; }catch(_e){ return null; } }
  function writeState(st){ try{ set(KEY, JSON.stringify(st)); }catch(_e){} }
  function currentTab(){
    try{ if(typeof _activeTab !== 'undefined' && _activeTab) return _activeTab; }catch(_e){}
    try{
      var names = ['nearby','list','region','route'];
      for(var i=0;i<names.length;i++){
        var el = document.getElementById('sheet-' + names[i]);
        if(el && el.classList && el.classList.contains('open')) return names[i];
      }
    }catch(_e){}
    return 'map';
  }
  function bodySnapshot(tab){
    try{
      if(!(tab === 'nearby' || tab === 'list' || tab === 'region')) return '';
      var el = document.getElementById(tab + '-body');
      if(!el) return '';
      var html = el.innerHTML || '';
      if(html.length > 160000) return '';
      return html;
    }catch(_e){ return ''; }
  }
  function mapCamera(){
    try{
      if(typeof _map !== 'undefined' && _map && _map.getCenter && _map.getLevel){
        ...
        return {lat:c && c.getLat ? c.getLat() : null, lng:c && c.getLng ? c.getLng() : null, level:_map.getLevel()};
      }
    }catch(_e){}
    return null;
  }
  function record(reason){
    try{
      if(window._appExiting) return null;
      if(!appActive()) return null;
      var m = '';
      try{ m = (typeof _mode !== 'undefined') ? _mode : ''; }catch(_e){}
      if(!modeOk(m)) return null;
      var tab = currentTab();
      var st = {
        kind:'category', mode:m, tab:tab, w:width(), ts:now(), reason:reason||'record',
        html:bodySnapshot(tab), scroll:0, camera:mapCamera()
      };
      try{ var b=document.getElementById(tab+'-body'); if(b) st.scroll=b.scrollTop||0; }catch(_e){}
      writeState(st);
      return st;
  ...
  function valid(st){
    try{
      if(!st || st.kind !== 'category' || !modeOk(st.mode)) return false;
      if(!st.ts || now() - st.ts > MAX_AGE) return false;
      var cw = width();
      if(!st.w || !cw) return false;
      return Math.abs(cw - Number(st.w || 0)) >= WIDTH_DELTA;
    }catch(_e){ return false; }
  }
  function armAppBack(reason){
    try{
      if(!appActive() || coverVisible()) return false;
      if(typeof window._oaiArmAppBackTrap === 'function') return !!window._oaiArmAppBackTrap(reason || 'fold-surface-v327', {force:true});
      var href = location.href.split('#')[0];
      history.replaceState({_p:0, oai_app_root:reason||'fold-root-v327'}, '', href);
      history.pushState({_p:1, oai_app_trap:reason||'fold-trap-v327'}, '', href);
      return true;
    }catch(_e){ return false; }
  }
  function restoreBody(st){
    try{
      var tab = st && st.tab;
      if(!(tab === 'nearby' || tab === 'list' || tab === 'region')) return;
      var el = document.getElementById(tab + '-body');
      if(st.html && el) el.innerHTML = st.html;
      if(st.scroll){ setTimeout(function(){ try{ var b=document.getElementById(tab+'-body'); if(b) b.scrollTop = st.scroll || 0; }catch(_e){} }, 80); }
    }catch(_e){}
  }
  function applyCamera(st){
    try{
      var cam = st && st.camera;
      if(!cam || !cam.lat || !cam.lng || typeof _map === 'undefined' || !_map || !window.kakao || !kakao.maps) return;
      var pos = new kakao.maps.LatLng(cam.lat, cam.lng);
      if(cam.level && _map.setLevel) _map.setLevel(cam.level);
      if(_map.setCenter) _map.setCenter(pos);
    }catch(_e){}
  }
  window.oaiApplyFoldSurfaceV327 = function(reason){
    try{
      var st = window.__OAI_FOLD_STATE_V327__ || readState();
      if(!st || !modeOk(st.mode)) return false;
      window.__OAI_FOLD_RESTORE_V327_UNTIL__ = now() + 7000;
      applyCamera(st);
      var tab = st.tab || 'map';
      if(tab === 'nearby' || tab === 'list' || tab === 'region'){
        if(typeof openTab === 'function') openTab(tab, {skipNearbyLoad:true, foldRestore:true});
        restoreBody(st);
      }else if(tab === 'route'){
        if(typeof openTab === 'function') openTab('route', {foldRestore:true});
      }
      setTimeout(function(){ armAppBack(reason || 'fold-apply'); }, 80);
      setTimeout(function(){ armAppBack((reason || 'fold-apply') + '-late'); }, 520);
      st.w = width(); st.ts = now(); st.reason = 'restored:' + (reason || 'fold');
      writeState(st);
      return true;
    }catch(_e){ return false; }
  };
  window.oaiRecordFoldSurfaceV327 = record;
  window.oaiArmFoldBackV327 = armAppBack;

  function tryRestore(reason){
    try{
      if(_restoreStarted) return false;
      var st = readState();
      if(!valid(st)) return false;
      _restoreStarted = true;
      markNoIntro();
      window.__OAI_FOLD_STATE_V327__ = st;
      window.__OAI_FOLD_RESTORE_V327_UNTIL__ = now() + 9000;
      window.__OAI_SKIP_AUTO_LOCATE_UNTIL__ = now() + 9000;
      window.__OAI_SUPPRESS_NEARBY_LOAD_UNTIL__ = now() + 9000;
      try{ window._noAutoNearby = true; }catch(_e){}
      if(typeof startApp === 'function'){
        startApp(st.mode, {foldRestoreV327:true, foldStateV327:st});
        setTimeout(function(){ armAppBack('fold-restore-start'); }, 250);
        setTimeout(function(){ armAppBack('fold-restore-start-late'); }, 1000);
        return true;
      }
    }catch(_e){}
    return false;
  }
  window.oaiTryRestoreFoldSurfaceV327 = tryRestore;
  function schedule(reason){ setTimeout(function(){ tryRestore(reason); }, 120); setTimeout(function(){ tryRestore(reason+'-late'); }, 620); }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ schedule('dom'); }, {once:true});
  else schedule('dom-ready');
  ...
  window.addEventListener('pagehide', function(){ try{ record('pagehide'); markNoIntro(); }catch(_e){} }, {passive:true});
  document.addEventListener('visibilitychange', function(){
    try{
      if(document.visibilityState === 'hidden'){ record('hidden'); markNoIntro(); }
      else setTimeout(function(){ armAppBack('visible'); }, 180);
    }catch(_e){}
  }, {passive:true});
  window.addEventListener('resize', function(){ try{ record('resize'); markNoIntro(); }catch(_e){} }, {passive:true});
  try{ if(window.visualViewport) window.visualViewport.addEventListener('resize', function(){ try{ record('visualViewport-resize'); markNoIntro(); }catch(_e){} }, {passive:true}); }catch(_e){}
  setInterval(function(){ try{ record('heartbeat'); }catch(_e){} }, 1800);
})();
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-327: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V8-1-14-327: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.APP_VERSION || 'V8-1-14-327');
    var current = '';
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-327'));
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-327';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-327';
}
...
/* V8-1-14-327: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V8-1-14-327: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V8-1-14-327: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V8-1-14-327: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V8-1-14-327: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V8-1-14-327: 동작 없는 빈 포커스 리스너와 빈 지도 진입 훅 제거 */
...
/* V8-1-14-327: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V8-1-14-327';
...
const _PRAYER_ASSET_VERSION='V8-1-14-327';
...
const _RETREAT_ASSET_VERSION='V8-1-14-327';
...
// V8-1-14-327: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
const _SHRINE_ASSET_VERSION='V8-1-14-327';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-327: 성지/성당/피정 진입은 cover를 내리기 전 내위치 안내 화면을 먼저 열어 첫 화면으로 고정한다.
...
function oaiPreopenNearbySheetForCategory(){
  try{
    ...
    if(body){
      // V8-1-14-327: 데이터 지연 후 startApp이 다시 호출되어도 같은 내주변 로딩 화면을 다시 그리지 않는다.
      try{
...
function startApp(mode, opts){
  opts = opts || {};
  var oaiFoldRestoreV327 = !!(opts && opts.foldRestoreV327);
  var oaiFoldStateV327 = opts && opts.foldStateV327 ? opts.foldStateV327 : null;
  if(oaiFoldRestoreV327){
    try{ window.__OAI_FOLD_RESTORE_V327_UNTIL__ = (Date.now ? Date.now() : new Date().getTime()) + 9000; }catch(_e){}
    try{ window.__OAI_FOLD_STATE_V327__ = oaiFoldStateV327 || window.__OAI_FOLD_STATE_V327__ || null; }catch(_e){}
  }
  if(mode==='shrine' && (!_shrineRawLoaded || !SHRINES.length)){
    _mode='shrine';
    try{
      if(!oaiFoldRestoreV327){ try{ oaiPreopenNearbySheetForCategory(); }catch(_e){} }
      const cover=$('cover');
  ...
  _filterDio='all';
  _listSrch='';
  window._noAutoNearby = !!oaiFoldRestoreV327;  // 직접 진입 시 nearby 열기 허용, Fold 복귀 시 자동 내주변 금지
 ...
 _renderDioFilterBars();  _screen='map';
  if((mode==='shrine' || mode==='parish' || mode==='retreat') && !oaiFoldRestoreV327){
  ...
  setTimeout(function(){ try{ if(typeof window.oaiRecordFoldSurfaceV327 === 'function') window.oaiRecordFoldSurfaceV327('startApp:'+mode); }catch(_e){} }, 260);
}
...
function goToCover(){
  ...
  // V8-1-14-327: 커버 복귀 때 화면 높이/폰 화면 크기를 다시 계산하지 않는다.
}
...
function _onMapReady(){
 const KM=kakao.maps;window._LL=KM.LatLng;window._MM=KM.Marker;window._MI=KM.MarkerImage;window._SZ=KM.Size;window._PT=KM.Point;window._LB=KM.LatLngBounds;window._PL=KM.Polyline;window._EL=KM.event.addListener;
  ...
  renderList();
  var oaiFoldRestoreActiveV327 = !!(window.__OAI_FOLD_RESTORE_V327_UNTIL__ && (Date.now ? Date.now() : new Date().getTime()) < window.__OAI_FOLD_RESTORE_V327_UNTIL__);
  try{ if(oaiFoldRestoreActiveV327 && typeof window.oaiApplyFoldSurfaceV327 === 'function') window.oaiApplyFoldSurfaceV327('map-ready-before-auto'); }catch(_e){}
  if(!oaiFoldRestoreActiveV327) _autoLocate();
  if(_mode==='parish') { _buildParishDioSystem(); _syncParishDioLabels(); }
  else if(_mode==='retreat') _buildRetreatMarkers();
  if(!window._noAutoNearby && !oaiFoldRestoreActiveV327){
    openTab('nearby');
  }
  if(oaiFoldRestoreActiveV327){
    setTimeout(function(){ try{ if(typeof window.oaiApplyFoldSurfaceV327 === 'function') window.oaiApplyFoldSurfaceV327('map-ready'); }catch(_e){} }, 120);
  }
  window._noAutoNearby = false;
  try{ _updateShrineVisitMapFilterUI(); _updateShrineVisitCardsButtonUI(); }catch(e){ console.warn('[가톨릭길동무]', e); }
  if(typeof oaiHideCategoryEntryVeil==='function') setTimeout(oaiHideCategoryEntryVeil, 260);
...
function openTab(name, opts){
  ...
  if(name==='nearby'){
    try{ window.__OAI_NEARBY_PREOPEN_MODE__ = null; }catch(_e){}
    var oaiSkipNearbyLoad = !!(opts && (opts.skipNearbyLoad || opts.foldRestore));
    var oaiSuppressNearby = !!(window.__OAI_SUPPRESS_NEARBY_LOAD_UNTIL__ && (Date.now ? Date.now() : new Date().getTime()) < window.__OAI_SUPPRESS_NEARBY_LOAD_UNTIL__);
    if(!oaiSkipNearbyLoad && !oaiSuppressNearby) _loadNearby();
  }
  ...
  setTimeout(function(){ try{ if(typeof window.oaiRecordFoldSurfaceV327 === 'function') window.oaiRecordFoldSurfaceV327('openTab:'+name); }catch(_e){} }, 80);
}
...
function _autoLocate(){
  try{ if(window.__OAI_SKIP_AUTO_LOCATE_UNTIL__ && (Date.now ? Date.now() : new Date().getTime()) < window.__OAI_SKIP_AUTO_LOCATE_UNTIL__) return; }catch(_e){}
...
function _loadNearby(){
  try{ if(window.__OAI_SUPPRESS_NEARBY_LOAD_UNTIL__ && (Date.now ? Date.now() : new Date().getTime()) < window.__OAI_SUPPRESS_NEARBY_LOAD_UNTIL__) return; }catch(_e){}
  ...
  try{ _updateShrineVisitCardsButtonUI(); }catch(_e){}
  try{ _updateShrineNearbyLocationButtonUI(); }catch(_e){}
  // V8-1-14-327: 카테고리 진입 직후 이미 열린 로딩 화면은 유지하고, 빈 화면/이전 목록일 때만 로딩 화면을 그린다.
  try{
    if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
      body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
    }
...
function _loadNearbyWithDist(lat,lng,items,getIdx,getColor,getLabel){
  ...
  if(body && _isNearbyLoadCurrent(requestMode,requestToken,body)){
    // V8-1-14-327: 자동차 거리 계산 단계에서 같은 로딩 화면을 다시 렌더링하지 않는다.
    try{
      if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
        body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
      }
