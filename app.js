...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(marker){ marker.textContent = ''; marker.style.display = 'none'; marker.style.visibility = 'hidden'; marker.setAttribute('hidden','hidden'); }
  }catch(e){ console.warn('[가톨릭길동무]', e); }
}
...
const _PARISH_ASSET_VERSION='V1-3';
...
const _PRAYER_ASSET_VERSION='V1-3';
...
const _RETREAT_ASSET_VERSION='V1-3';
...
const _SHRINE_ASSET_VERSION='V1-3';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
document.addEventListener('DOMContentLoaded', function bindEvents() {
  ...
  (function(){
    ...
    on('cover-menu-guide-btn', 'click', function(e){
      if(e && e.preventDefault) e.preventDefault();
      closeMenu();
      try{ if(typeof openGuideManual === 'function') openGuideManual(); else if(window.openGuideManual) window.openGuideManual(); }catch(err){ console.warn('[가톨릭길동무]', err); }
    });
