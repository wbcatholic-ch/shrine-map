...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V2-148';
  }else if(!restore){
...
const _PARISH_ASSET_VERSION='V2-148';
...
const _PRAYER_ASSET_VERSION='V2-148';
...
const _SHRINE_ASSET_VERSION='V2-148';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
(function(){
  // V2-148: Android/WebView에서 키보드가 올라올 때 viewport 높이 축소를
...
(function(){
  ...
  function _isFoldWideMapEdgeBack(start, dx, dy, target){
    // V2-148: Fold 큰 화면 지도에서 시스템 back(popstate)와 JS edge swipe가
...
document.addEventListener('DOMContentLoaded', function bindEvents() {
  ...
  (function bindMyFaithLifePanel(){
    ...
    var confirmBtn = document.getElementById('my-faith-confirm-btn');
