...
function _ensureParishVisitBook(){
  ...
  document.body.appendChild(m);m.addEventListener('click',function(e){const t=e.target.closest&&e.target.closest('[data-pv-close],[data-pv-tab],[data-pv-card],[data-pv-more]');if(!t)return;if(t.hasAttribute('data-pv-close')){_closeParishVisitBook();return;}if(t.hasAttribute('data-pv-tab')){const tab=t.getAttribute('data-pv-tab');_parishVisitTab=tab==='stats'?'stats':tab==='unvisited'?'unvisited':'visited';_parishUnvisitedLimit=80;_renderParishVisitBook();return;}if(t.hasAttribute('data-pv-more')){_parishUnvisitedLimit+=80;_renderParishVisitBook();return;}const p=PARISHES[parseInt(t.getAttribute('data-pv-card'),10)];if(p)_openParishVisitDetail(p);});m.addEventListener('change',function(e){if(e.target.matches('[data-pv-sort]')){_parishVisitSort=e.target.value==='oldest'?'oldest':'recent';_renderParishVisitBook();}if(e.target.matches('[data-pv-dio]')){_parishVisitDio=e.target.value||'all';_parishUnvisitedLimit=80;_renderParishVisitBook();}});return m;
}
...
window.addEventListener('popstate',function(){
  if(_parishVisitBookOpen()) _closeParishVisitBook({fromPopstate:true});
