...
function markExternalReturnStabilize(kind){
  try{
    ...
    // V8-1-14-324: 외부 이동 전 보호창은 #oai-external-entry-guard로 유지하고, 숨김 복귀 때도 같은 보호창을 이어서 사용한다.
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
(function(){
  'use strict';
  if(window.__OAI_VISIBLE_SCREEN_RESTORE_V320_CLEANUP__) return;
  window.__OAI_VISIBLE_SCREEN_RESTORE_V320_CLEANUP__ = true;
  ...
  var KEY_VIEWPORT_NO_INTRO = 'oai_viewport_no_intro_until_v320';
  var KEY_VIEWPORT_WIDTH = 'oai_viewport_width_v320';
...
/* V8-1-14-324:
   V315 기준 유지 + Fold/viewport reload 때 현재 화면을 한 번만 되살린다.
   - V315의 cover/back-controller 흐름은 건드리지 않음
   - pagehide마다 거리계산 상태를 복원하지 않음
   - 내주변/route는 자동 거리계산 위험이 있어 Fold 복귀 때 자동 open 금지
   - 복귀 후 일반 app history root/trap 한 쌍만 다시 세워 Android Back이 배경으로 빠지지 않게 함 */
(function(){
  'use strict';
  if(window.__OAI_FOLD_SURFACE_KEEPER_V324__) return;
  window.__OAI_FOLD_SURFACE_KEEPER_V324__ = true;

  var KEY = 'oai_fold_surface_keep_v324';
  var TTL = 10 * 60 * 1000;
  var WIDTH_DELTA = 72;
  var GUARD_MS = 6500;
  var _running = false;
  ...
  function setBoth(st){
    try{ var raw=JSON.stringify(st||{}); sessionStorage.setItem(KEY, raw); localStorage.setItem(KEY, raw); }catch(_e){}
  }
  function getRaw(){
    var raw='';
    try{ raw=sessionStorage.getItem(KEY)||''; }catch(_e){}
    if(!raw){ try{ raw=localStorage.getItem(KEY)||''; }catch(_e){} }
    return raw;
  }
  function read(){
    try{
      var raw=getRaw(); if(!raw) return null;
  ...
  function normalize(surface, reason){
    surface = surface || {};
    if(typeof surface === 'string') surface = {type:surface};
    var type = String(surface.type||'').toLowerCase();
    if(!type || type==='cover') return null;
    var out = {type:type, ts:now(), w:width(), h:height(), reason:String(reason||surface.reason||'store')};
    if(surface.mode) out.mode = String(surface.mode);
    if(surface.tab) out.tab = String(surface.tab);
    if(surface.cat) out.cat = String(surface.cat);
    return out;
  }
  function store(surface, reason){
    try{
      if(window.__OAI_FOLD_SURFACE_RESTORING__) return;
      var st=normalize(surface, reason); if(!st) return;
      setBoth(st);
    }catch(_e){}
  }
  function viewportChanged(st){
    try{ var w=width(); return !!(st && st.w && w && Math.abs(w-Number(st.w))>=WIDTH_DELTA); }catch(_e){ return false; }
  }
  function markRestored(st, reason){
    try{
      st = st || read(); if(!st) return;
      st.w=width(); st.h=height(); st.ts=now(); st.restoredAt=now(); st.restoredReason=reason||'restored';
      setBoth(st);
    }catch(_e){}
  }
  ...
  function primeAppHistory(reason){
    try{
      if(!isAppVisible()) return false;
      var stamp = String(reason || 'fold-app');
      /* V8-1-14-324:
         Fold 복귀 후 Back은 해시 이동이 아니라 기존 back-controller가 이해하는
         일반 app root/trap 한 쌍으로만 보강한다. cover trap은 건드리지 않는다. */
      if(typeof window._oaiArmAppBackTrap === 'function'){
        return window._oaiArmAppBackTrap(stamp, {force:true}) !== false;
      }
      var href = location.href.split('#')[0];
      history.replaceState({_p:0, oai_app_root:stamp}, '', href);
      history.pushState({_p:1, oai_app_trap:stamp}, '', href);
      return true;
    }catch(e){ console.warn('[가톨릭길동무]', e); return false; }
  }
  function primeAppHistoryLater(reason){
    [60,180,420,900,1500].forEach(function(ms){
      setTimeout(function(){ primeAppHistory(reason+'-'+ms); }, ms);
    });
  }
  function safeTab(tab){
    /* V8-1-14-324:
       성지/성당/피정은 Fold 이동 후 '찾기 목록'으로 강제 이동하지 않는다.
       사용자가 보던 원래 화면을 우선 복귀한다.
       허용 탭: nearby/list/region/route, 그 외는 지도 기본 화면으로 둔다. */
    tab = String(tab || '').toLowerCase();
    if(tab === 'nearby' || tab === 'list' || tab === 'region' || tab === 'route') return tab;
    return '';
  }
  function openStoredSurface(st, reason){
    if(!st || _running) return false;
    _running = true;
    window.__OAI_FOLD_SURFACE_RESTORING__ = true;
    window.__OAI_FOLD_RESTORE_NO_AUTOLOCATE_UNTIL__ = now() + GUARD_MS;
    window.__OAI_FOLD_RESTORE_NO_NEARBY_UNTIL__ = now() + GUARD_MS;
    window.__OAI_FOLD_RESTORE_SAFE_TAB__ = safeTab(st.tab);
    markRestored(st, reason || 'restore');
    var opened=false;
    try{
      if(st.type === 'map' && typeof startApp === 'function'){
        startApp(st.mode || 'shrine', {foldRestore:true, safeTab:safeTab(st.tab), foldKeepOriginal:true});
        opened=true;
      }else if(st.type === 'trail' && typeof window.openTrailView === 'function'){
        window.openTrailView({restore:true, foldRestore:true}); opened=true;
      }else if(st.type === 'web' && typeof window.openWebView === 'function'){
        window.openWebView({restore:true, foldRestore:true}); opened=true;
      }else if(st.type === 'diocese' && typeof openDioceseView === 'function'){
        openDioceseView({restore:true, foldRestore:true}); opened=true;
      }else if(st.type === 'prayer' && typeof openPrayerBook === 'function'){
        openPrayerBook({restore:true, instant:true, foldRestore:true}); opened=true;
      }
    }catch(e){ console.warn('[가톨릭길동무] Fold surface restore failed', e); }
    if(opened) primeAppHistoryLater('fold-surface-restore');
    setTimeout(function(){
      window.__OAI_FOLD_SURFACE_RESTORING__ = false;
      window.__OAI_FOLD_RESTORE_SAFE_TAB__ = '';
      window.__OAI_FOLD_RESTORE_KEEP_ORIGINAL__ = false;
      _running = false;
    }, GUARD_MS);
    return opened;
  }
  function tryRestore(reason){
    try{
      ...
      return openStoredSurface(st, reason || 'try');
    }catch(e){ console.warn('[가톨릭길동무]', e); return false; }
  }
  function relayoutOnly(reason){
    try{ if(typeof _map !== 'undefined' && _map && typeof _map.relayout === 'function') _map.relayout(); }catch(_e){}
    try{ if(typeof window.relayoutTrailMap === 'function'){ window.relayoutTrailMap(60); window.relayoutTrailMap(260); } }catch(_e){}
  }
  function schedule(reason){
    [0,90,240,520,900,1400].forEach(function(ms){ setTimeout(function(){ tryRestore(reason+'-'+ms); }, ms); });
  }

  window.oaiStoreFoldSurface = store;
  window.oaiClearFoldSurface = clear;
  window.oaiTryFoldSurfaceRestore = tryRestore;
  window.oaiPrimeFoldAppHistory = primeAppHistoryLater;
  ...
  window.addEventListener('resize', function(){ relayoutOnly('resize'); schedule('resize'); }, {passive:true});
  try{ if(window.visualViewport) window.visualViewport.addEventListener('resize', function(){ relayoutOnly('visualViewport'); schedule('visualViewport'); }, {passive:true}); }catch(_e){}
})();
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-324: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V8-1-14-324: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.APP_VERSION || 'V8-1-14-324');
    var current = '';
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-324'));
...
function openPrayerBook(opts){
  try{ if(window.oaiStoreFoldSurface && !(opts && (opts.restore || opts.foldRestore)) && !(opts && opts.fromMassQuick)) window.oaiStoreFoldSurface({type:'prayer'}, 'openPrayerBook'); }catch(_e){}
...
function openDioceseView(opts){
  try{ if(window.oaiStoreFoldSurface && !(opts && (opts.restore || opts.foldRestore))) window.oaiStoreFoldSurface({type:'diocese'}, 'openDioceseView'); }catch(_e){}
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-324';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-324';
}
...
/* V8-1-14-324: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V8-1-14-324: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V8-1-14-324: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V8-1-14-324: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V8-1-14-324: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V8-1-14-324: 동작 없는 빈 포커스 리스너와 빈 지도 진입 훅 제거 */
...
/* V8-1-14-324: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V8-1-14-324';
...
const _PRAYER_ASSET_VERSION='V8-1-14-324';
...
const _RETREAT_ASSET_VERSION='V8-1-14-324';
...
// V8-1-14-324: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
const _SHRINE_ASSET_VERSION='V8-1-14-324';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-324: 성지/성당/피정 진입은 cover를 내리기 전 내위치 안내 화면을 먼저 열어 첫 화면으로 고정한다.
...
function oaiPreopenNearbySheetForCategory(){
  try{
    ...
    if(body){
      // V8-1-14-324: 데이터 지연 후 startApp이 다시 호출되어도 같은 내주변 로딩 화면을 다시 그리지 않는다.
      try{
...
function startApp(mode, opts){
  opts = opts || {};
  var __oaiFoldSurfaceRestore = !!(opts.foldRestore || window.__OAI_FOLD_SURFACE_RESTORING__);
  var __oaiFoldSafeTab = opts.safeTab || window.__OAI_FOLD_RESTORE_SAFE_TAB__ || '';
  var __oaiFoldKeepOriginal = !!(opts.foldKeepOriginal || window.__OAI_FOLD_RESTORE_KEEP_ORIGINAL__);
  if(__oaiFoldSurfaceRestore){
    try{
      var __oaiFoldGuardUntil = (Date.now ? Date.now() : new Date().getTime()) + 14000;
      window.__OAI_FOLD_SURFACE_RESTORING__ = true;
      window.__OAI_FOLD_RESTORE_NO_AUTOLOCATE_UNTIL__ = __oaiFoldGuardUntil;
      window.__OAI_FOLD_RESTORE_NO_NEARBY_UNTIL__ = __oaiFoldGuardUntil;
      window.__OAI_FOLD_RESTORE_SAFE_TAB__ = __oaiFoldSafeTab || '';
      window.__OAI_FOLD_RESTORE_KEEP_ORIGINAL__ = __oaiFoldKeepOriginal;
    }catch(_e){}
  }
  try{ if(window.oaiStoreFoldSurface && !__oaiFoldSurfaceRestore) window.oaiStoreFoldSurface({type:'map', mode:mode, tab:_activeTab||''}, 'startApp'); }catch(_e){}
  if(mode==='shrine' && (!_shrineRawLoaded || !SHRINES.length)){
    _mode='shrine';
    try{
      if(!__oaiFoldSurfaceRestore){ try{ oaiPreopenNearbySheetForCategory(); }catch(_e){} }
      const cover=$('cover');
    ...
    _ensureShrineDataLoaded().then(function(){ startApp('shrine', opts); }).catch(function(err){
  ...
  if(mode==='retreat' && (!_retreatRawLoaded || !RETREATS.length)){
    ...
    _ensureRetreatDataLoaded().then(function(){ startApp('retreat', opts); }).catch(function(err){
  ...
  if(mode==='parish' && !_PARISH_SPLIT_LAZY_MODE && (!_parishRawLoaded || !PARISHES.length)){
    ...
    _ensureParishDataLoaded().then(function(){ startApp('parish', opts); }).catch(function(err){
  ...
  _filterDio='all';
  _listSrch='';
  window._noAutoNearby = !!__oaiFoldSurfaceRestore;  // 직접 진입은 nearby 허용, Fold 복귀는 자동 nearby 금지
 ...
 _renderDioFilterBars();  _screen='map';
  if(mode==='shrine' || mode==='parish' || mode==='retreat'){
    if(_carryNearbyPreopen){
      ...
      if(!__oaiFoldSurfaceRestore){ try{ oaiPreopenNearbySheetForCategory(); }catch(e){ console.warn('[가톨릭길동무]', e); } }
    }
  }
  ...
  if(__oaiFoldSurfaceRestore){
    try{
      if(typeof window.oaiPrimeFoldAppHistory === 'function') window.oaiPrimeFoldAppHistory('startApp-fold-ready');
    }catch(_e){}
  }
  if(mode==='shrine'){
...
function goToCover(){
  ...
  _screen='cover';
  try{ if(window.oaiClearFoldSurface) window.oaiClearFoldSurface('goToCover'); }catch(_e){}
  ...
  // V8-1-14-324: 커버 복귀 때 화면 높이/폰 화면 크기를 다시 계산하지 않는다.
}
...
function _onMapReady(){
 const KM=kakao.maps;window._LL=KM.LatLng;window._MM=KM.Marker;window._MI=KM.MarkerImage;window._SZ=KM.Size;window._PT=KM.Point;window._LB=KM.LatLngBounds;window._PL=KM.Polyline;window._EL=KM.event.addListener;
  ...
  renderList();
  var __oaiSkipFoldAutoLocate=false;
  try{ __oaiSkipFoldAutoLocate = !!(window.__OAI_FOLD_SURFACE_RESTORING__ || (window.__OAI_FOLD_RESTORE_NO_AUTOLOCATE_UNTIL__ && Date.now() < Number(window.__OAI_FOLD_RESTORE_NO_AUTOLOCATE_UNTIL__))); }catch(_e){}
  if(!__oaiSkipFoldAutoLocate) _autoLocate();
  ...
  if(!window._noAutoNearby){
    ...
    try{
      var __safeTab = window.__OAI_FOLD_RESTORE_SAFE_TAB__ || '';
      var __allowSafeTab = (__safeTab === 'nearby' || __safeTab === 'list' || __safeTab === 'region' || __safeTab === 'route');
      if(__allowSafeTab){
        setTimeout(function(){ try{ openTab(__safeTab); }catch(_e){} }, 80);
        setTimeout(function(){ try{ if(_activeTab !== __safeTab) openTab(__safeTab); }catch(_e){} }, 320);
        if(__safeTab === 'nearby'){
          /* V8-1-14-324: 내주변을 보던 중 Fold 복귀하면 지도만 보이지 않게
             시트는 원래대로 열되, guard 시간 동안 거리계산 재시작은 막는다. */
          setTimeout(function(){
            try{
              var body=$('nearby-body');
              if(body && !body.children.length){
                body.innerHTML='<div class="empty-msg">Fold 화면 전환 후 내주변 화면으로 돌아왔습니다.<br>필요하면 내 위치 버튼으로 다시 계산해 주세요.</div>';
              }
            }catch(_e){}
          }, 420);
        }
...
function openTab(name, opts){
  ...
  _activeTab=name;
  try{
    if(window.oaiStoreFoldSurface && !window.__OAI_FOLD_SURFACE_RESTORING__ && (_mode==='shrine'||_mode==='parish'||_mode==='retreat')){
      window.oaiStoreFoldSurface({type:'map', mode:_mode, tab:name}, 'openTab:'+name);
    }
  }catch(_e){}
  try{ _updateShrineNearbyLocationButtonUI(); }catch(_e){}
  ...
  if(name==='nearby'){
    try{ window.__OAI_NEARBY_PREOPEN_MODE__ = null; }catch(_e){}
    var __oaiNoNearbyCalc=false;
    try{ __oaiNoNearbyCalc = !!(window.__OAI_FOLD_SURFACE_RESTORING__ || (window.__OAI_FOLD_RESTORE_NO_NEARBY_UNTIL__ && Date.now() < Number(window.__OAI_FOLD_RESTORE_NO_NEARBY_UNTIL__))); }catch(_e){}
    if(!__oaiNoNearbyCalc) _loadNearby();
  }
...
function _loadNearby(){
  ...
  if(!body) return;
  try{
    var __oaiBlockNearby = !!(window.__OAI_FOLD_SURFACE_RESTORING__ || (window.__OAI_FOLD_RESTORE_NO_NEARBY_UNTIL__ && Date.now() < Number(window.__OAI_FOLD_RESTORE_NO_NEARBY_UNTIL__)));
    if(__oaiBlockNearby){
      _cancelNearbyLoad();
      if(_mode==='shrine'){
  ...
  try{ _updateShrineVisitCardsButtonUI(); }catch(_e){}
  try{ _updateShrineNearbyLocationButtonUI(); }catch(_e){}
  // V8-1-14-324: 카테고리 진입 직후 이미 열린 로딩 화면은 유지하고, 빈 화면/이전 목록일 때만 로딩 화면을 그린다.
  try{
    if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
      body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
    }
...
function _loadNearbyWithDist(lat,lng,items,getIdx,getColor,getLabel){
  ...
  if(body && _isNearbyLoadCurrent(requestMode,requestToken,body)){
    // V8-1-14-324: 자동차 거리 계산 단계에서 같은 로딩 화면을 다시 렌더링하지 않는다.
    try{
      if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
        body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
      }
