...
function _runRefreshAppFilesOnly(){
  ...
  try{
    if(btn){
      btn.disabled = true;
      btn.textContent = ((btn.getAttribute('data-target-version') || 'V1-47') + ' 새로고침 중');
    }
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
        var target = btn.getAttribute('data-target-version') || 'V1-47';
    var current = '';
    ...
    if(marker){
      marker.textContent = target || 'V1-47';
...
(function(){
  ...
  function closeGuideManual(){
    ...
    try{
      ...
      if(typeof _resetCoverBackTrap === 'function') _resetCoverBackTrap('guide-ok-as-x');
      else if(typeof _ensureCoverBackTrap === 'function') _ensureCoverBackTrap('guide-ok-as-x');
    }catch(e){ console.warn('[가톨릭길동무]', e); }
  }
  ...
  function shouldShowIntro(forceRefresh){
    ...
    try{
      ...
      // Google Play/TWA 앱에서 처음 열린 경우에만 1회 표시한다.
      if(isKakaoBrowser()) return false;
      if(!isStandaloneApp()) return false;
...
// Google Play Android 정리본: 모바일 브라우저 안내 로직 제거
  ...
  function bindGuide(){
    ...
    if(ok) ok.setAttribute('data-guide-close','manual');
var favOk=document.getElementById('favorites-reset-notice-ok');
    ...
    // V1-47 cover menu popstate close
    window.addEventListener('popstate', function(){
      if(modal.classList.contains('show')){
        closeMenu();
        try{ if(typeof window.markCoverMenuBackConsumed === 'function') window.markCoverMenuBackConsumed('cover-menu-popstate'); }catch(e){}
      }
    });
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V1-47';
  }else if(!restore){
...
const _PARISH_ASSET_VERSION='V1-47';
...
const _PRAYER_ASSET_VERSION='V1-47';
...
const _RETREAT_ASSET_VERSION='V1-47';
...
const _SHRINE_ASSET_VERSION='V1-47';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
/* V1-47: 개인정보/문의건의에서 index로 돌아온 뒤 커버 trap을 새로 시작 */
(function(){
  if(window.__OAI_EXTERNAL_MENU_RETURN_TRAP_FIX__) return;
  window.__OAI_EXTERNAL_MENU_RETURN_TRAP_FIX__ = true;

  function markExternalMenuReturn(kind){
    try{ sessionStorage.setItem('oai_external_menu_return_needs_fresh_trap', kind || 'menu-page'); }catch(e){}
  }

  function peekExternalMenuReturn(){
    try{
      return sessionStorage.getItem('oai_external_menu_return_needs_fresh_trap') ||
             localStorage.getItem('oai_external_menu_return_needs_fresh_trap') || '';
    }catch(e){
      try{ return localStorage.getItem('oai_external_menu_return_needs_fresh_trap') || ''; }catch(_e){ return ''; }
    }
  }

  function consumeExternalMenuReturn(){
    var v = '';
    try{ v = sessionStorage.getItem('oai_external_menu_return_needs_fresh_trap') || ''; }catch(e){}
    try{ if(!v) v = localStorage.getItem('oai_external_menu_return_needs_fresh_trap') || ''; }catch(e){}
    try{ sessionStorage.removeItem('oai_external_menu_return_needs_fresh_trap'); }catch(e){}
    try{ localStorage.removeItem('oai_external_menu_return_needs_fresh_trap'); }catch(e){}
    return v;
  }

  function coverIsVisibleNow(){
    try{
      if(typeof _isCoverScreenVisible === 'function') return _isCoverScreenVisible();
      var cover = document.getElementById('cover');
      if(!cover) return !document.documentElement.classList.contains('app-active');
      if(cover.classList.contains('hidden')) return false;
      var st = window.getComputedStyle ? window.getComputedStyle(cover) : null;
  ...
  function forceFreshCoverTrap(reason){
    try{ if(typeof _resetCoverExitReady === 'function') _resetCoverExitReady(); }catch(e){ console.warn('[가톨릭길동무]', e); }
    try{ if(typeof _clearCoverExitArmed === 'function') _clearCoverExitArmed(); }catch(e){ console.warn('[가톨릭길동무]', e); }
    try{
      var href = location.href.split('#')[0];
      history.replaceState({_p:0, oai_cover_root:reason || 'external-menu-return'}, '', href);
      history.pushState({_p:1, oai_cover_trap:reason || 'external-menu-return'}, '', href);
    }catch(e){ console.warn('[가톨릭길동무]', e); }
  }

  function handleExternalMenuReturn(reason){
    var kind = peekExternalMenuReturn();
    if(!kind) return false;

    /* V1-47: 개인정보 복귀는 커버 표시 판정에 막히면 안 된다.
       개인정보는 index 바깥의 별도 페이지이므로, 돌아온 뒤에는 커버 종료 trap이 반드시 새로 시작되어야 한다. */
    if(!coverIsVisibleNow()){
      if(kind === 'privacy'){
        try{
          if(typeof goToCover === 'function') goToCover();
          else document.documentElement.classList.remove('app-active','parish-mode','retreat-mode');
        }catch(e){ console.warn('[가톨릭길동무]', e); }
      }else{
        return false;
      }
    ...
    consumeExternalMenuReturn();
    forceFreshCoverTrap((reason || 'external-menu-return') + '-' + kind);
    return true;
  }

  try{ window.oaiMarkExternalMenuReturn = markExternalMenuReturn; }catch(e){}

  window.addEventListener('pageshow', function(){ setTimeout(function(){ handleExternalMenuReturn('pageshow'); }, 0); }, true);
  window.addEventListener('focus', function(){ setTimeout(function(){ handleExternalMenuReturn('focus'); }, 0); }, true);
  document.addEventListener('visibilitychange', function(){
    if(document.visibilityState === 'visible') setTimeout(function(){ handleExternalMenuReturn('visible'); }, 0);
  }, true);
  setTimeout(function(){ handleExternalMenuReturn('load-0'); }, 0);
  setTimeout(function(){ handleExternalMenuReturn('load-80'); }, 80);
  setTimeout(function(){ handleExternalMenuReturn('load-220'); }, 220);
})();
...
document.addEventListener('DOMContentLoaded', function bindEvents() {
  ...
  (function(){
    ...
    function markCoverMenuBackConsumed(reason){
      try{
        window.__oaiCoverMenuBackConsumedUntil = Date.now() + 900;
        window.__oaiCoverMenuBackConsumedReason = reason || 'cover-menu';
        if(typeof _resetCoverExitReady === 'function') _resetCoverExitReady();
        if(typeof _clearCoverExitArmed === 'function') _clearCoverExitArmed();
      }catch(e){ console.warn('[가톨릭길동무]', e); }
    }
    window.markCoverMenuBackConsumed = markCoverMenuBackConsumed;
    ...
    on('cover-menu-qna-btn', 'click', function(e){
      if(e && e.preventDefault) e.preventDefault();
      try{ if(typeof window.oaiMarkExternalMenuReturn === 'function') window.oaiMarkExternalMenuReturn('qna'); }catch(_e){}
      closeMenu();
...
// V1-47 cover menu hardware back guard
(function(){
  window.addEventListener('popstate', function(){
    try{
      if(window.isCoverMenuPopupOpen && window.isCoverMenuPopupOpen()){
        if(window.closeCoverMenuPopup) window.closeCoverMenuPopup();
        if(typeof window.markCoverMenuBackConsumed === 'function') window.markCoverMenuBackConsumed('cover-menu-capture-popstate');
      }
    }catch(e){ console.warn('[가톨릭길동무]', e); }
  }, true);
})();
