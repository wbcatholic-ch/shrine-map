...
function markExternalReturnStabilize(kind){
  try{
    ...
    // V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full: 외부 이동 전 보호창은 #oai-external-entry-guard로 유지하고, 숨김 복귀 때도 같은 보호창을 이어서 사용한다.
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
(function(){
  ...
  var _bgReturnReadyRetry = 0;
  var _bgReturnWatchdogTimer = 0;
  ...
  function scheduleBackgroundReturn(reason){
    try{
      ...
      /* V8-1-14-221: 10분 미만의 짧은 백그라운드 복귀는 화면을 절대 덮거나 freeze하지 않는다.
         기존에는 짧은 복귀에도 resume-freeze를 걸어 Android WebView 첫 페인트가 흰색으로 스치는 경우가 있었다.
         긴 복귀만 1회 지연 판단하고, 짧은 복귀는 기록 정리만 빠르게 실행한다. */
      if(!started || elapsed < MEDIUM_BG_RETURN_MS){
        _bgReturnTimer = setTimeout(function(){
          try{ checkBackgroundReturn(r + '-short-no-freeze'); }catch(_e){ console.warn('[가톨릭길동무]', _e); }
        }, 80);
        return;
      }
      /* 10분 이상 복귀는 visible/pageshow/focus가 연속으로 들어와도 한 번만 판단한다.
         너무 이른 첫 이벤트가 BG_KEY를 지워 원래 화면 복귀가 누락되는 문제를 막는다. */
      armResumeFreeze(r, RESUME_FREEZE_MIN_MS);
      _bgReturnTimer = setTimeout(function(){
        try{ checkBackgroundReturn(r); }catch(_e){ console.warn('[가톨릭길동무]', _e); }
      }, 320);
  ...
  function clearBackgroundReturnWatchdog(){
    try{ clearTimeout(_bgReturnWatchdogTimer); _bgReturnWatchdogTimer = 0; }catch(_e){}
  }
  function armBackgroundReturnWatchdog(reason, ms){
    try{
      clearBackgroundReturnWatchdog();
      _bgReturnWatchdogTimer = setTimeout(function(){
        try{
          var veil = document.getElementById('oai-background-return-photo-veil');
          if(veil && veil.parentNode) veil.parentNode.removeChild(veil);
          document.documentElement.classList.remove('oai-background-return-intro','oai-cover-booting','oai-cover-revealing','oai-cover-under-intro-reveal');
          document.documentElement.removeAttribute('data-oai-background-return-reason');
          sessionStorage.removeItem('oai_background_intro_return_until');
        }catch(_e){}
        _bgIntroRunning = false;
        clearReturnCoordinatorLock();
        settleCurrentScreen(reason || 'background-return-watchdog');
      }, Math.max(2200, ms || 5200));
    }catch(e){ console.warn('[가톨릭길동무]', e); }
  }

  function ensureBackgroundReturnPhotoVeil(){
    try{
      ...
      if(!document.getElementById('oai-background-return-photo-veil-style')){
        ...
        st.textContent = '\n'
          + '#oai-background-return-photo-veil{position:fixed;inset:0;z-index:2147482498;background:#fbf8f1;opacity:0;transition:opacity .34s ease;pointer-events:none;display:block;visibility:visible;contain:layout paint style;}\n'
          + '#oai-background-return-photo-veil.oai-show{opacity:1;}\n'
          + '#oai-background-return-photo-veil .oai-bg-photo{position:absolute;left:50%;top:50%;width:min(84vw,540px);height:min(84vh,740px);transform:translate(-50%,-50%) scale(.84);background-image:linear-gradient(to right,#fbf8f1 0%,rgba(251,248,241,.96) 5%,rgba(251,248,241,.60) 13%,rgba(251,248,241,.16) 24%,rgba(251,248,241,0) 36%,rgba(251,248,241,0) 64%,rgba(251,248,241,.16) 76%,rgba(251,248,241,.60) 87%,rgba(251,248,241,.96) 95%,#fbf8f1 100%),linear-gradient(to bottom,#fbf8f1 0%,rgba(251,248,241,.90) 6%,rgba(251,248,241,.36) 17%,rgba(251,248,241,0) 30%,rgba(251,248,241,0) 70%,rgba(251,248,241,.36) 83%,rgba(251,248,241,.90) 94%,#fbf8f1 100%),radial-gradient(ellipse 96% 88% at 50% 48%,rgba(251,248,241,0) 50%,rgba(251,248,241,.08) 62%,rgba(251,248,241,.34) 76%,rgba(251,248,241,.76) 90%,rgba(251,248,241,1) 100%),url("intro-cross-jesus.jpg?v=V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full");background-position:center center,center center,center center,center center;background-size:100% 100%,100% 100%,100% 100%,auto 106%;background-repeat:no-repeat,no-repeat,no-repeat,no-repeat;border-radius:38px;background-color:#fbf8f1;box-shadow:0 0 0 999px #fbf8f1,0 8px 18px rgba(80,54,18,.05),inset 0 0 72px rgba(251,248,241,.28),inset 0 0 150px rgba(251,248,241,.18);will-change:transform,opacity;backface-visibility:hidden;animation:oaiIntroSmoothPhotoDesktopV85 1.85s cubic-bezier(.22,.61,.36,1) both;}\n'
  ...
  function showMediumBackgroundReturnOverlay(reason){
    if(_bgIntroRunning) return;
    _bgIntroRunning = true;
    var finished = false;
    function finishMedium(veil){
      if(finished) return;
      finished = true;
      clearBackgroundReturnWatchdog();
    ...
    try{
      ...
      clearHomeReturnSoftVeil(true);
      clearRecentResumePatchClasses();
      armResumeFreeze(reason || 'medium-background-return', 3900);
      setReturnCoordinatorLock(reason || 'medium-background-return', 3900);
      armBackgroundReturnWatchdog(reason || 'medium-background-return', 5600);
      try{ sessionStorage.setItem('oai_background_intro_return_until', String(now() + 4200)); }catch(_e){}
      var veil = ensureBackgroundReturnPhotoVeil();
      if(veil){
      ...
      setTimeout(function(){
        ...
        setTimeout(function(){ finishMedium(veil); }, 390);
      }, 2050);
    }catch(e){
      console.warn('[가톨릭길동무]', e);
      finishMedium(document.getElementById('oai-background-return-photo-veil'));
    }
  }

  function showBackgroundReturnIntro(reason, toCover){
    if(!toCover){ showMediumBackgroundReturnOverlay(reason || 'medium-background-return'); return; }
    if(_bgIntroRunning) return;
    _bgIntroRunning = true;
    var root = document.documentElement;
    var finished = false;
    function finishLong(){
      ...
      clearBackgroundReturnWatchdog();
      try{
        root.classList.remove('oai-cover-revealing','oai-cover-under-intro-reveal','oai-background-return-intro','oai-cover-booting');
    ...
    try{
      ...
      armBackgroundReturnWatchdog(reason || 'background-return-intro', 6800);
      ...
      setTimeout(function(){
        ...
        setTimeout(function(){ finishLong(); }, 1150);
      }, 2200);
    }catch(e){
      console.warn('[가톨릭길동무]', e);
      finishLong();
  ...
  function checkBackgroundReturn(reason){
    try{
      ...
      if(!isReturnableScreenActive()){
        clearHomeReturnSoftVeil(true);
        if(elapsed >= MEDIUM_BG_RETURN_MS && _bgReturnReadyRetry < 5){
          _bgReturnReadyRetry += 1;
          clearTimeout(_bgReturnTimer);
          _bgReturnTimer = setTimeout(function(){
            try{ checkBackgroundReturn((reason || 'return') + '-ready-retry'); }catch(_e){ console.warn('[가톨릭길동무]', _e); }
          }, 260);
          return;
        }
        _bgReturnReadyRetry = 0;
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.APP_VERSION || 'V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full');
    var current = '';
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full'));
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full';
}
...
const _PRAYER_ASSET_VERSION='V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full';
...
// V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full: 성지/성당/피정 진입은 cover를 내리기 전 내위치 안내 화면을 먼저 열어 첫 화면으로 고정한다.
...
function oaiPreopenNearbySheetForCategory(){
  try{
    ...
    if(body){
      // V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full: 데이터 지연 후 startApp이 다시 호출되어도 같은 내주변 로딩 화면을 다시 그리지 않는다.
      try{
...
function goToCover(){
  ...
  // V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full: 커버 복귀 때 화면 높이/폰 화면 크기를 다시 계산하지 않는다.
}
...
function _loadNearby(){
  ...
  try{ _updateShrineVisitCardsButtonUI(); }catch(_e){}
  try{ _updateShrineNearbyLocationButtonUI(); }catch(_e){}
  // V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full: 카테고리 진입 직후 이미 열린 로딩 화면은 유지하고, 빈 화면/이전 목록일 때만 로딩 화면을 그린다.
  try{
    if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
      body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
    }
...
function _loadNearbyWithDist(lat,lng,items,getIdx,getColor,getLabel){
  ...
  if(body && _isNearbyLoadCurrent(requestMode,requestToken,body)){
    // V8-1-14-222_sheet_swipe_no_bottom_flash_map_stable_full: 자동차 거리 계산 단계에서 같은 로딩 화면을 다시 렌더링하지 않는다.
    try{
      if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
        body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
      }
