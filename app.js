...
function oaiShowExternalEntryGuard(kind){
  // V8-1-14-167-SHRINE-BANNER-ON-COVER:
...
(function(){
  ...
  /* V8-1-14-167-SHRINE-BANNER-ON-COVER: 미사용 후 복귀는 예전 WebView 방식으로 단순화한다.
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-167-SHRINE-BANNER-ON-COVER: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-167-SHRINE-BANNER-ON-COVER'));
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V8-1-14-167-SHRINE-BANNER-ON-COVER';
...
const _PRAYER_ASSET_VERSION='V8-1-14-167-SHRINE-BANNER-ON-COVER';
...
// V8-1-14-167-SHRINE-BANNER-ON-COVER: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-167-SHRINE-BANNER-ON-COVER: 성지/성당/피정 진입은 cover를 내리기 전 내위치 안내 화면을 먼저 열어 첫 화면으로 고정한다.
...
function goToCover(){
  ...
  // V8-1-14-167-SHRINE-BANNER-ON-COVER: 커버 복귀 때 화면 높이/폰 화면 크기를 다시 계산하지 않는다.
}
