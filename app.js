...
function _runRefreshAppFilesOnly(){
  ...
  try{
    if(btn){
      btn.disabled = true;
      btn.textContent = ((btn.getAttribute('data-target-version') || 'V1-25') + ' 새로고침 중');
    }
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
        var target = btn.getAttribute('data-target-version') || 'V1-25';
    var current = '';
    ...
    if(marker){
      marker.textContent = target || 'V1-25';
...
(function(){
  ...
  function openGuideManual(){
    ...
    try{ sessionStorage.setItem('oai_opening_guide_page','1'); }catch(e){}
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V1-25';
  }else if(!restore){
...
const _PARISH_ASSET_VERSION='V1-25';
...
const _PRAYER_ASSET_VERSION='V1-25';
...
const _RETREAT_ASSET_VERSION='V1-25';
...
const _SHRINE_ASSET_VERSION='V1-25';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
document.addEventListener('DOMContentLoaded', function bindEvents() {
  ...
  (function(){
    ...
    window.markCoverMenuBackConsumed = function(){
      try{ window.__oaiCoverMenuBackConsumedUntil = Date.now() + 900; }catch(_e){}
    };
    ...
    on('cover-menu-guide-link', 'click', function(){ closeMenu(); });
