...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.OAI_APP_BUILD_VERSION || window.APP_VERSION || 'V8-1-14-641');
    var current = '';
    /* V8-1-14-621:
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-641'));
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-641';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-641';
}
...
const _PARISH_ASSET_VERSION='V8-1-14-641';
...
const _PRAYER_ASSET_VERSION='V8-1-14-641';
...
const _RETREAT_ASSET_VERSION='V8-1-14-641';
...
const _SHRINE_ASSET_VERSION='V8-1-14-641';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function _showParishNearbyMarkersOnMap(items, lat, lng, phase){
  ...
  try{
    ...
    // V8-1-14-641: 첫 진입에서는 가장 가까운 성당을 자동 선택하지 않는다.
...
function renderList(){
  ...
  items.forEach((s,i)=>{
    ...
    const matchDio = (_filterDio==='all'||s.diocese===_filterDio);
    if(!matchDio) return;
  ...
  if(Object.keys(groups).length===0){
    ...
    else if(q && _filterDio!=='all') body.innerHTML='<div class="empty-msg">선택한 교구 안에 검색 결과가 없습니다</div>';
...
function resetRoute(opts){
  ...
  _hide($('rs-result'));
  _setRouteResultTipVisible(false);
  // V8-1-14-641: 다시선택에서는 경로선과 결과판을 먼저 모두 닫은 뒤 경유지 UI를 한 번만 동기화한다.
