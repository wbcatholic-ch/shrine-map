...
function markExternalReturnStabilize(kind){
  try{
    ...
    // V8-1-14-318: 외부 이동 전 보호창은 #oai-external-entry-guard로 유지하고, 숨김 복귀 때도 같은 보호창을 이어서 사용한다.
  }catch(e){ console.warn("[가톨릭길동무]", e); }
}
...
(function(){
  'use strict';
  if(window.__OAI_VISIBLE_SCREEN_RESTORE_V318_CLEANUP__) return;
  window.__OAI_VISIBLE_SCREEN_RESTORE_V318_CLEANUP__ = true;
  ...
  var KEY_VIEWPORT_NO_INTRO = 'oai_viewport_no_intro_until_v318';
  var KEY_VIEWPORT_WIDTH = 'oai_viewport_width_v318';
...
/* V8-1-14-318:
   V315 안정 흐름 보존 + Fold/viewport reload 때 현재 카테고리 1회 유지.
   - pagehide마다 화면을 복원 대상으로 저장하지 않음
   - 사용자가 실제로 카테고리에 들어간 순간의 surface만 저장
   - 부팅 시 폭 변화가 확인될 때만 1회 복귀
   - cover/back trap은 절대 건드리지 않음 */
(function(){
  'use strict';
  if(window.__OAI_FOLD_SURFACE_KEEPER_V318__) return;
  window.__OAI_FOLD_SURFACE_KEEPER_V318__ = true;

  var KEY = 'oai_fold_surface_keep_v318';
  var TTL = 6 * 60 * 1000;
  ...
  function normalizeSurface(surface){
    ...
    var out={type:type, ts:now(), w:width(), h:height(), reason:String(surface.reason||'')};
    if(surface.mode) out.mode=String(surface.mode);
    return out;
  }
  function store(surface, reason){
    try{
      var st=normalizeSurface(surface);
      if(!st) return;
      st.reason = reason || st.reason || 'store';
      setBoth(st);
    }catch(_e){}
  }
  function viewportChanged(st){
    try{
      ...
      return Math.abs(w-Number(st.w)) >= 72;
    }catch(_e){ return false; }
  }
  function markRestored(st, reason){
    try{
      ...
      st.ts = now();
      st.restoredAt = now();
      st.restoredReason = reason || 'restored';
      setBoth(st);
    }catch(_e){}
  }
  function openStoredSurface(st, reason){
    ...
    setTimeout(function(){ window.__OAI_FOLD_SURFACE_RESTORING__ = false; _running=false; }, RESTORE_GUARD_MS);
    return opened;
  }
  ...
  function schedule(reason){
    [0,80,240,520,960,1500].forEach(function(ms){ setTimeout(function(){ tryRestore(reason + '-' + ms); }, ms); });
  }
...
function _renderShrineVisitDetail(idx){
  ...
  // V8-1-14-318: V117 기준에서 V118 성지정보 카드 색상과 V123 굿뉴스 파란색을 실제 생성 버튼에 직접 적용한다.
...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V8-1-14-318: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.APP_VERSION || 'V8-1-14-318');
    var current = '';
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-318'));
...
function openPrayerBook(opts){
  try{ if(window.oaiStoreFoldSurface && !(opts && opts.restore) && !(opts && opts.fromMassQuick)) window.oaiStoreFoldSurface({type:'prayer'}, 'openPrayerBook'); }catch(_e){}
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  try{ if(window.oaiStoreFoldSurface && !restore) window.oaiStoreFoldSurface({type:'diocese'}, 'openDioceseView'); }catch(_e){}
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-318';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-318';
}
...
/* V8-1-14-318: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V8-1-14-318: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V8-1-14-318: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V8-1-14-318: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V8-1-14-318: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V8-1-14-318: 동작 없는 빈 포커스 리스너와 빈 지도 진입 훅 제거 */
...
/* V8-1-14-318: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V8-1-14-318';
...
const _PRAYER_ASSET_VERSION='V8-1-14-318';
...
const _RETREAT_ASSET_VERSION='V8-1-14-318';
...
// V8-1-14-318: 오래 미사용 후 복귀 시 마지막 위치를 먼저 보여주고, 새 GPS가 잡히면 최신 위치로 교체한다.
...
const _SHRINE_ASSET_VERSION='V8-1-14-318';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
...
function oaiShowCategoryEntryVeil(mode){
  // V8-1-14-318: 성지/성당/피정 진입은 cover를 내리기 전 내위치 안내 화면을 먼저 열어 첫 화면으로 고정한다.
...
function oaiPreopenNearbySheetForCategory(){
  try{
    ...
    if(body){
      // V8-1-14-318: 데이터 지연 후 startApp이 다시 호출되어도 같은 내주변 로딩 화면을 다시 그리지 않는다.
      try{
...
function startApp(mode){
  var __oaiFoldSurfaceRestore = !!window.__OAI_FOLD_SURFACE_RESTORING__;
  try{ if(window.oaiStoreFoldSurface && !__oaiFoldSurfaceRestore) window.oaiStoreFoldSurface({type:'map', mode:mode}, 'startApp'); }catch(_e){}
  ...
  _filterDio='all';
  _listSrch='';
  window._noAutoNearby = !!__oaiFoldSurfaceRestore;  // 직접 진입은 nearby 허용, Fold surface 복귀는 자동 nearby 금지
...
function goToCover(){
  ...
  // V8-1-14-318: 커버 복귀 때 화면 높이/폰 화면 크기를 다시 계산하지 않는다.
}
...
function _onMapReady(){
 const KM=kakao.maps;window._LL=KM.LatLng;window._MM=KM.Marker;window._MI=KM.MarkerImage;window._SZ=KM.Size;window._PT=KM.Point;window._LB=KM.LatLngBounds;window._PL=KM.Polyline;window._EL=KM.event.addListener;
  ...
  try{ __oaiSkipFoldAutoLocate = !!(window.__OAI_FOLD_SURFACE_RESTORING__ || (window.__OAI_FOLD_KEEP_SKIP_AUTOLOCATE_UNTIL && Date.now() < Number(window.__OAI_FOLD_KEEP_SKIP_AUTOLOCATE_UNTIL))); }catch(_e){}
...
function _loadNearby(){
  ...
  try{ _updateShrineVisitCardsButtonUI(); }catch(_e){}
  try{ _updateShrineNearbyLocationButtonUI(); }catch(_e){}
  // V8-1-14-318: 카테고리 진입 직후 이미 열린 로딩 화면은 유지하고, 빈 화면/이전 목록일 때만 로딩 화면을 그린다.
  try{
    if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
      body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
    }
...
function _loadNearbyWithDist(lat,lng,items,getIdx,getColor,getLabel){
  ...
  if(body && _isNearbyLoadCurrent(requestMode,requestToken,body)){
    // V8-1-14-318: 자동차 거리 계산 단계에서 같은 로딩 화면을 다시 렌더링하지 않는다.
    try{
      if(!body.querySelector || !body.querySelector('.nearby-distance-loading')){
        body.innerHTML='<div class="empty-msg nearby-distance-loading"><span class="nearby-distance-spinner" aria-hidden="true"></span><span class="nearby-distance-title">📍 내 위치 기준 자동차 거리 계산 중</span><span class="nearby-distance-sub">위치 확인 후 가까운 장소를 준비합니다.</span></div>';
      }
