...
function _ensureShrineVisitCardsModal(){
  ...
  modal.addEventListener('click', function(e){
    ...
    if(tab){
      ...
      setTimeout(_scrollShrineVisitCardsBodyTop, 0);
      return;
    }
...
function _renderShrineVisitModalList(item){
  ...
  list.innerHTML='<div class="shrine-visit-list-title">방문 날짜 '+visits.length+'회</div>'+visits.map(function(v,i){ return '<div class="shrine-visit-date-row"><span>'+_formatVisitDate(v.date)+'</span><button type="button" data-visit-del="'+i+'">삭제</button></div>'; }).join('');
...
function openDioceseView(opts){
  ...
  if(needsLoad){
    ...
    frame.src='diocese.html?v=V6-98';
...
/* V6-98: 새 창 방식으로 전환된 성지 외부 링크의 옛 core return 저장 함수 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V6-98: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V6-98: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  if(!url) return false;
  try{ document.activeElement && document.activeElement.blur && document.activeElement.blur(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V6-98: 새 창/외부 브라우저 방식은 앱 화면을 떠나지 않으므로 남은 외부 복귀 흔적만 정리한다. */
  try{
    ...
    sessionStorage.removeItem('oai_external_nav_hold_until');
    sessionStorage.removeItem('oai_external_nav_force_release_at');
    document.documentElement.classList.remove('oai-external-leaving','oai-external-return-freeze','oai-returning');
  }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{
    var opened = null;
    try{ opened = window.open(url, '_blank'); }catch(_e){ opened = null; }
    if(opened){
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V6-98: 새 창 방식에서는 현재 화면을 그대로 유지하므로 복귀 상태 저장/복원은 하지 않는다. */
...
/* V6-98: 동작 없는 빈 포커스 리스너와 빈 지도 진입 훅 제거 */
...
/* V6-98: 현재 성지 외부 링크는 새 창 방식이므로 옛 core external return 복원 로직은 제거하고,
...
const _PARISH_ASSET_VERSION='V6-98';
...
const _PRAYER_ASSET_VERSION='V6-98';
...
const _RETREAT_ASSET_VERSION='V6-98';
...
const _SHRINE_ASSET_VERSION='V6-98';
let SHRINES = [];
let JUKRIMGUL_IDX = -1;
