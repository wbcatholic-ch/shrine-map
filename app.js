...
(function(){
  ...
  function applyCoverForLongReturn(reason){
    try{
      ...
      /* V8-1-14-678: 30분 이상 복귀는 논리적 새 시작이므로 이전 검색어·검색결과를 먼저 비운다.
...
/* V8-1-14-678: 순례 상세의 '지도에서 보기'는 카드 데이터와 지도 중심을 하나의 pending target으로 함께 적용한다. */
...
function syncCoverUpdateVersionState(){
  try{
    ...
    if(!btn || !box) return;
    var target = btn.getAttribute('data-target-version') || (window.OAI_APP_BUILD_VERSION || window.APP_VERSION || 'V8-1-14-678');
    var current = '';
    /* V8-1-14-621:
...
(function(){
  ...
  function openPrivacyPolicyModal(){
    try{
      ...
      if(frame){
        var src=frame.getAttribute('data-src') || ('privacy.html?embedded=1&v=' + encodeURIComponent(window.APP_VERSION || 'V8-1-14-678'));
...
function openDioceseView(opts){
  ...
  var restore = !!(opts && opts.restore);
  var url = (typeof oaiGetDioceseFrameUrl === 'function') ? oaiGetDioceseFrameUrl() : 'diocese.html?v=V8-1-14-678';
...
function oaiGetDioceseFrameUrl(){
  return 'diocese.html?v=V8-1-14-678';
}
...
const _PARISH_ASSET_VERSION='V8-1-14-678';
...
function _isParishNearbyMapMode(){
  try{
    ...
    /* V8-1-14-678: Fold 큰 화면에서 내주변 패널이 열린 채 지도 교구명을 누르면
...
const _PRAYER_ASSET_VERSION='V8-1-14-678';
...
const _RETREAT_ASSET_VERSION='V8-1-14-678';
...
/* V8-1-14-678: 화면에 표시하는 실제 주소(addr)와 차량 길찾기 목적지(navAddr)를 분리한다.
...
const _SHRINE_ASSET_VERSION='V8-1-14-678';
...
/* V8-1-14-678: 실제 앱 새 시작(콜드 스타트/새 문서 로드)과 30분 이상 background 복귀에서만
 ...
 const KM=kakao.maps;window._LL=KM.LatLng;window._MM=KM.Marker;window._MI=KM.MarkerImage;window._SZ=KM.Size;window._PT=KM.Point;window._LB=KM.LatLngBounds;window._PL=KM.Polyline;window._EL=KM.event.addListener;
  ...
  if(!window.__OAI_MAIN_MAP_CLICK_BOUND__){
    ...
    kakao.maps.event.addListener(_map,'click',()=>{
      // V8-1-14-678: 지도 빈 곳 클릭은 패널 표시만 접고 선택 마커와 경로선은 유지한다.
  ...
  /* V8-1-14-678: 상세에서 지정한 성지가 있으면 현재 위치 갱신은 하되 지도 중심은 선택 성지에 고정한다. */
...
/* V8-1-14-678: 정보카드가 열린 동안 선택 장소의 지도 위치는 이 공통 소유자가 관리한다.
...
function _showParishNearbyMarkersOnMap(items, lat, lng, phase){
  ...
  try{
    ...
    // V8-1-14-678: 첫 진입에서는 가장 가까운 성당을 자동 선택하지 않는다.
...
function renderList(){
  ...
  items.forEach((s,i)=>{
    ...
    /* V8-1-14-678: 성당은 교구 탭이 선택되어 있어도 검색어가 있으면 전국 전체에서 찾는다.
...
function setDioFilter(v,btn){
  ...
  btn?.classList.add('active');
  _scrollDioFilterButtonIntoView(btn);
  /* V8-1-14-678: 성당 전국 검색 중 교구 탭을 눌러도 검색어와 결과를 유지한다.
...
function _syncRouteWaypointBoxes(){
  ...
  const slotStates=OAI_ROUTE_WAYPOINT_CONFIGS.map(function(cfg,index){
    ...
    return {
      ...
      has:has,
      enabled:enabled,
      // V8-1-14-678: Fold wide에서도 빈 경유지 1~5를 강제로 펼치지 않는다.
...
function resetRoute(opts){
  ...
  _hide($('rs-result'));
  _setRouteResultTipVisible(false);
  // V8-1-14-678: 다시선택에서는 경로선과 결과판을 먼저 모두 닫은 뒤 경유지 UI를 한 번만 동기화한다.
...
function _selectRouteItem(idx){
  ...
  const s=items[idx];
  if(!s||!s.lat||!s.lng) return;
  // V8-1-14-678: 사용자가 길찾기 카드를 접어 둔 상태에서 지도 마커를 선택하면
...
/* V8-1-14-678: 최초 앱 진입에서도 지도 화면과 무관하게 최신 위치를 한 번 확인한다. */
(function(){
  try{
