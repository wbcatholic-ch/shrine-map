...
function _ensureShrineVisitCardsModal(){
  ...
  modal.addEventListener('click', function(e){
    ...
    if(tab){
      ...
      _shrineVisitCardsTab=tab.getAttribute('data-shrine-visit-cards-tab')||'visited';
...
function _renderShrineVisitDetail(idx){
  ...
  body.innerHTML='<section class="shrine-visit-detail-hero"><div class="shrine-visit-detail-kicker">순례 기록</div><div class="shrine-visit-detail-count">순례 '+count+'회</div><div class="shrine-visit-detail-recent">최근 순례일 '+_visitHtmlEsc(recent)+'</div><div class="shrine-visit-detail-date-title">순례 날짜</div><div class="shrine-visit-detail-date-list">'+dateHtml+'</div></section><section class="shrine-visit-detail-info"><div class="shrine-visit-detail-info-head"><div class="shrine-visit-detail-section-title">성지 정보</div><button type="button" class="shrine-visit-detail-map-btn" data-shrine-detail-map="'+idx+'">지도에서 보기</button></div><div class="shrine-visit-detail-name">'+_visitHtmlEsc(item.name||'')+'</div><div class="shrine-visit-detail-row"><span>교구</span><strong>'+_visitHtmlEsc(item.diocese||'—')+'</strong></div><div class="shrine-visit-detail-row"><span>주소</span><strong>'+_visitHtmlEsc(item.addr||'—')+'</strong></div><div class="shrine-visit-detail-row"><span>전화</span><strong>'+telText+'</strong></div><div class="shrine-visit-detail-actions">'+primaryRow+linkRow+kakaoRow+'</div></section>';
}
...
function _setMassQuickReturn(on){
  try{
    ...
    if(on){
      ...
      /* V6-110: 매일미사/성가/성경 배너 복귀 상태는 장기 보존하지 않고 세션 안에서만 유지한다. */
...
function closeMassQuickMenu(opts){
  ...
  try{ _stabilizeCoverBackAfterFaithPortal('mass-quick-close-cover'); }catch(e){ console.warn('[가톨릭길동무]', e); }
}
...
function closeMissa(){
  ...
  if(typeof _shouldFaithReturnToMassQuick === 'function' ? _shouldFaithReturnToMassQuick() : _shouldMassQuickReturn()){
    _returnToMassQuickMenu();
  }else {
    try{ if(typeof _clearFaithReturnTarget === 'function') _clearFaithReturnTarget(); }catch(e){ console.warn('[가톨릭길동무]', e); }
    if(typeof goToCover==='function') goToCover();
    _stabilizeCoverBackAfterFaithPortal('faith-close-cover');
  }
}
...
/* V6-110: 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일하고 옛 core return 저장 함수는 제거 */
...
function _getShrineHomepageUrl(item){
  ...
  if(!hp) return '';
  /* V6-110: 신규 성지는 성지추가.xlsx의 '홈페이지' 열을 그대로 홈페이지 버튼에 연결한다. */
...
function _getShrineGuideUrl(item){
  if(!item) return '';
  /* V6-110: 성지추가.xlsx의 '주교회의 성지안내/성지 상세' URL을 우선 사용한다. */
...
function openCatholicExternalPreserveApp(url, kind){
  ...
  try{ if(typeof _resetCoverExitReady==='function') _resetCoverExitReady(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  try{ if(typeof _clearCoverExitArmed==='function') _clearCoverExitArmed(); }catch(e){ console.warn("[가톨릭길동무]", e); }
  /* V6-110: 성지·성당·피정 외부 웹사이트도 웹사이트 카테고리와 같은 보호창 이동 흐름으로 통일한다. */
  try{
...
function openShrineExternalLikeFaithPortal(url, extra){
  ...
  if(!url) return;
  extra = extra || {};
  /* V6-110: 성지 상세/홈페이지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 흐름을 사용한다. */
...
/* V6-110: 동작 없는 빈 포커스 리스너와 빈 지도 진입 훅 제거 */
...
/* V6-110: 현재 성지 외부 링크는 웹사이트 카테고리와 같은 보호창 이동 방식이므로 옛 core external return 복원 로직은 제거하고,
...
function _primeCoverExitGuardAfterCoverReturn(reason, opts){
  try{
    reason = reason || 'cover-return';
    opts = opts || {};
    function arm(tag){
      try{
        if(document.documentElement.classList.contains('app-active')) return;
        ...
        _forceNextCoverBackToast(reason + '-first-toast-' + tag);
        if(typeof window._oaiArmCoverBackTrap === 'function') window._oaiArmCoverBackTrap(reason + '-' + tag, {force:true});
        else if(typeof _resetCoverBackTrap === 'function') _resetCoverBackTrap(reason + '-' + tag);
        else if(typeof _ensureCoverBackTrap === 'function') _ensureCoverBackTrap(reason + '-' + tag);
        try{ if(typeof window.__oaiArmEarlyCoverBackGuard === 'function') window.__oaiArmEarlyCoverBackGuard(reason + '-early-' + tag, true); }catch(_e){}
      }catch(e){ console.warn('[가톨릭길동무]', e); }
    }
    _resetCoverExitReady();
    _clearCoverExitArmed();
    ...
    _forceNextCoverBackToast(reason + '-first-toast');
    var delays = opts.delays || [0, 60, 160, 320, 620];
    delays.forEach(function(delay){ setTimeout(function(){ arm(String(delay)); }, delay); });
  }catch(e){ console.warn('[가톨릭길동무]', e); }
}

function _stabilizeCoverBackAfterFaithPortal(reason){
  _primeCoverExitGuardAfterCoverReturn(reason || 'faith-portal-cover');
}
...
function goToCover(){
  ...
  try{ if(typeof _primeCoverExitGuardAfterCoverReturn === 'function') _primeCoverExitGuardAfterCoverReturn('go-to-cover'); }catch(e){ console.warn('[가톨릭길동무]', e); }
  try{
...
(function installOaiMapSearchKeyboardViewportLock(){
  ...
  function restore(reason){
    try{
      ...
      if(!el || !lock || el.id!==lock.inputId) return;
      var sheet=document.getElementById('sheet-'+lock.sheetName);
      var body=getScrollBox(lock.sheetName);
      if(window.scrollTo) window.scrollTo(0, lock.winY || 0);
      if(document.documentElement) document.documentElement.scrollTop=lock.docY || lock.winY || 0;
      if(document.body) document.body.scrollTop=lock.bodyY || 0;
      if(sheet) sheet.scrollTop=lock.sheetTop || 0;
      if(body && reason!=='input') body.scrollTop=lock.bodyTop || 0;
    }catch(e){ console.warn('[가톨릭길동무]', e); }
  }
  function start(el){
    try{
