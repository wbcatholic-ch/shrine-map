...
function oaiReleaseStabilityVeil(){
  try{
    ...
    if(reason === 'external-leave'){
      ...
      // 너무 오래 남는 것을 막기 위해 22초 안전 해제만 둔다.
...
function oaiPrepareRefreshVeil(reason, duration, carryDuration){
  try{
    var d = Math.max(260, duration || OAI_REFRESH_VEIL_MS);
    var carry = Math.max(d, carryDuration || OAI_REFRESH_CARRY_MS || d);
    var now = Date.now ? Date.now() : new Date().getTime();
    /* oai_refresh_veil_until은 “새 문서가 이 시간 안에 뜨면 보호막을 보이라”는 감지 기한이다.
...
async function _runClearAppFilesCacheCompletely(){
  try{
    ...
    if(typeof oaiHoldStabilityVeil === 'function') oaiHoldStabilityVeil('long-refresh-progress', OAI_REFRESH_PROGRESS_HOLD_MS);
  }catch(e){ console.warn('[가톨릭길동무]', e); }
  try{
...
function openPrayerBook(opts){
  ...
  // V2-S-r8: restore 변수 미정의 오류 방지. 주요기도문 초기화가 중간에 끊기면
...
function normalizeCatholicExternalUrl(url){
  ...
  try{
    ...
    // V2-S-r8: 원주·인천교구 대표 홈페이지는 공식 등록 주소가 HTTP이므로
...
function restoreDioceseExternalState(opts){
  ...
  try{
    ...
    // V2-S-r8 stable: frame.contentWindow가 있다는 이유만으로 '살아 있다'고 판단하면 안 된다.
...
// V2-S-r8: 성당 데이터를 교구별 parishes-*.js 파일로 실제 분리한다.
...
function oaiEnterView(el){
  ...
  try{
    ...
    // V2-S-r8: 화면 진입 효과는 안정막으로 덮지 않고, 실제 화면 자체를 아주 짧게 fade-in 한다.
...
function _onMapReady(){
 const KM=kakao.maps;window._LL=KM.LatLng;window._MM=KM.Marker;window._MI=KM.MarkerImage;window._SZ=KM.Size;window._PT=KM.Point;window._LB=KM.LatLngBounds;window._PL=KM.Polyline;window._EL=KM.event.addListener;
  ...
  if(!window._noAutoNearby){
    // V2-S-r8: 성당 첫 진입도 기존 기준대로 내주변 탭을 먼저 연다.
...
function _buildParishDioSystem(){
  ...
  Object.entries(_DIO_CFG).forEach(([code,cfg])=>{
    // V2-S-r8: 군종교구는 데이터/검색에는 남기되 지도 위 교구 라벨에서는 제외한다.
    if(code==='ML') return;
...
function renderList(){
  ...
  if(q){
    ...
    let matchAll=false;
    if(_mode==='parish'){
      /* V2-S-r8: 성당찾기는 선택한 교구 안에서 성당명 첫 글자 일치 또는 주소 포함으로만 찾는다. */
...
function filterModal(q){
  ...
  if(q){
    ...
    let matchAll=false;
    if(_mode==='parish'){
      /* V2-S-r8: 성당 길찾기 검색도 선택한 교구 안에서 성당명 첫 글자 일치 또는 주소 포함으로만 찾는다. */
      matchAll = nameNorm.startsWith(nq) || addrNorm.includes(nq);
    } else {
      const tokens=q.trim().split(/\s+/);
      matchAll=tokens.length>=2
